...
-- threshold columns + catégorie de réglages 'kds' sur la famille
-- get_settings_by_category / set_setting. Auth pattern mirrors
-- floor_plan_crud.test.sql: EMP000 (ADMIN) carries settings.update +
-- settings.read ("ADMIN+ via settings.update").
--
-- Adaptation vs the task brief's pseudo-SQL (DEV-S57-02 — the LIVE body is
-- authority, not the brief): la convention live de l'écriture de réglage lève
...
--
-- Lot 1.B (2026-08-03) — repointé v1 -> versions vivantes : les deux familles
-- avaient été bumpées sans que cette suite suive, et ses 13 assertions
-- tombaient en « function does not exist » à chaque run nocturne.
--
...
-- NOTE: every settings write above shares this transaction's now(), so
