...
SELECT ok(
  ...
  AND EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'refund_order_rpc_v9'),
  'T33: refund_order_rpc dropped ; refund_order_rpc_v9 exists'
);

SELECT ok(
  NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'complete_order_with_payment' AND pronamespace = 'public'::regnamespace)
  AND EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'complete_order_with_payment_v24'),
  'T34: complete_order_with_payment dropped ; complete_order_with_payment_v24 exists'
);
