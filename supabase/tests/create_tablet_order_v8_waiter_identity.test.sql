...
SELECT isnt_empty(
  $$ SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
      WHERE n.nspname = 'public' AND p.proname = 'create_tablet_order_v10' $$,
  'T2 — create_tablet_order_v10 existe'
);
...
SELECT is(
  (SELECT p.prosecdef FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public' AND p.proname = 'create_tablet_order_v10'),
  true,
  'T3 — create_tablet_order_v10 est SECURITY DEFINER'
);
...
SELECT throws_ok(
  format(
    $$ SELECT create_tablet_order_v10(
...
-- prédicat d'un SELECT : `WHERE o.id = create_tablet_order_v10(…)` ferait
