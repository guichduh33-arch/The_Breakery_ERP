...
--   VOID   : void_order_rpc_v10 — idempotency_key replay via refunds.idempotency_key.
...
-- ===========================================================================
-- VOID T1 — first void_order_rpc_v10 call with key K succeeds : order voided,
...
BEGIN
  PERFORM set_config('request.jwt.claim.sub', v_cashier_auth::text, true);
  v_order := complete_order_with_payment_v25(
  ...
  v_res := void_order_rpc_v10(v_order_id, 'S55 idempotency void test', v_manager_prof, v_cashier_auth, v_key);
...
-- ===========================================================================
-- VOID T2 — second void_order_rpc_v10 call with the SAME key K replays :
...
BEGIN
  PERFORM set_config('request.jwt.claim.sub', v_cashier_auth::text, true);
  v_res2 := void_order_rpc_v10(v_order_id, 'S55 idempotency void test replay', v_manager_prof, v_cashier_auth, v_key);
...
-- non-served order_item (cancel_order_item_rpc_v6 requires status IN ('draft','pending_payment') — ADR-009 déc. 3
...
-- ===========================================================================
-- CANCEL T3 — first cancel_order_item_rpc_v6 call with key K succeeds :
...
BEGIN
  PERFORM set_config('request.jwt.claim.sub', v_cashier_auth::text, true);
  v_res := cancel_order_item_rpc_v6(v_item, 'S55 idempotency cancel test', v_manager_prof, v_cashier_auth, v_key);
...
-- ===========================================================================
-- CANCEL T4 — second cancel_order_item_rpc_v6 call with the SAME key K
...
BEGIN
  PERFORM set_config('request.jwt.claim.sub', v_cashier_auth::text, true);
  v_res2 := cancel_order_item_rpc_v6(v_item, 'S55 idempotency cancel test replay', v_manager_prof, v_cashier_auth, v_key);
...
BEGIN
  PERFORM set_config('request.jwt.claim.sub', v_cashier_auth::text, true);
  BEGIN
    PERFORM cancel_order_item_rpc_v6(v_item, 'S55 idempotency cancel test K2', v_manager_prof, v_cashier_auth, v_key2);
