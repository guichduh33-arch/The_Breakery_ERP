...
--
-- Les deux portes qui portent le drapeau sont couvertes (`fire_counter_order_v9`,
-- `create_tablet_order_v10`). `add_order_item_v6` n'en a pas : elle n'est ni
...
-- ===========================================================================
-- PORTE COMPTOIR — fire_counter_order_v9
-- ===========================================================================
...
BEGIN
  PERFORM set_config('request.jwt.claim.sub', current_setting('a22t.tab_auth'), true);
  BEGIN
    PERFORM create_tablet_order_v10(
      p_client_uuid  := gen_random_uuid(),
      p_waiter_id    := current_setting('a22t.tab_prof')::uuid,
      p_table_number := '',
      p_order_type   := 'take_out'::order_type,
