...
// record_production_v3 -> _v4 (renamed mechanically below). The baguette recipe
...
describe.skipIf(!process.env.SUPABASE_SERVICE_ROLE_KEY)('record_production_v6 + revert_production_v2 — integration', () => {
  ...
  it('manager happy path: 50 baguettes → 5 movements + 5 balanced JEs', async () => {
    ...
    const { data, error } = await sb.rpc('record_production_v6', {
  ...
  it('idempotency replay: same key returns same production_id', async () => {
    ...
    const r1 = await sb.rpc('record_production_v6', {
    ...
    const r2 = await sb.rpc('record_production_v6', {
      p_product_id: baguette.id, p_quantity_produced: 10, p_section_id: sectionId,
      p_batch_number: 'VT-IDEM', p_quantity_waste: 0, p_notes: null, p_idempotency_key: key,
    });
  ...
  it('insufficient stock raises error with material name in message', async () => {
    ...
    try {
      const { error } = await sb.rpc('record_production_v6', {
  ...
  it('cashier role: record_production_v6 raises forbidden', async () => {
  ...
  it('admin reverts production within 24h → stock restored', async () => {
    const sb = jwtClient(managerToken);
    const { data: produced, error: prodErr } = await sb.rpc('record_production_v6', {
  ...
  it('manager cannot revert production → forbidden', async () => {
    const sb = jwtClient(managerToken);
    const { data: produced } = await sb.rpc('record_production_v6', {
