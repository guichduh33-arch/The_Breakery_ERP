...
describe.skipIf(!process.env.SUPABASE_SERVICE_ROLE_KEY)('B2B Foundation — record_b2b_payment (v3) + create_b2b_order_v1 + adjust', () => {
  ...
  beforeAll(async () => {
    ...
    // S78 (D-6) : BEV-AMER est soft-deleted — create_b2b_order le refusait
  ...
  it('S1: create_b2b_order + record_b2b_payment full → balance back to 0', async () => {
    ...
    const { data: orderResult, error: orderErr } = await rpc(sb)('create_b2b_order_v7', {
  ...
  it('S3: 2nd partial payment after order — balance reflects', async () => {
    ...
    // 1) Create order 100K
    const { data: orderRes, error: orderErr } = await rpc(sb)('create_b2b_order_v7', {
      p_customer_id: customerId,
