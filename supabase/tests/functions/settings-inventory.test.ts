...
// Verifies: get_settings_by_category_v8('inventory') returns allow_negative_stock
...
describe.skipIf(!process.env.SUPABASE_SERVICE_ROLE_KEY)('inventory settings — allow_negative_stock', () => {
  ...
  it('reads the inventory category with a boolean default', async () => {
    ...
    const { data, error } = await sb.rpc('get_settings_by_category_v8', { p_category: 'inventory' });
  ...
  it('round-trips a write through set_setting_v5', async () => {
    ...
    const { data: after, error: getErr } = await sb.rpc('get_settings_by_category_v8', { p_category: 'inventory' });
  ...
  it('cashier cannot read settings (permission denied)', async () => {
    ...
    const { error } = await sb.rpc('get_settings_by_category_v8', { p_category: 'inventory' });
    expect(error).not.toBeNull();
  });
});
