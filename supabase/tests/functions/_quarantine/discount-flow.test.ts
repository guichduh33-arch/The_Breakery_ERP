...
// Motif : appelle complete_order_with_payment + create_tablet_order + pay_existing_order (v1 nus, tous DROPPÉS live) — courants v17 (EF-only) / v4 / v11.
// Réécriture = session dédiée (hors périmètre S77). Exclu du run via vitest.config.ts (**/_quarantine/**).
// Couverture actuelle : ancres pgTAP s44_money_gates / canonical_line_price / combo_sale / reversal_idempotency + vitest process-payment.test.ts (EF).
//
...
// Kept for the `auth-verify-pin — required_permission extension` block below,
// which exercises the EF directly (that EF's gate IS the test subject).
