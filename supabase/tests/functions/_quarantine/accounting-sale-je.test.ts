...
// Motif : appelle complete_order_with_payment_v9 (DROPPÉ live) — money-path courant = complete_order_with_payment_v17 via l'EF process-payment.
// Réécriture = session dédiée (hors périmètre S77). Exclu du run via vitest.config.ts (**/_quarantine/**).
// Couverture actuelle : ancres pgTAP s44_money_gates + accounting.test.sql (JE de vente) + vitest process-payment.test.ts (EF).
//
