...
//   3. reopen_held_order_v3(order_id) → returns items[] with is_locked=true +
//      order_items.id; DB row is_held=false; order NOT deleted (status='pending_payment').
//   4. A 2nd reopen_held_order_v3(order_id) throws P0002 (already open / not held).
...
describe.skipIf(!process.env.SUPABASE_TEST_LIVE)('reopen_held_order_v3 (live)', () => {
