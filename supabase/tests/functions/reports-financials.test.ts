...
describe.skipIf(!process.env.SUPABASE_SERVICE_ROLE_KEY)('reports — financial RPCs (live)', () => {
  ...
  it.runIf(!!process.env.SUPABASE_SERVICE_ROLE_KEY)(
    'get_profit_loss_v1 returns the expected JSON shape for a 30-day window',
    async () => {
      const sb = jwtClient(adminToken);
      const end   = new Date().toISOString().slice(0, 10);
      const start = new Date(Date.now() - 30 * 86_400_000).toISOString().slice(0, 10);
      const { data, error } = await sb.rpc('get_profit_loss_v1', {
        p_date_start: start,
        p_date_end:   end,
      });
  ...
  it.runIf(!!process.env.SUPABASE_SERVICE_ROLE_KEY)(
    'get_balance_sheet_v1 returns balanced shape + computes CYE',
    async () => {
      const sb = jwtClient(adminToken);
      const today = new Date().toISOString().slice(0, 10);
      const { data, error } = await sb.rpc('get_balance_sheet_v1', {
        p_as_of_date: today,
      });
      expect(error).toBeNull();
      expect(data).toBeTypeOf('object');
