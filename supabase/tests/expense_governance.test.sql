...
--
-- Tests cover:
--   T1  : submit_expense_v3 amount=50k → auto_approved=true
--   T2  : submit_expense_v3 amount=500k → snapshot has 1 step
--   T3  : submit_expense_v3 amount=2M → snapshot has 2 steps
...
--   T17 : delete_expense_threshold_v2 → row removed
...
SELECT submit_expense_v3('eeeeeeee-0000-0000-0000-000000000001');
SELECT submit_expense_v3('eeeeeeee-0000-0000-0000-000000000002');
SELECT submit_expense_v3('eeeeeeee-0000-0000-0000-000000000003');
...
VALUES ('eeeeeeee-0000-0000-0000-000000000011', 'EXP-GOV28-011', 'aaaaaaaa-0000-0000-0000-000000000001',
        ...
        (SELECT id FROM user_profiles WHERE auth_user_id = '00000000-0000-0000-0000-000000000004' LIMIT 1), 'draft');
SELECT submit_expense_v3('eeeeeeee-0000-0000-0000-000000000011');
...
VALUES ('eeeeeeee-0000-0000-0000-000000000015', 'EXP-GOV28-015', 'aaaaaaaa-0000-0000-0000-000000000001',
        ...
        (SELECT id FROM user_profiles WHERE auth_user_id = '00000000-0000-0000-0000-000000000004' LIMIT 1), 'draft');
SELECT submit_expense_v3('eeeeeeee-0000-0000-0000-000000000015');
...
SET LOCAL "request.jwt.claims" = '{"sub":"00000000-0000-0000-0000-000000000001"}';
SELECT approve_expense_v4('eeeeeeee-0000-0000-0000-000000000016', '123456');
...
SET LOCAL "request.jwt.claims" = '{"sub":"00000000-0000-0000-0000-000000000004"}';
SELECT throws_ok(
  $$ SELECT approve_expense_v4('eeeeeeee-0000-0000-0000-000000000002', '123456') $$,
  'P0001', NULL,
...
-- T19: approve_expense_v4 with a WRONG manager PIN → P0003 invalid_pin (H1 fix).
...
SET LOCAL "request.jwt.claims" = '{"sub":"00000000-0000-0000-0000-000000000001"}';
SELECT throws_ok(
  $$ SELECT approve_expense_v4('eeeeeeee-0000-0000-0000-000000000002', '999999') $$,
  'P0003', NULL,
  'T19 : wrong manager PIN → P0003 invalid_pin'
);

-- T20: approve_expense_v4 with NULL manager PIN → P0001 pin_required (H1 fix).
SELECT throws_ok(
  $$ SELECT approve_expense_v4('eeeeeeee-0000-0000-0000-000000000002', NULL) $$,
  'P0001', NULL,
