...
-- _resolve_combo_price_v1, exercised through complete_order_with_payment_v24.
--
...
BEGIN
  ...
  IF v_sess IS NULL THEN
    v_sess := '00000000-0000-0000-0000-0000000c5001';
...
INSERT INTO products (id, sku, name, category_id, retail_price, product_type, current_stock, track_inventory, combo_base_price) VALUES
  ('00000000-0000-0000-0000-0000000cb001','S57-CB1','S57 Combo Pricing','9c751b3c-2cbf-49a9-a442-cc6a4b5ffc4a',0,'combo',0,false,40000),
  ('00000000-0000-0000-0000-0000000fb001','S57-FP1','S57 Regular Size','9c751b3c-2cbf-49a9-a442-cc6a4b5ffc4a',15000,'finished',100,true,NULL),
  ('00000000-0000-0000-0000-0000000fb002','S57-FP2','S57 Large Size','9c751b3c-2cbf-49a9-a442-cc6a4b5ffc4a',15000,'finished',100,true,NULL),
  ('00000000-0000-0000-0000-0000000fb003','S57-FP3','S57 Water','9c751b3c-2cbf-49a9-a442-cc6a4b5ffc4a',10000,'finished',100,true,NULL),
  ('00000000-0000-0000-0000-0000000fb004','S57-FP4','S57 Non-combo standalone','9c751b3c-2cbf-49a9-a442-cc6a4b5ffc4a',20000,'finished',100,true,NULL),
  -- Not a member of any combo_group_options -> used for T2 (combo_invalid_component).
  ('00000000-0000-0000-0000-0000000fb099','S57-FP99','S57 Outsider Product','9c751b3c-2cbf-49a9-a442-cc6a4b5ffc4a',5000,'finished',100,true,NULL);
...
INSERT INTO combo_groups (id, combo_product_id, name, group_type, is_required, min_select, max_select, sort_order) VALUES
  ('00000000-0000-0000-0000-0000000c9100','00000000-0000-0000-0000-0000000cb001','Size','single',true,1,1,0),
  ('00000000-0000-0000-0000-0000000c9101','00000000-0000-0000-0000-0000000cb001','Drink','single',true,1,1,1);
INSERT INTO combo_group_options (group_id, component_product_id, surcharge, is_default, sort_order) VALUES
  ('00000000-0000-0000-0000-0000000c9100','00000000-0000-0000-0000-0000000fb001',0,   true,  0),
  ('00000000-0000-0000-0000-0000000c9100','00000000-0000-0000-0000-0000000fb002',8000,false, 1),
  ('00000000-0000-0000-0000-0000000c9101','00000000-0000-0000-0000-0000000fb003',0,   true,  0);
...
BEGIN
  r := complete_order_with_payment_v24(
    ...
    p_items := '[{"product_id":"00000000-0000-0000-0000-0000000cb001","quantity":1,"unit_price":40000,"modifiers":[],
                  "combo_components":[{"product_id":"00000000-0000-0000-0000-0000000fb002","quantity":1},
                                      {"product_id":"00000000-0000-0000-0000-0000000fb003","quantity":1}]}]'::jsonb,
...
BEGIN
  p := get_customer_product_price('00000000-0000-0000-0000-0000000fb004', NULL);
  r := complete_order_with_payment_v24(
    p_session_id := current_setting('csp.sess')::uuid,
    p_order_type := 'take_out'::order_type,
    p_items := ('[{"product_id":"00000000-0000-0000-0000-0000000fb004","quantity":1,"unit_price":'||p||',"modifiers":[]}]')::jsonb,
...
SELECT throws_ok($q$ SELECT complete_order_with_payment_v24(
    ...
    p_items := '[{"product_id":"00000000-0000-0000-0000-0000000cb001","quantity":1,"unit_price":40000,"modifiers":[],
                  "combo_components":[{"product_id":"00000000-0000-0000-0000-0000000fb099","quantity":1},
                                      {"product_id":"00000000-0000-0000-0000-0000000fb003","quantity":1}]}]'::jsonb,
...
SELECT throws_ok($q$ SELECT complete_order_with_payment_v24(
    p_session_id := current_setting('csp.sess')::uuid, p_order_type := 'take_out'::order_type,
    p_items := '[{"product_id":"00000000-0000-0000-0000-0000000cb001","quantity":1,"unit_price":40000,"modifiers":[],
                  "combo_components":[{"product_id":"00000000-0000-0000-0000-0000000fb003","quantity":1}]}]'::jsonb,
    p_payment := '{"method":"cash","amount":40000,"cash_received":40000,"change_given":0}'::jsonb) $q$,
