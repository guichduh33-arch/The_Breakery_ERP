...
-- ADR-007 déc. 5 — pgTAP suite for upsert_section_v2 / delete_section_v2
-- (migration _206).
--
-- Coverage (7 asserts) :
...
BEGIN
  ...
  v_result := upsert_section_v2(jsonb_build_object(
...
SELECT throws_ok(
  $q$SELECT upsert_section_v2('{"code":"NOPE","name":"nope","kind":"sales"}'::JSONB)$q$,
  '42501',
  'permission_denied',
...
SELECT throws_ok(
  $q$SELECT upsert_section_v2('{"code":"BAD","name":"bad","kind":"spaceship"}'::JSONB)$q$,
  '22023',
  'invalid_kind',
...
DECLARE
  v_result JSONB;
BEGIN
  v_result := delete_section_v2(current_setting('breakery.t206_section_id')::UUID);
...
-- T6 : update d'une section supprimée → P0002.
SELECT throws_ok(
  format($q$SELECT upsert_section_v2(jsonb_build_object(
