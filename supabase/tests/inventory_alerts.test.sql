...
--   06 — get_product_dashboard_v1 exists + forbids anonymous.
--   07 — get_product_dashboard_v1 raises product_not_found on bad uuid.
...
SELECT * FROM (VALUES
  ...
  ((SELECT throws_ok($$SELECT get_product_dashboard_v1(gen_random_uuid())$$, 'P0003', NULL, 'T_ALERT_06'))),
  ...
  ((SELECT ok((SELECT prosrc FROM pg_proc WHERE proname='get_product_dashboard_v1') LIKE '%product_not_found%', 'T_ALERT_07: product_not_found raise present')))
) AS t(line);
