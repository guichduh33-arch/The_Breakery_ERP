...
BEGIN
  r := complete_order_with_payment_v22(
    ...
    p_payment := '{"method":"cash","amount":40000,"cash_received":40000,"change_given":0}'::jsonb);
  PERFORM void_order_rpc_v9((r->>'order_id')::uuid, 'modifier void test', '00000000-0000-0000-0000-000000000004', '00000000-0000-0000-0000-000000000002');
END $$;
...
BEGIN
  ...
  PERFORM refund_order_rpc_v9((r->>'order_id')::uuid,
...
BEGIN
  ...
  PERFORM void_order_rpc_v9((r->>'order_id')::uuid, 'display modifier void', '00000000-0000-0000-0000-000000000004', '00000000-0000-0000-0000-000000000002');
END $$;
...
SELECT throws_ok($q$
  SELECT complete_order_with_payment_v22(
    p_session_id := '00000000-0000-0000-0000-00000000e201', p_order_type := 'take_out'::order_type,
