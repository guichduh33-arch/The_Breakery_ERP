...
-- ADR-006 déc. 9 — PIN policy (migration _220) : set_setting_v11 clés
-- pin_max_failed [3,10] / pin_lockout_minutes [5,120], readback catégorie
-- security via get_settings_by_category_v9, CHECKs table, audit, ACL.
...
DO $$ DECLARE v JSONB; BEGIN
  v := get_settings_by_category_v9('security')->'settings';
...
DO $$ DECLARE v JSONB; BEGIN
  PERFORM set_setting_v11('pin_max_failed', '3'::jsonb, 'security');
  PERFORM set_setting_v11('pin_lockout_minutes', '30'::jsonb, 'security');
  v := get_settings_by_category_v9('security')->'settings';
...
DO $$ BEGIN
  PERFORM set_setting_v11('pin_max_failed', '"5"'::jsonb, 'security');
...
DO $$ DECLARE v_ok BOOLEAN := true; BEGIN
  BEGIN
    PERFORM set_setting_v11('pin_max_failed', '2'::jsonb, 'security');
    v_ok := false;
  EXCEPTION WHEN SQLSTATE '22023' THEN NULL; END;
  BEGIN
    PERFORM set_setting_v11('pin_max_failed', '11'::jsonb, 'security');
    v_ok := false;
  EXCEPTION WHEN SQLSTATE '22023' THEN NULL; END;
  BEGIN
    PERFORM set_setting_v11('pin_max_failed', '5.5'::jsonb, 'security');
    v_ok := false;
  EXCEPTION WHEN SQLSTATE '22023' THEN NULL; END;
  BEGIN
    PERFORM set_setting_v11('pin_lockout_minutes', '121'::jsonb, 'security');
    v_ok := false;
  EXCEPTION WHEN SQLSTATE '22023' THEN NULL; END;
  BEGIN
    PERFORM set_setting_v11('pin_lockout_minutes', '4'::jsonb, 'security');
    v_ok := false;
  EXCEPTION WHEN SQLSTATE '22023' THEN NULL; END;
