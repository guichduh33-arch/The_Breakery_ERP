...
-- get_settings_by_category_v10, ecriture de reglage round-trip + validation + audit.
...
DO $$ DECLARE v JSONB; BEGIN
  v := get_settings_by_category_v10('printing')->'settings';
...
DO $$ DECLARE v JSONB; BEGIN
  PERFORM set_setting_v13('display_slogan', to_jsonb('Test slogan'::text), 'customer_display');
  PERFORM set_setting_v13('display_footer_message', to_jsonb(''::text), 'customer_display');
  v := get_settings_by_category_v10('customer_display')->'settings';
...
DO $$ DECLARE v JSONB; BEGIN
  PERFORM set_setting_v13('pos_auto_print_receipt', 'false'::jsonb, 'printing');
  v := get_settings_by_category_v10('printing')->'settings';
...
DO $$ DECLARE v_ok BOOLEAN := true; BEGIN
  BEGIN
    PERFORM set_setting_v13('display_slogan', '42'::jsonb, 'customer_display');
    v_ok := false;
  EXCEPTION WHEN SQLSTATE '22023' THEN
    v_ok := v_ok AND SQLERRM = 'setting_type_invalid';
  END;
  BEGIN
    PERFORM set_setting_v13('pos_auto_open_drawer', '"yes"'::jsonb, 'printing');
    v_ok := false;
  EXCEPTION WHEN SQLSTATE '22023' THEN
    v_ok := v_ok AND SQLERRM = 'setting_type_invalid';
  END;
...
DO $$ BEGIN
  PERFORM set_setting_v13('display_slogan', to_jsonb(repeat('x', 81)), 'customer_display');
