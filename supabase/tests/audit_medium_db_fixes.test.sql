...
--   M2 : calculate_pb1_payable_v3 sums status IN ('posted','locked') (Lot D1 : _v1 → _v2)
...
-- M2: calculate_pb1_payable_v3 body sums status IN (posted, locked)
...
SELECT ok(
  pg_get_functiondef('calculate_pb1_payable_v3(date,date)'::regprocedure) LIKE '%''posted'', ''locked''%',
  'M2 : calculate_pb1_payable_v3 sums status IN (posted, locked)'
);
