...
--   T1. fire_counter_order_v9 numérote P<DDMMYYYY><NNN> (défaut 'P').
--   T2. create_tablet_order_v10 numérote T2… quand la tablette envoie T2.
...
BEGIN
  ...
  v_tablet := create_tablet_order_v10(
...
BEGIN
  v_first := fire_counter_order_v9(
    v_uuid,
    (SELECT session_id FROM _fx),
    jsonb_build_array(jsonb_build_object(
      'product_id', (SELECT product_id FROM _fx), 'quantity', 1, 'unit_price', 35000, 'modifiers', '[]'::jsonb))
  );
  v_second := fire_counter_order_v9(v_uuid, (SELECT session_id FROM _fx), '[]'::jsonb);
