...
-- S44 Wave B/D — pay_existing_order_v16 : display_stock parity (P1-C), JE split
...
BEGIN
  ...
  PERFORM pay_existing_order_v16(p_order_id := v_oid, p_payment := jsonb_build_object('method','cash','amount',40000,'cash_received',40000,'change_given',0));
...
BEGIN
  ...
  PERFORM pay_existing_order_v16(p_order_id := v_oid, p_payment := jsonb_build_object('method','cash','amount',35000,'cash_received',35000,'change_given',0));
...
BEGIN
  ...
  PERFORM pay_existing_order_v16(p_order_id := v_oid, p_payment := jsonb_build_object('method','qris','amount',35000));
...
BEGIN
  ...
  BEGIN PERFORM pay_existing_order_v16(p_order_id := v_oid, p_payment := jsonb_build_object('method','cash','amount',35000,'cash_received',50000,'change_given',20000));
...
BEGIN
  ...
  BEGIN PERFORM pay_existing_order_v16(p_order_id := v_oid,
...
BEGIN
  ...
  v_env := pay_existing_order_v16(p_order_id := v_oid,
...
BEGIN
  ...
  PERFORM pay_existing_order_v16(p_order_id := v_oid, p_payment := jsonb_build_object('method','cash','amount',20000,'cash_received',20000,'change_given',0));
  SELECT quantity::int INTO v_q0 FROM display_stock WHERE product_id=current_setting('s44.disp')::uuid;
  PERFORM void_order_rpc_v8(v_oid, 's44 void test', current_setting('s44.voider')::uuid, current_setting('s44.auth')::uuid);
