...
-- S44 Wave B/D — pay_existing_order_v17 : display_stock parity (P1-C), JE split
...
BEGIN
  ...
  PERFORM pay_existing_order_v17(p_order_id := v_oid, p_payment := jsonb_build_object('method','cash','amount',40000,'cash_received',40000,'change_given',0));
...
BEGIN
  ...
  PERFORM pay_existing_order_v17(p_order_id := v_oid, p_payment := jsonb_build_object('method','cash','amount',35000,'cash_received',35000,'change_given',0));
...
BEGIN
  ...
  PERFORM pay_existing_order_v17(p_order_id := v_oid, p_payment := jsonb_build_object('method','qris','amount',35000));
...
BEGIN
  ...
  BEGIN PERFORM pay_existing_order_v17(p_order_id := v_oid, p_payment := jsonb_build_object('method','cash','amount',35000,'cash_received',50000,'change_given',20000));
...
BEGIN
  ...
  BEGIN PERFORM pay_existing_order_v17(p_order_id := v_oid,
...
BEGIN
  ...
  v_env := pay_existing_order_v17(p_order_id := v_oid,
...
BEGIN
  ...
  PERFORM pay_existing_order_v17(p_order_id := v_oid, p_payment := jsonb_build_object('method','cash','amount',20000,'cash_received',20000,'change_given',0));
