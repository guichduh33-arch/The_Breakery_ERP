...
--
-- Coverage (4 asserts) :
--   1. Direct UPDATE cost_price as `authenticated` raises 42501 insufficient_privilege.
--   2. update_cost_price_v1 as SUPER_ADMIN happy path : returns movement_id +
--      cost_price actually mutated.
--   3. receive_stock_v1 still updates cost_price via the WAC trigger
--      (regression guard — REVOKE column UPDATE must not break this path).
--   4. The cost_price_correction audit row was emitted with the expected payload.
--
