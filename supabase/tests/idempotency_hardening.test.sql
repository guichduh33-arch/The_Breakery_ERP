...
--   - create_tablet_order_v9 (idempotent replay via p_client_uuid ; +p_notes -> orders.notes)
...
--
-- T1  create_tablet_order_v9 first call with a fresh client_uuid → succeeds, returns UUID order_id
-- T2  create_tablet_order_v9 same p_client_uuid second call → returns SAME order_id,
...
-- T8  create_tablet_order_v9 EXECUTE REVOKE — anon has NO EXECUTE privilege
-- T9  toutes les versions antérieures de create_tablet_order sont droppées
-- T10 create_tablet_order_v9 with p_notes writes orders.notes verbatim
--
...
-- ===========================================================================
-- T1 — create_tablet_order_v9 first call with a fresh client_uuid succeeds
-- ===========================================================================
DO $t1$
DECLARE
...
SELECT ok(current_setting('breakery.t1_pass')::boolean,
  'T1: create_tablet_order_v9 first call with fresh client_uuid returns a UUID order_id');

-- ===========================================================================
-- T2 — create_tablet_order_v9 same p_client_uuid second call → replay
-- ===========================================================================
DO $t2$
DECLARE
  v_admin_uid     UUID := current_setting('breakery.admin_uid')::uuid;
...
BEGIN
  ...
  v_order_id_2 := create_tablet_order_v9(
    p_client_uuid  => v_client,
    p_waiter_id    => v_admin_pid,
...
SELECT ok(current_setting('breakery.t2_pass')::boolean,
  'T2: create_tablet_order_v9 replay returns same order_id, 1 orders row, 1 idempotency_keys row');
...
-- ===========================================================================
-- T8 — create_tablet_order_v9 EXECUTE REVOKE: anon has NO EXECUTE
-- ===========================================================================
SELECT ok(
  NOT has_function_privilege(
    'anon',
    'public.create_tablet_order_v9(uuid, uuid, text, order_type, jsonb, text, uuid, boolean, text)',
    'EXECUTE'
  ),
  'T8: anon has NO EXECUTE privilege on create_tablet_order_v9'
);
...
SELECT ok(current_setting('breakery.t10_pass')::boolean,
  'T10: create_tablet_order_v9 writes p_notes verbatim to orders.notes');
