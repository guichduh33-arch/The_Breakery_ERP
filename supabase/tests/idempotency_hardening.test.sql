...
-- Session 59 (17 D1.1) — bump vers create_tablet_order_v4 (+p_notes) ; v2 droppée.
-- Lot 1.B (2026-08-03) — repointé sur create_tablet_order_v5 : v4 avait été
-- droppée par le bump « tournée supplémentaire » (+p_order_id) sans que cette
-- suite suive, et les 10 assertions tombaient en « function does not exist ».
--
...
--   - create_tablet_order_v8 (idempotent replay via p_client_uuid ; +p_notes -> orders.notes)
...
--
-- T1  create_tablet_order_v8 first call with a fresh client_uuid → succeeds, returns UUID order_id
-- T2  create_tablet_order_v8 same p_client_uuid second call → returns SAME order_id,
...
-- T8  create_tablet_order_v8 EXECUTE REVOKE — anon has NO EXECUTE privilege
-- T9  toutes les versions antérieures de create_tablet_order sont droppées
-- T10 create_tablet_order_v8 with p_notes writes orders.notes verbatim
--
...
-- ===========================================================================
-- T1 — create_tablet_order_v8 first call with a fresh client_uuid succeeds
-- ===========================================================================
DO $t1$
DECLARE
...
BEGIN
  ...
  v_order_id := create_tablet_order_v8(
...
SELECT ok(current_setting('breakery.t1_pass')::boolean,
  'T1: create_tablet_order_v8 first call with fresh client_uuid returns a UUID order_id');

-- ===========================================================================
-- T2 — create_tablet_order_v8 same p_client_uuid second call → replay
-- ===========================================================================
DO $t2$
DECLARE
  v_admin_uid     UUID := current_setting('breakery.admin_uid')::uuid;
...
BEGIN
  ...
  v_order_id_2 := create_tablet_order_v8(
    p_client_uuid  => v_client,
    p_waiter_id    => v_admin_pid,
...
SELECT ok(current_setting('breakery.t2_pass')::boolean,
  'T2: create_tablet_order_v8 replay returns same order_id, 1 orders row, 1 idempotency_keys row');
...
-- ===========================================================================
-- T8 — create_tablet_order_v8 EXECUTE REVOKE: anon has NO EXECUTE
-- ===========================================================================
SELECT ok(
  NOT has_function_privilege(
    'anon',
    'public.create_tablet_order_v8(uuid, uuid, text, order_type, jsonb, text, uuid, boolean, text)',
    'EXECUTE'
  ),
  'T8: anon has NO EXECUTE privilege on create_tablet_order_v8'
);

-- ===========================================================================
-- T9 — versionnage monotone : aucune version antérieure ne survit
-- ===========================================================================
SELECT is(
  (SELECT count(*)::int FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public'
      AND p.proname IN ('create_tablet_order', 'create_tablet_order_v2',
                        'create_tablet_order_v3', 'create_tablet_order_v4',
...
SELECT ok(current_setting('breakery.t10_pass')::boolean,
  'T10: create_tablet_order_v8 writes p_notes verbatim to orders.notes');
