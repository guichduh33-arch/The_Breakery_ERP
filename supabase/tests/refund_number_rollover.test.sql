...
SELECT is(
  (SELECT r->>'idempotent_replay' FROM refund_order_rpc_v9(
...
SELECT ok(
  ...
  AND EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'refund_order_rpc_v9' AND pronamespace = 'public'::regnamespace)
  AND EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'void_order_rpc_v9' AND pronamespace = 'public'::regnamespace)
  AND NOT has_function_privilege('authenticated', 'public.refund_order_rpc_v9(uuid, jsonb, jsonb, text, uuid, uuid, uuid)', 'EXECUTE')
  AND NOT has_function_privilege('authenticated', 'public.void_order_rpc_v9(uuid, text, uuid, uuid, uuid)', 'EXECUTE'),
