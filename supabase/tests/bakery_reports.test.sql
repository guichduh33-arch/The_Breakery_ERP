...
-- bump calculate_pb1_payable_v3 gatée, 20260818000007 — v1 droppée ; même
-- signature, même gate, aucune assertion ne dépend du changement).
--   T7  : get_pb1_report_v3 happy month — 8-key JSONB
--   T8  : get_pb1_report_v3 CASHIER denied (42501)
--   T9  : get_pb1_report_v3 month=13 raises 22023
...
BEGIN
  SET LOCAL "request.jwt.claims" = '{"sub":"00000000-0000-0000-0000-000000000004"}';
  v_result := get_pb1_report_v3(5, 2026);
...
SELECT ok(
  ...
  'T7: get_pb1_report_v3 happy month returns 8-key JSONB with balance_account_code=2110'
);
...
BEGIN
  SET LOCAL "request.jwt.claims" = '{"sub":"00000000-0000-0000-0000-000000000002"}';
  BEGIN
    PERFORM get_pb1_report_v3(5, 2026);
  EXCEPTION WHEN insufficient_privilege THEN
    v_caught := true;
  END;
...
SELECT ok(
  ...
  'T8: get_pb1_report_v3 CASHIER raises 42501'
);
...
BEGIN
  SET LOCAL "request.jwt.claims" = '{"sub":"00000000-0000-0000-0000-000000000004"}';
  BEGIN
    PERFORM get_pb1_report_v3(13, 2026);
