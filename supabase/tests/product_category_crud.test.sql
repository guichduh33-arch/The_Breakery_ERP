...
-- Session 27b — pgTAP suite for create_product_v3 + categories CRUD.
--
-- Coverage (10 asserts) :
--   T1   MANAGER create_product_v3 happy path : products row created
--   T1b  product_unit_contexts seeded with base unit on create
--   T2   CASHIER cannot create_product_v3 (42501)
--   T3   missing sku raises 22023 missing_required_fields
--   T4   duplicate sku raises 23505 sku_taken
--   T5   create_category_v2 auto-slugify produces lowercase hyphenated slug
--   T6   empty name raises 22023
--   T7   reorder_categories_v2 assigns 10, 20, ... in the given order
...
BEGIN
  ...
  v_result := create_product_v3(jsonb_build_object(
...
INSERT INTO pgtap_results(line)
  SELECT is(
    ...
    'T1 MANAGER create_product_v3 happy path : products row created with matching sku'
  );
...
INSERT INTO pgtap_results(line)
  SELECT throws_ok(
    format(
      $q$SELECT create_product_v3(jsonb_build_object(
...
-- T5 + T6 : create_category_v2
DO $t5$
DECLARE
  v_result JSONB;
  v_name TEXT := 'S27B Cat ' || substr(gen_random_uuid()::TEXT, 1, 6);
BEGIN
  v_result := create_category_v2(jsonb_build_object('name', v_name));
...
INSERT INTO pgtap_results(line)
  SELECT throws_ok(
    $q$SELECT create_category_v2(jsonb_build_object('name', ''))$q$,
    '22023',
    'missing_required_fields',
    'T6 empty name raises 22023'
  );

-- T7 + T8 + T9 : reorder_categories_v2
DO $t7$
DECLARE
  v_reversed UUID[];
  v_first UUID;
...
BEGIN
  ...
  PERFORM reorder_categories_v2(v_reversed);
...
INSERT INTO pgtap_results(line)
  SELECT throws_ok(
    $q$SELECT reorder_categories_v2(ARRAY[gen_random_uuid()])$q$,
    '22023',
...
INSERT INTO pgtap_results(line)
  SELECT throws_ok(
    format(
      $q$SELECT reorder_categories_v2(ARRAY[%L::UUID, %L::UUID])$q$,
