...
--   * pay_existing_order_v20 : p_payments multi-règlements + p_offline_replay,
...
-- 11: pay_existing_order_v20 porte p_payments ET p_offline_replay.
SELECT ok(
  (SELECT pg_get_function_identity_arguments(p.oid) LIKE '%p_payments jsonb%'
   ...
   WHERE n.nspname = 'public' AND p.proname = 'pay_existing_order_v20'),
  'pay_existing_order_v20 carries p_payments and p_offline_replay');
...
SELECT ok(
  (SELECT prosrc LIKE '%Invalid tender count%'
   FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = 'pay_existing_order_v20'),
  'pay_existing_order_v20 enforces the 1..5 tender bound');
...
SELECT ok(
  (SELECT bool_and(NOT has_function_privilege('anon', p.oid, 'EXECUTE'))
   FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public'
     AND p.proname IN ('set_setting_v13', 'get_settings_by_category_v10', 'pay_existing_order_v20')),
  'anon has no EXECUTE on set_setting_v13 / get_settings_by_category_v10 / pay_existing_order_v20');
