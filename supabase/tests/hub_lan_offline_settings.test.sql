...
-- pay_existing_order_v14 (p_offline_replay, arbitrage A4). Auth pattern :
...
-- 12: pay_existing_order_v14 porte p_offline_replay boolean (A4).
SELECT ok(
  (SELECT pg_get_function_identity_arguments(p.oid) LIKE '%p_offline_replay boolean%'
   FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = 'pay_existing_order_v14'),
  'pay_existing_order_v14 signature carries p_offline_replay boolean');
...
SELECT ok(
  (SELECT bool_and(NOT has_function_privilege('anon', p.oid, 'EXECUTE'))
   FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public'
     AND p.proname IN ('set_setting_v5', 'get_settings_by_category_v7', 'pay_existing_order_v14')),
  'anon has no EXECUTE on set_setting_v5 / get_settings_by_category_v7 / pay_existing_order_v14');
