...
-- set_setting_v5 + catégorie 'network' de get_settings_by_category_v4, et
...
SELECT is(
  get_settings_by_category_v4('network')->'settings',
...
SELECT ok(
  (SELECT bool_and(NOT has_function_privilege('anon', p.oid, 'EXECUTE'))
   FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public'
     AND p.proname IN ('set_setting_v5', 'get_settings_by_category_v4', 'pay_existing_order_v13')),
  'anon has no EXECUTE on set_setting_v5 / get_settings_by_category_v4 / pay_existing_order_v13');
