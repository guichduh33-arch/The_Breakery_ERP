-- 20260816000004_bump_create_b2b_order_v6_source_numbering.sql
-- Numérotation par origine (voir 20260816000001) — porte back office (B2B).
-- v6 = v5 (corps live pg_get_functiondef, verbatim) + order_number au format
-- BO<DDMMYYYY><NNN> (remplace 'B2B-YYYYMMDD-NNNN', même séquence quotidienne
-- partagée order_sequences). L'invoice_number (_next_b2b_invoice_number_v1)
-- reste inchangé — c'est lui la référence de facturation B2B.
-- Label rpc_version v5-s69 → v6.

CREATE OR REPLACE FUNCTION public.create_b2b_order_v6(p_customer_id uuid, p_items jsonb, p_notes text DEFAULT NULL::text, p_delivery_date date DEFAULT NULL::date, p_idempotency_key uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DROP FUNCTION IF EXISTS public.create_b2b_order_v5(uuid, jsonb, text, date, uuid);

REVOKE ALL ON FUNCTION public.create_b2b_order_v6(uuid, jsonb, text, date, uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.create_b2b_order_v6(uuid, jsonb, text, date, uuid) FROM anon;
GRANT EXECUTE ON FUNCTION public.create_b2b_order_v6(uuid, jsonb, text, date, uuid) TO authenticated;
ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;

COMMENT ON FUNCTION public.create_b2b_order_v6(uuid, jsonb, text, date, uuid) IS
  'v6 = v5 + numérotation par origine : order_number BO<DDMMYYYY><NNN> sur la séquence quotidienne partagée order_sequences (remplace B2B-YYYYMMDD-NNNN). invoice_number inchangé. Aucun autre changement de comportement.';
