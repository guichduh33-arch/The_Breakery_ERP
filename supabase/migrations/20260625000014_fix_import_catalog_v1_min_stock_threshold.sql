-- 20260625000014_fix_import_catalog_v1_min_stock_threshold.sql
-- S41 corrective — DEV-S41-T5-01
-- Bug: INSERT into products passes r.min_stock_threshold directly (NULL when not
-- provided by the payload). The column is NOT NULL with DEFAULT 0, so a plain
-- INSERT with an explicit NULL overrides the DEFAULT and raises 23502.
-- Fix: wrap with COALESCE(r.min_stock_threshold, 0) on the INSERT path.
-- The UPDATE path already uses COALESCE correctly (line 600 of _011).
  ...
  FOR r IN SELECT * FROM t_item ORDER BY row_num LOOP
    ...
    IF v_pid IS NULL THEN
      INSERT INTO products (
        ...
        COALESCE(r.min_stock_threshold, 0), r.shelf_life_hours  -- FIX: was r.min_stock_threshold (NULL override)
