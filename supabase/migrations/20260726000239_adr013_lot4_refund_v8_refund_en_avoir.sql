-- ADR-013 Lot 4 (D6) — refund_order_rpc v7 -> v8 : refund emis en avoir.
-- Le tender store_credit est exempte du plafond PAR METHODE (conversion de
-- n'importe quel canal paye en avoir) mais exige un client rattache (P0015) ;
-- la part store_credit credite le ledger via _apply_store_credit_v1 (expiry
-- posee selon business_config). Le JE CR 2220 vient du trigger differe
-- fn_create_je_for_refund (repoint _232). Plafond global inchange.
-- Corps de depart = pg_get_functiondef live v7 (2026-07-26). DROP v7 ici.
CREATE OR REPLACE FUNCTION public.refund_order_rpc_v8(p_order_id uuid, p_lines jsonb, p_tenders jsonb, p_reason text, p_authorized_by uuid, p_idempotency_key uuid, p_acting_auth_user_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
END $function$;
-- Défense en profondeur : réplique l'ACL de v7 (service_role uniquement).
REVOKE ALL ON FUNCTION public.refund_order_rpc_v8(uuid, jsonb, jsonb, text, uuid, uuid, uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.refund_order_rpc_v8(uuid, jsonb, jsonb, text, uuid, uuid, uuid) FROM anon;
REVOKE ALL ON FUNCTION public.refund_order_rpc_v8(uuid, jsonb, jsonb, text, uuid, uuid, uuid) FROM authenticated;
ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.refund_order_rpc_v8(uuid, jsonb, jsonb, text, uuid, uuid, uuid) TO service_role;

-- Versioning monotone : DROP de la version précédente dans la même migration.
DROP FUNCTION IF EXISTS public.refund_order_rpc_v7(uuid, jsonb, jsonb, text, uuid, uuid, uuid);

COMMENT ON FUNCTION public.refund_order_rpc_v8(uuid, jsonb, jsonb, text, uuid, uuid, uuid) IS
  'Refund money-path (EF refund-order uniquement, service_role). v8 ADR-013 Lot 4 : refund-en-avoir (tender store_credit -> ledger + CR 2220, P0015 client requis).';
