-- 20260727000250_adr008_d4_record_batch_production_v3_v4.sql
--
-- ADR-008 D4 — Production à stock insuffisant : blocage par défaut.
--
-- Étape 3/3 : la production par lot suit la même règle que la production unitaire.
--
-- La chaîne à deux étages est conservée telle que l'ADR-008 D9.4 la décrit
-- (« le wrapper est l'entrée canonique ; l'implémentation reste interne ») ;
-- la numérotation monotone du projet impose simplement de nouveaux numéros :
--
--   record_batch_production_v1 (impl)    → record_batch_production_v3 (impl)
--   record_batch_production_v2 (wrapper) → record_batch_production_v4 (wrapper)
--
-- Corps repris du LIVE. Différences fonctionnelles :
--   1. L'impl ne lit plus `business_config.allow_negative_stock` (réglage de
--      vente) : le contrôle de pénurie agrégé BLOQUE le lot par défaut.
--   2. `p_batch.force_negative` (booléen JSON, validé au bord) est la seule
--      échappatoire, gatée par `inventory.production.force_negative`, tracée
--      dans `audit_logs`, et propagée à `record_production_v2`.
--
-- Sécurité : seul le wrapper v4 est exposé à `authenticated`. L'impl v3 n'est
-- appelable que par le wrapper (SECURITY DEFINER, propriétaire postgres) — ce
-- qui ferme l'accès client direct visé par ADR-008 D9.4.

CREATE OR REPLACE FUNCTION public.record_batch_production_v3(p_batch jsonb, p_items jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- ADR-008 D4 : forçage explicite, validé au bord puis gaté par sa permission.
  ...
  FOR v_agg IN SELECT * FROM _batch_items ORDER BY item_idx LOOP
    ...
    v_rec_result := record_production_v2(
...
CREATE OR REPLACE FUNCTION public.record_batch_production_v4(p_batch jsonb, p_items jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  v_result := record_batch_production_v3(p_batch, p_items);
...
COMMENT ON FUNCTION public.record_batch_production_v3(jsonb, jsonb)
  IS 'ADR-008 D4 — implémentation interne du lot de production. Bloque en stock insuffisant ; p_batch.force_negative (permission inventory.production.force_negative) est la seule échappatoire. Appelée uniquement par record_batch_production_v4.';
COMMENT ON FUNCTION public.record_batch_production_v4(jsonb, jsonb)
  IS 'ADR-008 D4 — entrée canonique du lot de production (patch production_date + délégation à record_batch_production_v3).';

DROP FUNCTION IF EXISTS public.record_batch_production_v2(jsonb, jsonb);
DROP FUNCTION IF EXISTS public.record_batch_production_v1(jsonb, jsonb);

-- Impl interne : aucun accès client (le wrapper SECURITY DEFINER l'appelle).
REVOKE EXECUTE ON FUNCTION public.record_batch_production_v3(jsonb, jsonb) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.record_batch_production_v3(jsonb, jsonb) FROM anon;
REVOKE EXECUTE ON FUNCTION public.record_batch_production_v3(jsonb, jsonb) FROM authenticated;

REVOKE EXECUTE ON FUNCTION public.record_batch_production_v4(jsonb, jsonb) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.record_batch_production_v4(jsonb, jsonb) FROM anon;
GRANT  EXECUTE ON FUNCTION public.record_batch_production_v4(jsonb, jsonb) TO authenticated;
