-- 20260519000103_fix_record_batch_production_temptbl.sql
-- Session 15 / Phase 4.A — Fix : tolerate same-transaction reinvocation.
--
-- record_batch_production_v1 creates a TEMP TABLE `_batch_items` ON COMMIT
-- DROP. PostgREST executes each RPC call in its own transaction, so this is
-- a non-issue in production. However :
--   - The pgTAP acceptance suite (BEGIN .. ROLLBACK envelope) invokes the
--     function several times in one transaction.
--   - Future server-side callers (cron, EFs) may chain RPCs.
--
-- We add a defensive `DROP TABLE IF EXISTS pg_temp._batch_items` before the
-- CREATE — mirrors deviation pack D-S15-1A-TEMPTBL-01 already applied to
-- record_production_v1's _bom_flatten / _leaf_consumption temp tables in
-- migration 20260519000006.
--
...
BEGIN
  ...
  -- D-S15-1A-TEMPTBL-01 : tolerate same-tx reinvocation.
