-- 20260621000010_bump_complete_order_v11.sql
-- Session 37 / Wave A / Task A1 (SEC-01 + SEC-02 + SEC-05).
-- Bump v10 → v11 :
--   1. Discount gate server-side : tout discount (order-level OU line-level) exige
--      p_discount_authorized_by porteur de la permission `sales.discount` (D4 : perm
--      existante réutilisée, seedée S5 20260508000002) + PIN valide via verify_user_pin
--      (idiome close_fiscal_period_v1). PIN transporté par l'EF process-payment depuis
--      le header x-manager-pin → p_manager_pin (S25 : jamais en body JSON côté client).
--   2. Réconciliation unit_price : le prix attendu server-side est
--      get_customer_product_price(product_id, p_customer_id) — la même source que le
--      POS honnête (catégorie client retail/wholesale/discount_percentage/custom).
--      Lignes is_promo_gift exemptées (prix 0 validé par le bloc promotions).
--      Mismatch → le serveur force le prix attendu + audit `order.price_overridden`.
--   3. Audit `order.discount_applied` (cols canoniques actor_id/action/entity_type/
--      entity_id/metadata) quand un discount est appliqué.
--   4. Envelope : + `loyalty_balance_after` (POS-04, additive).
-- Versioning monotone : DROP v10 dans la même migration.

DO $drop$
DECLARE _r RECORD;
BEGIN
  FOR _r IN SELECT oid::regprocedure AS sig FROM pg_proc
    WHERE proname = 'complete_order_with_payment_v10' AND pronamespace = 'public'::regnamespace
  LOOP
    EXECUTE 'DROP FUNCTION ' || _r.sig::text || ' CASCADE';
  END LOOP;
END $drop$;

CREATE OR REPLACE FUNCTION public.complete_order_with_payment_v11(
  p_session_id uuid,
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- leur promotion_id figure dans p_promotions (validé par le bloc promotions
    -- plus bas) — sinon un client pourrait taguer n'importe quelle ligne en
    -- cadeau à prix 0.
  ...
  IF v_has_discount THEN
    ...
    END IF;
    IF p_manager_pin IS NULL OR NOT verify_user_pin(p_discount_authorized_by, p_manager_pin) THEN
...
COMMENT ON FUNCTION public.complete_order_with_payment_v11(
  uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, numeric, jsonb, jsonb, text
) IS
  'S37 bump v10 → v11 (SEC-01/02/05). Discount gate server-side : sales.discount + PIN '
  '(verify_user_pin) requis pour tout discount order/line-level. unit_price réconcilié contre '
  'get_customer_product_price (gift lines exemptées) + audit order.price_overridden. '
  'Audit order.discount_applied. Envelope + loyalty_balance_after (POS-04).';
