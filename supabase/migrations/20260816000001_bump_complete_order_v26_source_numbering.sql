-- 20260816000001_bump_complete_order_v26_source_numbering.sql
-- Numérotation des commandes par origine + date + séquence quotidienne partagée :
-- `P16082026001` = <code origine><DDMMYYYY><NNN>. La séquence reste order_sequences
-- (reset à minuit locale Asia/Makassar, fuseau de session DB — ADR-019) ; seul le
-- format du numéro change ('#0001' → 'P16082026001'). LPAD(3) déborde naturellement
-- à 4 chiffres au-delà de 999 commandes/jour.
--
-- v26 = v25 (corps live pg_get_functiondef, verbatim) + p_source_code text DEFAULT 'P'
-- validé `^(P|T[0-9]+|BO)$` (label d'origine, pas un montant — le client peut le
-- fournir) + nouvelle ligne de génération. L'index d'unicité par jour local
-- (orders_order_number_per_day_key) reste valide tel quel.

CREATE OR REPLACE FUNCTION public.complete_order_with_payment_v26(p_session_id uuid, p_order_type order_type, p_items jsonb, p_payment jsonb DEFAULT NULL::jsonb, p_idempotency_key uuid DEFAULT NULL::uuid, p_customer_id uuid DEFAULT NULL::uuid, p_loyalty_points_redeemed integer DEFAULT 0, p_table_number text DEFAULT NULL::text, p_discount_amount numeric DEFAULT 0, p_discount_type text DEFAULT NULL::text, p_discount_value numeric DEFAULT NULL::numeric, p_discount_reason text DEFAULT NULL::text, p_discount_authorized_by uuid DEFAULT NULL::uuid, p_promotions jsonb DEFAULT '[]'::jsonb, p_payments jsonb DEFAULT NULL::jsonb, p_discount_auth_id uuid DEFAULT NULL::uuid, p_source_code text DEFAULT 'P')
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Numérotation par origine : validé après le replay d'idempotence (un replay
  -- rend le résultat de la 1ère exécution, le code n'y sert plus).
  ...
  -- partagée entre toutes les portes (CURRENT_DATE = jour métier, fuseau de
  -- session Asia/Makassar — même bucket que le reset de order_sequences).
...
DROP FUNCTION IF EXISTS public.complete_order_with_payment_v25(uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, uuid);

REVOKE ALL ON FUNCTION public.complete_order_with_payment_v26(uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, uuid, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.complete_order_with_payment_v26(uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, uuid, text) FROM anon;
GRANT EXECUTE ON FUNCTION public.complete_order_with_payment_v26(uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, uuid, text) TO authenticated, service_role;
ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;

COMMENT ON FUNCTION public.complete_order_with_payment_v26(uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, uuid, text) IS
  'v26 = v25 + numérotation par origine : p_source_code (P | Tn | BO, défaut P) + format <code><DDMMYYYY><NNN> sur la séquence quotidienne partagée order_sequences. Aucun autre changement de comportement.';
