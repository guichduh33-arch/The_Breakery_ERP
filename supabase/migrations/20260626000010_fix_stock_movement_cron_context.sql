...
-- profil SYSTEM non-loggable (pin_hash non-bcrypt + is_active=false) et la
-- primitive l'utilise comme acteur quand (auth.uid() IS NULL AND
-- session_user = 'postgres') — PostgREST se connecte en 'authenticator',
-- donc anon/authenticated ne passent JAMAIS cette branche.
-- Signature INCHANGÉE → pas de bump, ACL conservées par CREATE OR REPLACE.

INSERT INTO public.user_profiles (id, employee_code, full_name, pin_hash, role_code, is_active)
VALUES ('00000000-0000-0000-0000-000000000999', 'SYS-CRON', 'System (cron)',
        'no-login', 'SUPER_ADMIN', false)   -- pin_hash non-bcrypt → aucun PIN ne matche
ON CONFLICT (id) DO NOTHING;

CREATE OR REPLACE FUNCTION public.record_stock_movement_v1(p_product_id uuid, p_movement_type movement_type, p_quantity numeric, p_reason text, p_unit_cost numeric DEFAULT NULL::numeric, p_supplier_id uuid DEFAULT NULL::uuid, p_idempotency_key uuid DEFAULT NULL::uuid, p_unit text DEFAULT NULL::text, p_from_section_id uuid DEFAULT NULL::uuid, p_to_section_id uuid DEFAULT NULL::uuid, p_metadata jsonb DEFAULT '{}'::jsonb, p_lot_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
