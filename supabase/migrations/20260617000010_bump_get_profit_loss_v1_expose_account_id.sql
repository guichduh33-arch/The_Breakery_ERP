...
-- Session 32 / Wave 1.B / migration _010 :
--   Additive bump: expose `account_id` (UUID) in each per-account line of the
--   `lines` JSONB array returned by get_profit_loss_v1.
--
-- Motivation :
--   BO drill-down links (DrilldownLink / buildDrilldownUrl, S31) navigate to the
--   General Ledger by UUID. The CTE `agg` already SELECTs a.id AS account_id
--   but it was not surfaced in the jsonb_build_object for each line.
--
-- Change (ONLY):
--   In the jsonb_build_object inside jsonb_agg, add 'account_id', account_id
--   as the FIRST key (before 'code').
--
-- All other logic is IDENTICAL to _017 (S26 dedupe-void NOT EXISTS fix is preserved).
--
-- Signature is UNCHANGED (same input args + RETURNS JSONB) → CREATE OR REPLACE,
-- no v2 bump required.
...
COMMENT ON FUNCTION public.get_profit_loss_v1(DATE, DATE, UUID) IS
  'S32 wave 1.B additive: each line in the `lines` array now includes '
  '`account_id` (UUID) as the first key, enabling BO drill-down to General '
  'Ledger by UUID. All S26 _017 logic preserved (dedupe sale_void NOT EXISTS '
  'when refund exists for same order, F-S26-AC-04).';
