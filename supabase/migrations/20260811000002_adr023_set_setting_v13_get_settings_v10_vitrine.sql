-- ADR-023 — l'écran de curation de la vitrine entre au back-office.
--
-- Les deux RPC de réglages sont des aiguillages `CASE ... WHEN '<clé>'` : y
-- ajouter un réglage impose de bumper les deux. Corps repris du corps LIVE
-- (pg_get_functiondef) par substitutions comptées ; v12 et v9 sont droppées
-- dans la même migration.
--
-- Ce que v13 ajoute :
--   display_showcase_product_ids  la sélection ordonnée, bornée à 12, dont
--                                 chaque id doit désigner un produit existant.
--                                 Aucun prix n'y entre (ADR-023 déc. 2).
--   display_show_ready_orders     l'interrupteur de la file de retrait.
--
-- Ce que v10 ajoute : les deux mêmes clés à la catégorie `customer_display`.
--
-- Les colonnes, elles, sont posées par 20260811000001 (lot du rendu).

CREATE OR REPLACE FUNCTION public.set_setting_v13(p_key text, p_value jsonb, p_category text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  CASE p_key
    ...
    -- ADR-019 (D3) : le fuseau métier est une constante de déploiement, pas un
    -- réglage. Il n'est plus écrivable à chaud — le changer est un geste de
    -- déploiement (migration du paramètre de session + colonne miroir + la
    -- constante client, dans le même lot). La colonne reste LISIBLE (D2).
    WHEN 'timezone' THEN
      RAISE EXCEPTION 'setting_not_configurable' USING ERRCODE = '22023',
        DETAIL = 'timezone is a deployment constant (ADR-019): change it by migration, not at runtime';
    ...
    WHEN 'enabled_payment_methods' THEN
      ...
      FOR v_elem IN SELECT * FROM jsonb_array_elements(p_value) LOOP
        IF jsonb_typeof(v_elem) <> 'string'
           OR (v_elem #>> '{}') NOT IN ('cash','card','qris','edc','transfer','store_credit','gopay','ovo','dana') THEN
          RAISE EXCEPTION 'setting_value_invalid' USING ERRCODE = '22023',
    ...
    -- ADR-023 (déc. 2) : la sélection de vitrine de l'écran client au repos.
    -- Une liste ORDONNÉE d'ids produit, et rien d'autre — jamais un prix, qui
    -- est résolu depuis le catalogue à l'affichage. Un id qui ne désigne aucun
    -- produit est refusé ici : il ne se verrait nulle part, la vitrine se
    -- contentant de le taire.
    WHEN 'display_showcase_product_ids' THEN
      IF jsonb_typeof(p_value) <> 'array' THEN
        RAISE EXCEPTION 'setting_type_invalid' USING ERRCODE = '22023', DETAIL = 'display_showcase_product_ids expects array';
      END IF;
      IF jsonb_array_length(p_value) > 12 THEN
        RAISE EXCEPTION 'setting_value_invalid' USING ERRCODE = '22023', DETAIL = 'display_showcase_product_ids max 12 products';
      END IF;
      IF EXISTS (SELECT 1 FROM jsonb_array_elements(p_value) AS e WHERE jsonb_typeof(e) <> 'string') THEN
        RAISE EXCEPTION 'setting_value_invalid' USING ERRCODE = '22023', DETAIL = 'display_showcase_product_ids expects an array of product ids';
      END IF;
      IF EXISTS (
        SELECT 1 FROM jsonb_array_elements_text(p_value) AS t(pid)
         WHERE NOT EXISTS (
           SELECT 1 FROM products pr WHERE pr.id::TEXT = t.pid AND pr.deleted_at IS NULL
         )
      ) THEN
        RAISE EXCEPTION 'setting_value_invalid' USING ERRCODE = '22023', DETAIL = 'display_showcase_product_ids references an unknown product';
      END IF;
      SELECT to_jsonb(display_showcase_product_ids) INTO v_old FROM business_config WHERE id = 1;
      UPDATE business_config SET display_showcase_product_ids = p_value, updated_at = now() WHERE id = 1;
      v_new := p_value;

    -- ADR-023 (déc. 3) : l'interrupteur de la file de retrait. Éteint, la
    -- vitrine occupe seule le repos ; allumé, l'écran retrouve son
    -- comportement d'avant. C'est un geste d'exploitation, pas un ADR.
    WHEN 'display_show_ready_orders' THEN
      IF jsonb_typeof(p_value) <> 'boolean' THEN
        RAISE EXCEPTION 'setting_type_invalid' USING ERRCODE = '22023', DETAIL = 'display_show_ready_orders expects boolean';
      END IF;
      SELECT to_jsonb(display_show_ready_orders) INTO v_old FROM business_config WHERE id = 1;
      UPDATE business_config SET display_show_ready_orders = (p_value #>> '{}')::BOOLEAN, updated_at = now() WHERE id = 1;
      v_new := p_value;
    ...
    WHEN 'offline_payments_enabled' THEN
      IF jsonb_typeof(p_value) <> 'boolean' THEN
        RAISE EXCEPTION 'setting_type_invalid' USING ERRCODE = '22023', DETAIL = 'offline_payments_enabled expects boolean';
      END IF;
      SELECT to_jsonb(offline_payments_enabled) INTO v_old FROM business_config WHERE id = 1;
      UPDATE business_config SET offline_payments_enabled = (p_value #>> '{}')::BOOLEAN, updated_at = now() WHERE id = 1;
      v_new := p_value;
...
DROP FUNCTION IF EXISTS public.set_setting_v12(text, jsonb, text);

CREATE OR REPLACE FUNCTION public.get_settings_by_category_v10(p_category text)
 RETURNS jsonb
 LANGUAGE plpgsql
  ...
  v_settings :=
    CASE LOWER(COALESCE(p_category, ''))
      ...
      WHEN 'customer_display' THEN jsonb_build_object(
        ...
        'display_showcase_product_ids',  v_row.display_showcase_product_ids,
        'display_show_ready_orders',     v_row.display_show_ready_orders
      )
...
DROP FUNCTION IF EXISTS public.get_settings_by_category_v9(text);

-- Grants : miroir de l'ACL live des versions remplacées
-- (postgres/authenticated/service_role). anon hérite EXECUTE via PUBLIC — le
-- REVOKE sur anon seul serait insuffisant.
REVOKE EXECUTE ON FUNCTION public.set_setting_v13(text, jsonb, text) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.set_setting_v13(text, jsonb, text) FROM anon;
REVOKE EXECUTE ON FUNCTION public.get_settings_by_category_v10(text) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.get_settings_by_category_v10(text) FROM anon;
ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.set_setting_v13(text, jsonb, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.set_setting_v13(text, jsonb, text) TO service_role;
GRANT EXECUTE ON FUNCTION public.get_settings_by_category_v10(text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_settings_by_category_v10(text) TO service_role;

COMMENT ON FUNCTION public.set_setting_v13(text, jsonb, text) IS
  'Écriture des réglages business_config (validation par clé, audit-log old/new). v13 (ADR-023) : ajoute la sélection de vitrine de l''écran client — liste ordonnée d''ids produit existants, 12 au plus, jamais de prix — et l''interrupteur de la file de retrait.';
COMMENT ON FUNCTION public.get_settings_by_category_v10(text) IS
  'Lecture d''une partition de business_config par catégorie symbolique. v10 (ADR-023) : la catégorie customer_display rend aussi la sélection de vitrine et l''interrupteur de la file de retrait.';
