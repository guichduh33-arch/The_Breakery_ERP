-- 20260901000007_pay_existing_order_v19_exclude_cancelled.sql
--
...
--
-- `pay_existing_order_v18` calculait le total facturé (`v_items_total`,
-- SUM(line_total)) ET parcourait la boucle de déstockage SANS filtrer
-- `is_cancelled = false`. Une ligne annulée par `cancel_order_item_rpc_v6`
-- (ADR-010) restait donc facturée et déduite du stock au paiement d'une
-- commande déjà envoyée en cuisine — alors que cette même RPC d'annulation
-- recalcule déjà `orders.subtotal/tax/total` avec `is_cancelled = false`,
-- valeur que v18 écrasait ensuite (`subtotal = v_items_total`).
--
-- Preuve ROUGE enregistrée sur le corps live (2026-09-05) : fire A 5000 +
-- B 3000, annulation de B, paiement 5000 → « Sum of tender amounts (5000.00)
-- != order total (8000.00) », SQLSTATE 23514, stock de A non déduit. Le
-- caissier ne peut encaisser que le total gonflé, et ce total déstocke la
-- ligne annulée.
--
-- Décisions de Mamat (2026-09-05) :
--   1. Exclusion par PRÉDICAT (`is_cancelled = false`), jamais par mise à zéro
--      de la ligne : la ligne annulée reste une trace (ADR-010), elle cesse
--      seulement de compter.
--   2. `is_promo_gift` n'est PAS filtré dans la boucle de déstockage : un
--      cadeau promotionnel consomme du stock. Les deux requêtes d'évaluation
--      promo (`v_eval_subtotal`, `v_eval_items`) portaient déjà les deux
--      filtres et sont inchangées.
--   3. `orders.subtotal = v_items_total` (UPDATE final) reçoit désormais la
--      somme filtrée, cohérente avec le recalcul de `cancel_order_item_rpc_v6`.
--
-- Changements par rapport au corps live de v18, et rien d'autre :
--   a. `v_items_total` : `WHERE order_id = p_order_id AND is_cancelled = false` ;
--   b. boucle `FOR v_item IN SELECT … FROM order_items oi JOIN products p …` :
--      `AND oi.is_cancelled = false` ;
--   c. les deux `rpc_version` des audit_logs : 'v19-exclude-cancelled'.
--
-- Versioning monotone : v19 créée, v18 droppée dans cette migration, signature
-- inchangée. Appelants : `useCheckout` (branche commande déjà persistée),
-- `offlineReplay` (POS) — repointés dans le lot client qui suit.
--
-- PROVENANCE DU CORPS : `pg_get_functiondef` sur la base live, relevé le
-- 2026-09-05 (md5 sans espaces 84ea0348375e4ed49a13b70d07c46985). Le garde
...
BEGIN
  ...
  LOOP
    IF v_item.product_type = 'combo' THEN
      ...
      END LOOP;
    ELSE
      -- Bug 2026-08-25 : miroir de complete_order_with_payment. Un produit
      -- suivi (ou en vitrine) vend son propre stock ; un fini non suivi avec
      -- deduct_stock consomme sa recette (_resolve_recipe_consumption_v1,
      -- descente ADR-016 : stop au premier intermediaire stocke) ; sans
      -- deduct_stock, aucune deduction.
      ...
      ELSIF v_item.deduct_stock THEN
        FOR v_cons IN SELECT * FROM _resolve_recipe_consumption_v1(v_item.product_id, v_item.quantity) LOOP
          PERFORM _record_sale_stock_v1(
            ...
            p_reference_id   := p_order_id,
            p_created_by     := v_profile_id,
            p_reason         := 'POS recipe consumption (pay existing)',
            p_unit           := v_cons.unit,
            p_allow_negative := v_allow_negative
          );
        END LOOP;
  ...
  IF p_discount_amount > 0 THEN
    INSERT INTO audit_logs (actor_id, action, entity_type, entity_id, metadata)
      VALUES (v_profile_id, 'order.discount_applied', 'orders', p_order_id, jsonb_build_object(
        ...
        'rpc_version',           'v19-exclude-cancelled'
      ));
  END IF;
...
REVOKE ALL ON FUNCTION public.pay_existing_order_v19(uuid, jsonb, uuid, integer, uuid, numeric, text, numeric, text, uuid, uuid, jsonb, jsonb, boolean) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.pay_existing_order_v19(uuid, jsonb, uuid, integer, uuid, numeric, text, numeric, text, uuid, uuid, jsonb, jsonb, boolean) FROM anon;
GRANT EXECUTE ON FUNCTION public.pay_existing_order_v19(uuid, jsonb, uuid, integer, uuid, numeric, text, numeric, text, uuid, uuid, jsonb, jsonb, boolean) TO authenticated;
GRANT EXECUTE ON FUNCTION public.pay_existing_order_v19(uuid, jsonb, uuid, integer, uuid, numeric, text, numeric, text, uuid, uuid, jsonb, jsonb, boolean) TO service_role;

COMMENT ON FUNCTION public.pay_existing_order_v19(uuid, jsonb, uuid, integer, uuid, numeric, text, numeric, text, uuid, uuid, jsonb, jsonb, boolean) IS
  'Pays an order already persisted server-side (fired counter order, reopened held order, picked-up tablet order): validates tenders (1..5, multi-tender), promotions (server re-evaluated, caps under advisory lock), loyalty redemption, manager-nonce discount, store credit, deducts sale stock through _record_sale_stock_v1 (own stock, recipe consumption or combo components) and posts payments + loyalty. v19 (2026-09-05, audit lot 1 P0 n°5): cancelled lines (is_cancelled) are excluded from the billed total and from stock deduction; is_promo_gift lines still consume stock. Gate payments.process (+ sales.discount on the authorizer). Idempotent via p_idempotency_key (orders.idempotency_key, replay returns the first result). Errors: P0001/P0002/P0003/P0010/P0014/P0015/P0016/23514.';

-- Versioning monotone : la v18 tombe dans la même migration.
DROP FUNCTION IF EXISTS public.pay_existing_order_v18(uuid, jsonb, uuid, integer, uuid, numeric, text, numeric, text, uuid, uuid, jsonb, jsonb, boolean);
