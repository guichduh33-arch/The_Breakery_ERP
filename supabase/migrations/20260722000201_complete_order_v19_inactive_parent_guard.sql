-- 20260722000201 - complete_order_with_payment v19 : garde vendabilite (ADR-011 dec. 2, etend ADR-007 dec. 2).
--
-- v19 = corps live v18 (pg_get_functiondef du 2026-07-22, jamais le fichier
-- de migration d'origine) + :
--   1. refus strict d'un produit is_active=false ("product_inactive: ...") ;
--   2. refus strict d'un produit-PARENT (a des variantes actives,
--      "product_is_parent: ...") - architecture linked-products S27c ;
--   3. un produit soft-deleted est traite comme introuvable (v18 le vendait) ;
--   4. composants de combo inactifs refuses ;
--   5. labels d'audit rpc_version corriges ('v17' obsolete -> 'v19').
-- Le stock epuise reste tolere serveur (vente offline / replay, ADR-011).
-- visible_on_pos reste un filtre d'affichage (masque != invendable, ADR-007).
--
-- Consommateur unique : EF process-payment (redeployee dans le meme chantier).
-- fire_counter_order_v4 / create_tablet_order_v4 / pay_existing_order_v13 sont
-- HORS perimetre (chemin replay offline - un garde y bloquerait le rattrapage).
CREATE OR REPLACE FUNCTION public.complete_order_with_payment_v19(p_session_id uuid, p_order_type order_type, p_items jsonb, p_payment jsonb DEFAULT NULL::jsonb, p_idempotency_key uuid DEFAULT NULL::uuid, p_customer_id uuid DEFAULT NULL::uuid, p_loyalty_points_redeemed integer DEFAULT 0, p_table_number text DEFAULT NULL::text, p_discount_amount numeric DEFAULT 0, p_discount_type text DEFAULT NULL::text, p_discount_value numeric DEFAULT NULL::numeric, p_discount_reason text DEFAULT NULL::text, p_discount_authorized_by uuid DEFAULT NULL::uuid, p_promotions jsonb DEFAULT '[]'::jsonb, p_payments jsonb DEFAULT NULL::jsonb, p_discount_auth_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DROP FUNCTION public.complete_order_with_payment_v18(uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, uuid);

REVOKE EXECUTE ON FUNCTION public.complete_order_with_payment_v19(uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, uuid) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.complete_order_with_payment_v19(uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, uuid) FROM anon;
ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.complete_order_with_payment_v19(uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, uuid) TO authenticated, service_role;

COMMENT ON FUNCTION public.complete_order_with_payment_v19(uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, uuid) IS
  'Money-path POS (EF process-payment). v19 : refus strict produits inactifs/parents/soft-deleted au paiement (ADR-011 dec. 2).';