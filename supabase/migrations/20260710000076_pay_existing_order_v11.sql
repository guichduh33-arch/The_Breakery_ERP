-- 20260710000076_pay_existing_order_v11.sql
-- S53 P1.4 — pay_existing_order v10 -> v11. Makes deduction flag-aware by routing all
-- three stock-deduction blocks (combo components, simple line, modifier ingredients) through
-- the internal helper _record_sale_stock_v1, which respects business_config.allow_negative_stock.
-- Removes now-dead DECLAREs: v_comp_product, v_ing_stock, v_ing_is_display, v_ing_track.
-- DROP v10 + REVOKE/GRANT.

DROP FUNCTION IF EXISTS public.pay_existing_order_v10(
  uuid, jsonb, uuid, integer, uuid, numeric, text, numeric, text, uuid, jsonb, jsonb
);

CREATE FUNCTION public.pay_existing_order_v11(
  p_order_id uuid,
