...
-- Body copied from LIVE v4 (pg_get_functiondef, DEV-S57-02); the ONLY changes are:
--   1. the two `v_unit_price := (v_item->>'unit_price')::numeric;` assignments now
--      resolve the price server-side via _resolve_b2b_line_price_v1 (client value ignored);
--   2. the price guard becomes `price_unresolved` (P0002) — a resolved price is never
--      negative (both price tables CHECK >= 0), only NULL if the product has no price at all;
--   3. audit metadata rpc_version 'v4-s68' -> 'v5-s69'.
-- Everything else (TOCTOU credit re-check, JE, display/recipe-aware stock via
-- _record_sale_stock_v1, invoice_number S68, idempotency) is byte-identical. DROP v4 same migration.
...
CREATE FUNCTION create_b2b_order_v5(
  p_customer_id uuid, p_items jsonb, p_notes text DEFAULT NULL::text,
  p_delivery_date date DEFAULT NULL::date, p_idempotency_key uuid DEFAULT NULL::uuid
) RETURNS jsonb
