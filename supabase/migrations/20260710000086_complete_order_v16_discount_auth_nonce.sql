-- 20260710000086_complete_order_v16_discount_auth_nonce.sql
-- S55 P1.5 (audit T7) Task 6 : complete_order_with_payment_v16 — discount-PIN
-- authorization moves out of SQL args entirely. The POS/EF verify the manager
-- PIN out-of-band (process-payment EF, service_role) and mint a single-use
-- nonce in discount_authorizations (see _085). This RPC consumes that nonce
-- atomically in place of the former p_manager_pin + _verify_pin_with_lockout
-- check. Behavior-identical otherwise — signature drops p_manager_pin, adds
-- p_discount_auth_id as the last (16th) arg.
-- Body copied verbatim from v15 (_074) with exactly 4 hunks:
--   1. Signature: p_manager_pin text -> p_discount_auth_id uuid (last arg).
--   2. Discount gate: PIN check replaced by atomic nonce-consumption UPDATE.
--   3. After order INSERT: trace consumed_order_id on the nonce.
--   4. Discount audit metadata: rpc_version 'v15' -> 'v16'.

CREATE OR REPLACE FUNCTION public.complete_order_with_payment_v16(
  p_session_id uuid,
