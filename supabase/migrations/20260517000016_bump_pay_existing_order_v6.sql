-- 20260517000016_bump_pay_existing_order_v6.sql
-- Session 13 / Phase 1.A / migration 03-001 :
--   Bump pay_existing_order → pay_existing_order_v6.
--   Same v5 body ; loyalty JE append now routes via mapping ; ensures unit
--   is populated on stock_movements (was missing from v5).
--
-- Decision D14 (RPC versioning) — drop v5 in same migration.
...
CREATE OR REPLACE FUNCTION pay_existing_order_v6(
  p_order_id                UUID,
...
BEGIN
  ...
  -- Loyalty JE append : v6 routes via mapping.
...
GRANT EXECUTE ON FUNCTION pay_existing_order_v6 TO authenticated;

COMMENT ON FUNCTION pay_existing_order_v6 IS
  'D14 bump v5 → v6. Loyalty JE append via accounting_mappings ; stock_movements.unit '
  'now explicitly populated. Same wire signature as v5 (mutually-exclusive p_payment / p_payments).';
