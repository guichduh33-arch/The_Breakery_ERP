-- [types-noop] Même signature et mêmes ACL ; corps repris de pg_get_functiondef.
-- Le plafond protège la valorisation automatique, pas l'enregistrement de la recette.
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.recipes WHERE product_id = p_product_id AND is_active AND deleted_at IS NULL) THEN
    IF v_cost IS NULL OR v_cost < 0 OR v_cost > 5000000 OR v_cost = 'NaN'::numeric THEN
      -- Comme le recalcul historique : conserver la valorisation, sans bloquer l'import.
      INSERT INTO public.audit_logs (actor_id, action, entity_type, entity_id, metadata)
        VALUES (p_profile, 'product.cost_recompute_skipped', 'product', p_product_id,
          jsonb_build_object('old_cost', v_old, 'computed_cost', v_cost,
            'reason', 'implausible_cost', 'recipe_version_id', v_version));
      RETURN v_version;
    END IF;
