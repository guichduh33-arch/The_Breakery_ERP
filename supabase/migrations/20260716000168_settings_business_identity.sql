-- 20260716000168_settings_business_identity.sql
-- Settings backlog §6.A (audit 2026-07-16) — company identity on documents.
--
-- Adds the four missing org-identity/ops keys on the `business_config` singleton:
--   npwp        — the bakery's own tax ID (rendered on receipts + B2B invoice PDFs;
--                 generate-pdf previously selected a phantom `npwp` column and always
--                 fell back to hardcoded defaults).
--   phone       — printed on the POS receipt header (was hardcoded in SuccessModal).
--   logo_url    — public URL of the brand logo (bucket `branding` below); embedded
--                 in PDF headers and email templates.
--   alert_email — internal recipient for operational notifications
--                 (low_stock_alert / po_received / expense_approved — wired by the
--                 notification-triggers lot).
--
-- RPC bump (versioning monotone, bodies copied from live pg_get_functiondef):
--   get_settings_by_category_v1 -> v2   (business branch exposes the new keys)
--   set_setting_v1              -> v2   (validation branches for the new keys)
-- Old versions DROPped in this same migration; call-sites repointed in the same
-- commit (hard cutover).
--
-- Also creates the PUBLIC storage bucket `branding` (logo upload from
-- /settings/general). PNG/JPEG only: pdf-lib can only embed those two formats.

-- =============================================================================
-- 1. Columns on the business_config singleton
-- =============================================================================
...
COMMENT ON COLUMN business_config.npwp        IS 'Bakery''s own NPWP (tax ID) — rendered on receipts and B2B invoice PDFs. NULL = not shown.';
COMMENT ON COLUMN business_config.phone       IS 'Business phone printed on the POS receipt header. NULL = not shown.';
COMMENT ON COLUMN business_config.logo_url    IS 'Public URL of the brand logo (bucket `branding`). PNG/JPEG. NULL = text-only headers.';
COMMENT ON COLUMN business_config.alert_email IS 'Internal recipient of operational alert notifications. NULL = internal alerts disabled.';

-- =============================================================================
-- 2. get_settings_by_category_v2 (body from live v1; business branch extended)
-- =============================================================================

CREATE FUNCTION public.get_settings_by_category_v2(p_category text)
 RETURNS jsonb
 LANGUAGE plpgsql
...
DROP FUNCTION public.get_settings_by_category_v1(text);

-- =============================================================================
-- 3. set_setting_v2 (body from live v1; four new validation branches)
-- =============================================================================

CREATE FUNCTION public.set_setting_v2(p_key text, p_value jsonb, p_category text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
DROP FUNCTION public.set_setting_v1(text, jsonb, text);

-- =============================================================================
-- 4. Grants — REVOKE pair (anon defense-in-depth) + mirror of v1's ACL
-- =============================================================================

REVOKE EXECUTE ON FUNCTION public.get_settings_by_category_v2(text) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.get_settings_by_category_v2(text) FROM anon;
GRANT  EXECUTE ON FUNCTION public.get_settings_by_category_v2(text) TO authenticated, service_role;

REVOKE EXECUTE ON FUNCTION public.set_setting_v2(text, jsonb, text) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.set_setting_v2(text, jsonb, text) FROM anon;
GRANT  EXECUTE ON FUNCTION public.set_setting_v2(text, jsonb, text) TO authenticated, service_role;

ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;

-- =============================================================================
-- 5. Storage bucket `branding` (public read; writes gated settings.update)
--    PNG/JPEG only — pdf-lib can only embed those two formats in PDF headers.
-- =============================================================================
...
VALUES (
  'branding',
  'branding',
  true,                                  -- public read (logo is printed on receipts anyway)
  1 * 1024 * 1024,                       -- 1 MB max
...
CREATE POLICY branding_insert ON storage.objects
  ...
  WITH CHECK (
    bucket_id = 'branding'
    AND has_permission(auth.uid(), 'settings.update')
  );
