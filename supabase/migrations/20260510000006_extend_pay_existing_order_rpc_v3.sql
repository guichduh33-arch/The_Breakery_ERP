...
BEGIN
  ...
  IF v_order.status <> 'pending_payment' THEN
    RAISE EXCEPTION 'Order is not in pending_payment status (current: %)', v_order.status
  ...
  -- Lire la promo frozen (set by create_tablet_order)
...
COMMENT ON FUNCTION pay_existing_order IS
  'Session 8 v3: P10 freeze — reads frozen promotion_total_amount from orders (set at create_tablet_order time). No re-evaluation of promotions. Math: total = items_total - promo - redemption - discount. JE via trigger on UPDATE status to paid.';
