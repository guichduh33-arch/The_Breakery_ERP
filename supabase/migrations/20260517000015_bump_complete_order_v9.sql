-- 20260517000015_bump_complete_order_v9.sql
-- Session 13 / Phase 1.A / migration 02-001 :
--   Bump complete_order_with_payment → complete_order_with_payment_v9.
--   Same body as v8 ; the sale JE is now emitted by the refactored trigger
--   (migration 000010) via accounting_mappings — no body change needed here.
--
-- Decision D14 (monotonic RPC versioning) — drop v8 in same migration.
-- v9 keeps the v8 signature verbatim (split-tender, promotions, discount, loyalty)
-- so the existing wire-protocol in apps/pos/src/features/payment/hooks/useCheckout.ts
-- continues to work after a function-name change.
...
CREATE OR REPLACE FUNCTION complete_order_with_payment_v9(
  p_session_id              UUID,
...
BEGIN
  ...
  -- Loyalty JE append : v9 routes via mapping (LOYALTY_LIABILITY + SALE_DISCOUNT).
...
GRANT EXECUTE ON FUNCTION complete_order_with_payment_v9 TO authenticated;

COMMENT ON FUNCTION complete_order_with_payment_v9 IS
  'D14 bump v8 → v9. Same args/body ; the sale JE now routes through accounting_mappings '
  'via the refactored trigger (migration 20260517000010). Loyalty JE lines also via mapping. '
  'Stock_movements explicitly populates unit (NOT NULL after Session 12 hotfix).';
