-- 20260712000163_settings_kds_thresholds.sql
-- S75 Lot 2 (Task 5) — business_config KDS threshold columns + settings RPC
-- category 'kds' (get_settings_by_category_v1 / set_setting_v1). Additive
-- CREATE OR REPLACE regrafted from the LIVE function bodies (DEV-S57-02) —
-- every pre-existing category/key branch is preserved byte-for-byte, only the
-- 'kds' category branch and 3 new p_key branches are new. No BEGIN/COMMIT
-- (apply_migration already wraps in a transaction).

-- 1. New business_config columns.
ALTER TABLE business_config
  ADD COLUMN kds_warning_threshold_minutes INT NOT NULL DEFAULT 5,
  ADD COLUMN kds_urgent_threshold_minutes  INT NOT NULL DEFAULT 10,
  ADD COLUMN kds_auto_archive_minutes      INT NOT NULL DEFAULT 5;

-- 2. get_settings_by_category_v1 — regrafted live body + 'kds' branch.
...
-- 3. set_setting_v1 — regrafted live body + 3 new p_key branches.
