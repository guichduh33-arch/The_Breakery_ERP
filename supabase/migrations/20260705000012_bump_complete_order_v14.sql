-- 20260705000012_bump_complete_order_v14.sql
-- Phase 2 — complete_order_with_payment v13 -> v14.
-- Adds: modifier-ingredient availability check (validation), resolved+converted
-- snapshot persisted into order_items.modifier_ingredients_deducted, and
-- display-aware ingredient stock deduction. Mirrors the combo component loop.
-- DROP v13 + CREATE v14 in one migration (monotonic versioning).

DROP FUNCTION IF EXISTS public.complete_order_with_payment_v13(
  uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, text
);

CREATE FUNCTION public.complete_order_with_payment_v14(
  p_session_id uuid,
