-- ADR-013 D14 (M1) -- Re-application forward-only du corps single-statement de
-- get_orders_list_v2.
--
-- Historique : le fix CTE-scope `20260529200749` (projection unique
-- `SELECT ... INTO v_lines, v_next FROM filtered f`) porte un numero INFERIEUR
-- au bump `20260618000011` (server filters) qui a reintroduit le bug 42P01
-- (deux statements, le CTE `filtered` mort au second). Le cloud LIVE est sain
-- (ce corps EST le corps live, dump pg_get_functiondef 2026-07-26) ; c'est le
-- REJEU chronologique (rebuild a blanc) qui aboutissait a l'etat casse.
-- Cette migration re-applique le corps correct au numero le plus haut :
-- no-op idempotent sur le live, etat terminal correct en rejeu.
-- Runbook DR "Recovery" (forward-only fix migration), regle projet : jamais
-- editer une migration publiee.
CREATE OR REPLACE FUNCTION public.get_orders_list_v2(p_start text, p_end text, p_filters jsonb DEFAULT '{}'::jsonb, p_limit integer DEFAULT 50, p_cursor timestamp with time zone DEFAULT NULL::timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- ACL defense-en-profondeur (re-assertion : le rejeu doit etre auto-suffisant).
REVOKE ALL ON FUNCTION public.get_orders_list_v2(p_start text, p_end text, p_filters jsonb, p_limit integer, p_cursor timestamp with time zone) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_orders_list_v2(p_start text, p_end text, p_filters jsonb, p_limit integer, p_cursor timestamp with time zone) FROM anon;
GRANT EXECUTE ON FUNCTION public.get_orders_list_v2(p_start text, p_end text, p_filters jsonb, p_limit integer, p_cursor timestamp with time zone) TO authenticated;

COMMENT ON FUNCTION public.get_orders_list_v2(p_start text, p_end text, p_filters jsonb, p_limit integer, p_cursor timestamp with time zone) IS
  'BO orders list v2 : filtres serveur JSONB + cursor. S33 Wave 4.1 corrective (CTE scope, lines+next_cursor en un seul SELECT INTO) re-appliquee forward-only par ADR-013 D14.';
