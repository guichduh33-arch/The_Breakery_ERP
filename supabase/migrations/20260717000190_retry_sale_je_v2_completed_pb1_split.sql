-- 20260717000190_retry_sale_je_v2_completed_pb1_split.sql
-- ADR-009 (résiduel déc. 4) — retry_sale_journal_entry_v2.
-- v1 avait deux dettes :
--   1. garde `status = 'paid'` strict → une commande passée completed (triggers
--      _189) avec JE vente manquante devenait irrécupérable ;
--   2. corps pré-PB1-split : TVA hardcodée total*10/110, pas de split par
--      méthode de paiement, pas de garde zéro-taxe, pas de skip historical
--      import — le retry recréait une JE d'une forme différente de celle que
--      le trigger live aurait produite.
-- v2 réplique la branche 'paid' du trigger LIVE create_sale_journal_entry
-- (pg_get_functiondef 2026-07-17) : tax = orders.tax_amount, split
-- order_payments par mapping key, fallback cash + audit_logs, ligne PB1
-- seulement si v_vat > 0, skip is_historical_import.
-- Garde élargie : status IN ('paid', 'completed').

CREATE OR REPLACE FUNCTION public.retry_sale_journal_entry_v2(p_order_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
REVOKE EXECUTE ON FUNCTION public.retry_sale_journal_entry_v2(uuid) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.retry_sale_journal_entry_v2(uuid) FROM anon;
ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.retry_sale_journal_entry_v2(uuid) TO authenticated;

COMMENT ON FUNCTION public.retry_sale_journal_entry_v2(uuid) IS
  'Recrée la JE vente manquante d''une commande paid|completed (parité trigger create_sale_journal_entry : tax_amount, split paiements, fallback cash). Idempotent : renvoie la JE existante sans doublon. Gate pos.sale.create.';

DROP FUNCTION public.retry_sale_journal_entry_v1(uuid);
