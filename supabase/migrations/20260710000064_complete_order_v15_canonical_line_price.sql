-- 20260710000064_complete_order_v15_canonical_line_price.sql
-- Bump v14 -> v15 : resolution canonique du prix de ligne cote serveur.
-- Signature IDENTIQUE a v14 (16 args, meme ordre/types) - blast-radius reduit.
--
-- Changements vs v14 (20260710000023) :
--   A. Boucle 1 (validation) : appel _resolve_line_price_v1 par ligne pour recuperer
--      unit_price, modifiers_total, line_subtotal, modifiers_resolved cote serveur.
--      price_adjustment client ignore (G1/G2). Override price-base : log audit conserve
--      si le client diverge (D4 = override silencieux + audit).
--   B. Boucle 2 (persist) : meme appel _resolve_line_price_v1 ; snapshot order_items.modifiers
--      utilise modifiers_resolved (price serveur, pas client).
--   C. Validation cadeau D2 (G3) : apres evaluate_promotions_v1, chaque ligne is_promo_gift
--      est validee contre applied_promotions[].free_items du serveur (product_id + quantity).
--      Ligne non autorisee -> check_violation.
--   D. Retour enrichi (additif) : nouvelle cle `lines` = ventilation par ligne (valeurs serveur).
--   E. DROP complete_order_with_payment_v14 dans la meme migration.
--   F. rpc_version -> 'v15' dans les 3 entrees audit_log.

CREATE OR REPLACE FUNCTION public.complete_order_with_payment_v15(
  p_session_id uuid,
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    IF v_item_product_type = 'combo' THEN
      FOR v_comp IN SELECT * FROM jsonb_array_elements(COALESCE(v_item->'combo_components', '[]'::jsonb)) LOOP
        ...
        SELECT
          p.id, 'sale', -v_comp_qty, COALESCE(p.unit, 'pcs'),
          'orders', v_order_id, v_profile_id
        FROM products p WHERE p.id = (v_comp->>'product_id')::UUID;

        UPDATE products
          SET current_stock = current_stock - v_comp_qty,
              updated_at = now()
        ...
        IF (SELECT is_display_item FROM products WHERE id = (v_comp->>'product_id')::UUID) THEN
          INSERT INTO display_movements (
            ...
            (v_comp->>'product_id')::UUID, 'sale', -v_comp_qty, 'POS combo sale', 'order', v_order_id, v_profile_id
          );
          UPDATE display_stock
            SET quantity = quantity - v_comp_qty,
                updated_at = now()
            WHERE product_id = (v_comp->>'product_id')::UUID;
        END IF;
      END LOOP;
    ELSE
      ...
      IF v_line_track THEN
        -- Fini suivi : decrement de son propre stock (1x), pas de cascade.
        INSERT INTO stock_movements (
          product_id, movement_type, quantity, unit, reference_type, reference_id, created_by
        )
        SELECT
      ...
      ELSIF v_line_deduct THEN
        -- Fait-a-la-commande : cascade recette, arret aux noeuds suivis.
        FOR v_cons IN SELECT * FROM _resolve_recipe_consumption_v1(v_product_id, v_quantity) LOOP
          ...
          IF v_cons_is_display THEN
            INSERT INTO display_movements (
              ...
              v_cons.product_id, 'sale', -v_cons.qty_base, 'POS recipe consumption',
              'order', v_order_id, v_profile_id
            );
            UPDATE display_stock
              SET quantity = quantity - v_cons.qty_base,
                  updated_at = now()
    ...
    IF v_mod_ingredients IS NOT NULL THEN
      ...
      LOOP
        ...
        UPDATE products
          SET current_stock = current_stock - v_ing.qty_base,
              updated_at = now()
        ...
        IF (SELECT is_display_item FROM products WHERE id = v_ing.product_id) THEN
          INSERT INTO display_movements (
            ...
            'POS modifier: ' || v_ing.group_name || ' / ' || v_ing.option_label,
            'order', v_order_id, v_profile_id
          );
          UPDATE display_stock
            SET quantity = quantity - v_ing.qty_base,
                updated_at = now()
...
-- REVOKE pair + GRANT v15 : miroir EXACT de v14 (20260705000013).
-- L'EF process-payment appelle ce RPC via le JWT utilisateur (role authenticated),
-- PAS service_role -> le GRANT TO authenticated est OBLIGATOIRE sinon la money-path
-- casse en permission denied apres le DROP v14. service_role conserve aussi (parite v14).
-- anon herite EXECUTE via PUBLIC ACL entry -> les REVOKE PUBLIC+anon sont tous deux requis.
REVOKE ALL ON FUNCTION public.complete_order_with_payment_v15(
  uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, text
) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.complete_order_with_payment_v15(
  uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, text
) FROM anon;
ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.complete_order_with_payment_v15(
  uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, text
) TO authenticated;
GRANT EXECUTE ON FUNCTION public.complete_order_with_payment_v15(
  uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, text
...
-- DROP v14 : signature exacte confirmee dans 20260710000023_complete_order_v14_flag_aware_deduction.sql.
DROP FUNCTION public.complete_order_with_payment_v14(
  uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, text
);
