-- fix(orders): void_order_rpc v8->v9 — même fix que refund v9 (numéro daté +
-- catch idempotence durci). Voir 20260727000244.
-- Corps de départ = pg_get_functiondef live de v8.

CREATE OR REPLACE FUNCTION public.void_order_rpc_v9(p_order_id uuid, p_reason text, p_authorized_by uuid, p_acting_auth_user_id uuid, p_idempotency_key uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DROP FUNCTION IF EXISTS public.void_order_rpc_v8(uuid, text, uuid, uuid, uuid);

REVOKE EXECUTE ON FUNCTION public.void_order_rpc_v9(uuid, text, uuid, uuid, uuid) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.void_order_rpc_v9(uuid, text, uuid, uuid, uuid) FROM anon;
REVOKE EXECUTE ON FUNCTION public.void_order_rpc_v9(uuid, text, uuid, uuid, uuid) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.void_order_rpc_v9(uuid, text, uuid, uuid, uuid) TO service_role;

COMMENT ON FUNCTION public.void_order_rpc_v9(uuid, text, uuid, uuid, uuid) IS
  'Void complet (EF void-order seulement). v9 : refund_number daté R-YYMMDD-NNNN + catch unique_violation durci (fix no-op silencieux au rollover de jour). anon-callable: no';