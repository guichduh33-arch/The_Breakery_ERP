-- Bug 2026-08-25 — un fini non suivi en stock (track_inventory = false,
-- deduct_stock = true, ex. Beef Cheese Croissant) paye via pay_existing_order
-- (commande envoyee en cuisine puis encaissee) deduisait SON propre stock au
-- lieu de consommer les ingredients de sa recette. La regle existe deja dans
-- complete_order_with_payment (branche track/deduct + _resolve_recipe_
-- consumption_v1, descente ADR-016) : pay_existing s'y aligne.
--
-- Corps construit depuis le corps LIVE de pay_existing_order_v17
-- (pg_get_functiondef, 2026-08-25). Changements :
--   1. boucle items : SELECT ajoute track_inventory + deduct_stock ;
--   2. branche non-combo : display/track -> stock propre (inchange) ;
--      deduct_stock seul -> consommation recette via _resolve_recipe_consumption_v1 ;
--      ni l'un ni l'autre -> aucune deduction (miroir complete_order) ;
--   3. DECLARE : + v_cons RECORD ;
--   4. rpc_version des audit_logs mis a jour.

CREATE OR REPLACE FUNCTION public.pay_existing_order_v18(p_order_id uuid, p_payment jsonb DEFAULT NULL::jsonb, p_customer_id uuid DEFAULT NULL::uuid, p_loyalty_points_redeemed integer DEFAULT 0, p_idempotency_key uuid DEFAULT NULL::uuid, p_discount_amount numeric DEFAULT 0, p_discount_type text DEFAULT NULL::text, p_discount_value numeric DEFAULT NULL::numeric, p_discount_reason text DEFAULT NULL::text, p_discount_authorized_by uuid DEFAULT NULL::uuid, p_discount_auth_id uuid DEFAULT NULL::uuid, p_promotions jsonb DEFAULT '[]'::jsonb, p_payments jsonb DEFAULT NULL::jsonb, p_offline_replay boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
  ...
  IF p_discount_amount > 0 THEN
    INSERT INTO audit_logs (actor_id, action, entity_type, entity_id, metadata)
      VALUES (v_profile_id, 'order.discount_applied', 'orders', p_order_id, jsonb_build_object(
        ...
        'rpc_version',           'v18-recipe-consumption'
      ));
  END IF;
...
-- Versioning monotone : v17 tombe dans la meme migration.
DROP FUNCTION public.pay_existing_order_v17(uuid, jsonb, uuid, integer, uuid, numeric, text, numeric, text, uuid, uuid, jsonb, jsonb, boolean);

-- Grants : paire REVOKE (defense-in-depth anon) + GRANT explicite, miroir des
-- grants live de v17 (authenticated + service_role).
REVOKE ALL ON FUNCTION public.pay_existing_order_v18(uuid, jsonb, uuid, integer, uuid, numeric, text, numeric, text, uuid, uuid, jsonb, jsonb, boolean) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.pay_existing_order_v18(uuid, jsonb, uuid, integer, uuid, numeric, text, numeric, text, uuid, uuid, jsonb, jsonb, boolean) FROM anon;
GRANT EXECUTE ON FUNCTION public.pay_existing_order_v18(uuid, jsonb, uuid, integer, uuid, numeric, text, numeric, text, uuid, uuid, jsonb, jsonb, boolean) TO authenticated, service_role;
