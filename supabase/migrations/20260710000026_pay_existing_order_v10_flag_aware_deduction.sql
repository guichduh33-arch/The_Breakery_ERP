-- 20260710000026_pay_existing_order_v10_flag_aware_deduction.sql
-- BUGFIX — #122 a câblé track_inventory/deduct_stock/allow_negative_stock dans la
-- VENTE DIRECTE (complete_order_with_payment_v14) mais a OUBLIÉ le chemin "payer
-- une commande déjà enregistrée" (held/fired → pending_payment) qui passe par
-- pay_existing_order_v10. Ce RPC gardait l'ANCIEN check stock INCONDITIONNEL sur
-- le stock propre du produit fini, ignorant track_inventory/deduct_stock ET
-- allow_negative_stock → un produit fait-à-la-commande (ex. "Egg Benedict",
-- track_inventory=false, current_stock=0) levait "Insufficient stock" au paiement.
--
-- Remplacement EN PLACE (CREATE OR REPLACE, signature inchangée → l'EF
-- process-payment et les call-sites POS ne bougent pas ; même précédent que #122
-- sur v14). Seule la boucle de déduction (combo + non-combo + modificateurs)
-- change, alignée sur complete_order_with_payment_v14 (Task 4) :
--   A. DECLARE : v_allow_negative, v_line_track, v_line_deduct, v_cons, v_cons_is_display
--   B. lecture business_config.allow_negative_stock (à côté de tax_rate)
--   C. checks combo + modificateurs gardés par allow_negative
--   D. non-combo : valide+déduit le fini SI track_inventory ; sinon (deduct_stock)
--      cascade recette via _resolve_recipe_consumption_v1 (arrêt aux nœuds suivis) ;
--      sinon rien — exactement comme v14.
-- NB : fire_counter_order_v4 ne déduit PAS le stock (snapshot only) → la déduction
-- au paiement reste unique, pas de double-déduction introduite.

CREATE OR REPLACE FUNCTION public.pay_existing_order_v10(
  p_order_id uuid,
...
DECLARE
  ...
  v_ing_track            BOOLEAN;
  -- BUGFIX (flag-aware deduction + negative-stock setting), mirrors v14 Task 4
...
BEGIN
  ...
  -- BUGFIX (B): tax_rate + réglage global stock négatif (à côté, comme v14).
  ...
  FOR v_item IN
    SELECT oi.product_id, oi.quantity, oi.combo_components, oi.modifier_ingredients_deducted,
           ...
           p.track_inventory, p.deduct_stock
      FROM order_items oi
  ...
  LOOP
    IF v_item.product_type = 'combo' THEN
      FOR v_comp IN SELECT * FROM jsonb_array_elements(COALESCE(v_item.combo_components, '[]'::jsonb)) LOOP
        ...
        END IF;
        -- BUGFIX (C): gate combo par allow_negative (mirrors v14).
      ...
      END LOOP;
    ELSE
      -- BUGFIX (D): validation flag-aware, gardée par allow_negative (mirrors v14 Task 4 C).
      IF v_item.is_display_item THEN
        IF NOT v_allow_negative
           AND COALESCE((SELECT quantity FROM display_stock WHERE product_id = v_item.product_id), 0) < v_item.quantity THEN
          RAISE EXCEPTION 'Insufficient display stock for product % (need %)',
            v_item.name, v_item.quantity USING ERRCODE = 'P0002';
        END IF;
      ELSIF v_item.track_inventory THEN
        IF NOT v_allow_negative AND v_item.current_stock < v_item.quantity THEN
          RAISE EXCEPTION 'Insufficient stock for product % (have %, need %)',
            v_item.name, v_item.current_stock, v_item.quantity USING ERRCODE = 'P0002';
        END IF;
      ...
      -- BUGFIX (D): déduction flag-aware (mirrors v14 Task 4 E).
      IF v_item.track_inventory THEN
        -- Fini suivi : décrément de son propre stock (1×), pas de cascade.
        INSERT INTO stock_movements (
          product_id, movement_type, quantity, unit, reference_type, reference_id, created_by
        ) VALUES (
          v_item.product_id, 'sale', -v_item.quantity, COALESCE(v_item.unit, 'pcs'),
          'orders', p_order_id, v_profile_id
        );
        UPDATE products
        ...
        FOR v_cons IN SELECT * FROM _resolve_recipe_consumption_v1(v_item.product_id, v_item.quantity) LOOP
          ...
          IF v_cons_is_display THEN
            INSERT INTO display_movements (
              ...
              v_cons.product_id, 'sale', -v_cons.qty_base, 'POS recipe consumption (pay existing)',
    ...
    IF v_item.modifier_ingredients_deducted IS NOT NULL THEN
      ...
      LOOP
        ...
        END IF;
        -- BUGFIX (C): gate modificateur par allow_negative (mirrors v14 Task 4 D).
...
-- CREATE OR REPLACE préserve les privilèges existants ; on réaffirme le REVOKE
-- pair anon (défense en profondeur, idempotent — cf. 20260705000017).
