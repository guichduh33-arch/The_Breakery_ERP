-- 20260717000183_adr009_d3_cancel_order_item_v5_pending_payment.sql
-- ADR-009 déc. 3 🟠 — l'annulation de ligne devient possible sur commande TIRÉE
-- non payée (pending_payment), plus seulement sur draft.
--
-- Constat (audit Orders 2026-07-17) : le POS propose l'annulation de ligne
-- (PIN + motif) dès qu'une commande est tirée ou reprise (ActiveOrderPanel:220,
-- pickedUpOrderId posé par fire_counter_order_v4), mais la garde draft-only de
-- cancel_order_item_rpc_v4 la refusait systématiquement au comptoir.
-- DESCRIPTION M2 (l.64) promet « retirer un article déjà en préparation =
-- code responsable + motif ».
--
-- v5 = corps LIVE de v4 (pg_get_functiondef, 2026-07-17) avec pour SEUL
-- changement la garde de statut : draft → draft|pending_payment.
-- Pas d'impact stock : la déduction n'a lieu qu'au passage paid ; le recalcul
-- des totaux via _pb1_split_v1 est déjà correct pour pending_payment
-- (pay_existing_order_v12 recalcule de toute façon au paiement).
-- ACLs : service_role only (EF cancel-item), trio REVOKE complet — leçon _084.

CREATE OR REPLACE FUNCTION public.cancel_order_item_rpc_v5(
...
DROP FUNCTION public.cancel_order_item_rpc_v4(uuid, text, uuid, uuid, uuid);

REVOKE EXECUTE ON FUNCTION public.cancel_order_item_rpc_v5(uuid, text, uuid, uuid, uuid) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.cancel_order_item_rpc_v5(uuid, text, uuid, uuid, uuid) FROM anon;
REVOKE EXECUTE ON FUNCTION public.cancel_order_item_rpc_v5(uuid, text, uuid, uuid, uuid) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.cancel_order_item_rpc_v5(uuid, text, uuid, uuid, uuid) TO service_role;

COMMENT ON FUNCTION public.cancel_order_item_rpc_v5(uuid, text, uuid, uuid, uuid) IS
  'Annule une ligne (flag is_cancelled) sur commande draft OU pending_payment (ADR-009 déc. 3), item non servi. PIN manager vérifié in-EF (cancel-item), idempotence via cancel_idempotency_key, totaux recalculés via _pb1_split_v1. service_role only.';
