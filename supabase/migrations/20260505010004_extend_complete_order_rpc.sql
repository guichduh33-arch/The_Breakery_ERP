-- 20260505010004_extend_complete_order_rpc.sql
-- Session 3 / migration 4 : RPC v3 — adds customer + loyalty (earn + redeem) to v2
--
-- Changes vs session 2 (000005) :
--   - signature gains p_customer_id UUID DEFAULT NULL
--                  and p_loyalty_points_redeemed INTEGER DEFAULT 0
--   - validates redeem guards (multiple of 100, sufficient balance, requires customer)
--   - computes v_redemption_amount = p_loyalty_points_redeemed * 10
--   - v_total = v_subtotal - v_redemption_amount (post-redeem tax base per L1+L5)
--   - ORDER INSERT gains customer_id + loyalty_* snapshot columns
--   - after INSERT (trigger fires base 3-line JE), appends DR LOYALTY_LIABILITY /
--     CR SALE_DISCOUNT to the same JE when v_redemption_amount > 0
--   - decrements customer.loyalty_points + inserts loyalty_transactions redeem row
...
BEGIN
  ...
  -- Idempotency check (inchangé session 1+2)
  ...
  -- 2. Loyalty redeem guards (before stock lock to fail fast)
  ...
  -- 4. Compute subtotal with modifiers (PB1 inclusive, extracted)
  ...
  -- 6. INSERT order — trigger fires base 3-line JE (DR CASH / CR SALES / CR TAX)
  INSERT INTO orders (
  ...
  --    Trigger already created the JE; extend it with DR LOYALTY_LIABILITY / CR SALE_DISCOUNT
  ...
  -- 8. INSERT order_items + stock_movements (inchangé session 2)
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
...
COMMENT ON FUNCTION complete_order_with_payment IS
  'RPC central transactionnel (session 3) : v2 + customer attach + loyalty earn/redeem + JE 2210/4900 lines.';
