-- 20260710000130_create_b2b_order_v4.sql
-- S68 — Facture PDF B2B : create_b2b_order_v3 → v4.
-- Attribue un invoice_number dédié (INV/YYYY/NNNNN via _next_b2b_invoice_number_v1)
-- à la création, persisté sur orders.invoice_number et renvoyé dans l'envelope
-- (fraîche ET replay idempotent). Corps repris du LIVE (pg_get_functiondef, DEV-S57-02).
-- Tout le reste (TOCTOU credit re-check, JE, stock display-aware, idempotence) inchangé.
-- DROP v3 même migration. Signature identique. AUCUNE taxe (tax_amount=0, B2B NON-PKP).

CREATE OR REPLACE FUNCTION public.create_b2b_order_v4(p_customer_id uuid, p_items jsonb, p_notes text DEFAULT NULL::text, p_delivery_date date DEFAULT NULL::date, p_idempotency_key uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Lock customer row FIRST, then re-check credit against the locked balance (TOCTOU fix S52).
  ...
  -- S68 — série de facture dédiée annuelle continue, attribuée à la création.
  -- Un RAISE ultérieur rollback ce tirage (une seule transaction) → pas de trou.
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- S53 P1.4: route through helper — gains display_stock / display_movements isolation.
