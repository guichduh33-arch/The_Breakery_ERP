-- ADR-013 Lot 3 -- complete_order_with_payment v19 -> v20.
-- Corps de depart = pg_get_functiondef LIVE (dump 2026-07-26). Changements :
--   1. D11 serveur : gardes etagees identiques au domaine (post_promo >= 0,
--      redemption <= post_promo, puis remise panier) au lieu de la
--      soustraction a plat a garde unique.
--   2. D15/M3 : SET search_path = public, pg_temp.
--   3. ERRCODE dedie P0014 sur le RAISE "authorizing manager" (P0001 avant) --
--      l'EF process-payment matche desormais le code, plus le texte du message.
-- Consommateur unique : EF process-payment (repointee + redeployee dans le
-- meme chantier -- lecon lot 6a). DROP v19 dans la meme migration (regle
-- versioning monotone).
CREATE OR REPLACE FUNCTION public.complete_order_with_payment_v20(p_session_id uuid, p_order_type order_type, p_items jsonb, p_payment jsonb DEFAULT NULL::jsonb, p_idempotency_key uuid DEFAULT NULL::uuid, p_customer_id uuid DEFAULT NULL::uuid, p_loyalty_points_redeemed integer DEFAULT 0, p_table_number text DEFAULT NULL::text, p_discount_amount numeric DEFAULT 0, p_discount_type text DEFAULT NULL::text, p_discount_value numeric DEFAULT NULL::numeric, p_discount_reason text DEFAULT NULL::text, p_discount_authorized_by uuid DEFAULT NULL::uuid, p_promotions jsonb DEFAULT '[]'::jsonb, p_payments jsonb DEFAULT NULL::jsonb, p_discount_auth_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
