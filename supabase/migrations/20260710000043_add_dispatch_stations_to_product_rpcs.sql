-- 20260710000043_add_dispatch_stations_to_product_rpcs.sql
-- Spec B-1 Ph2 Bloc 2.4 — étend l'allowlist create_product_v1 + update_product_v1
-- avec products.dispatch_stations (override multi-station de vente). Sans ça, le
-- champ envoyé par le formulaire BO tombe dans ignored_fields et n'est jamais
-- persisté (toggle inerte). CREATE OR REPLACE (signatures inchangées) → pas de
-- DROP/bump, REVOKE pairs S27 préservées. Base = 20260530192331 (= live, vérifié).
--
-- Sémantique du champ (text[]):
--   - clé absente              → conserve l'override existant (update) / NULL (create)
--   - clé présente = array     → pose l'override (CHECK: <@ {kitchen,barista,display})
--   - clé présente = null/JSON null → efface l'override (NULL = hériter de la catégorie)
