-- 20260813000005_bump_refund_order_rpc_v10_cross_shift.sql
-- Money-path UX/UI lot 6b — refund cross-shift controle depuis le back-office.
-- Corps copie du LIVE pg_get_functiondef de refund_order_rpc_v9 (md5 verifie),
-- substitutions chirurgicales comptees :
--   1. renommage v9 -> v10 ;
--   2. garde session conditionnelle : meme session ouverte par l'acteur =
--      comportement v9 identique ; sinon chemin cross-shift : permission
--      orders.refund exigee sur le profil ACTEUR (P0003), session d'origine
--      obligatoire (P0011 'order has no session'), refund rattache a la
--      session d'ORIGINE de la commande (jamais une session de l'acteur) ;
--   3. tout tender cash refuse en cross-shift : P0016
--      'cash_refund_requires_open_session' (store_credit et non-cash passent,
--      plafonds par methode et P0015 inchanges) ;
--   4. DROP v9 dans la meme migration ; grants identiques (service_role only).

CREATE OR REPLACE FUNCTION public.refund_order_rpc_v10(p_order_id uuid, p_lines jsonb, p_tenders jsonb, p_reason text, p_authorized_by uuid, p_idempotency_key uuid, p_acting_auth_user_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DROP FUNCTION public.refund_order_rpc_v9(uuid, jsonb, jsonb, text, uuid, uuid, uuid);

REVOKE EXECUTE ON FUNCTION public.refund_order_rpc_v10(uuid, jsonb, jsonb, text, uuid, uuid, uuid) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.refund_order_rpc_v10(uuid, jsonb, jsonb, text, uuid, uuid, uuid) FROM anon;
REVOKE EXECUTE ON FUNCTION public.refund_order_rpc_v10(uuid, jsonb, jsonb, text, uuid, uuid, uuid) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.refund_order_rpc_v10(uuid, jsonb, jsonb, text, uuid, uuid, uuid) TO service_role;
ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;

COMMENT ON FUNCTION public.refund_order_rpc_v10(uuid, jsonb, jsonb, text, uuid, uuid, uuid) IS
'v10 (lot 6b) : refund cross-shift controle. Meme session ouverte par l''acteur = chemin v9 inchange. Sinon : permission orders.refund exigee sur le profil acteur (P0003), refund rattache a la session d''origine de la commande, session NULL refusee (P0011), tender cash refuse (P0016 cash_refund_requires_open_session). service_role only — appelee par l''EF refund-order.';
