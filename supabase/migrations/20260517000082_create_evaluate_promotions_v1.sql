-- 20260517000082_create_evaluate_promotions_v1.sql
-- Session 13 / Phase 2.C — `evaluate_promotions_v1` SQL function.
--
-- Build-from-scratch (no predecessor function exists in V3 — confirmed via
-- `grep -RE "FUNCTION (public\.)?evaluate_promotions" supabase/migrations/`
-- returning 0 hits in Wave 0). Therefore: CREATE only, no DROP.
--
-- Contract:
--   evaluate_promotions_v1(
--     p_cart_items JSONB,    -- [{line_id, product_id, quantity, unit_price}, …]
...
--         "slug":          "...",
--         "name":          "...",
--         "type":          "bogo|threshold|bundle|percentage|fixed_amount|free_product",
--         "discount_amount": 15000,
--         "free_items": [{"product_id": "...", "quantity": 1}],
--         "description":  "..."
--       }, …
--     ],
--     "subtotal_before":           150000,
--     "subtotal_after_discount":   135000,
--     "total_discount":             15000
--   }
--
-- The function loads every active, non-deleted promotion (priority desc,
-- created_at desc), filters by matchers (date/dow/hour/min_items_total/
-- customer category+tier) then dispatches per `type` to inline-CTE math.
-- Stacking is applied at the end : anchor (first) always applied ;
-- subsequent require both anchor.stackable_with_promo AND
-- candidate.stackable_with_promo to be true.
--
-- Mirrors `packages/domain/src/promotions/bogoEngine.ts` semantics 1:1 so
-- the offline fallback returns identical results.

CREATE OR REPLACE FUNCTION public.evaluate_promotions_v1(
  p_cart_items JSONB,
  p_customer_id UUID DEFAULT NULL,
  p_subtotal NUMERIC DEFAULT NULL
)
...
COMMENT ON FUNCTION public.evaluate_promotions_v1(JSONB, UUID, NUMERIC) IS
  'Session 13 Phase 2.C — server-side promotion evaluator. Mirrors packages/domain/src/promotions/bogoEngine.ts. Returns JSONB with applied_promotions, subtotals, and total_discount.';

GRANT EXECUTE ON FUNCTION public.evaluate_promotions_v1(JSONB, UUID, NUMERIC)
