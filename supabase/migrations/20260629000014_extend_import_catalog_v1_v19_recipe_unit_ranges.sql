-- 20260629000014_extend_import_catalog_v1_v19_recipe_unit_ranges.sql
-- Corrective (DEV-S45-IMP-01 follow-up): extend V19 magnitude validation to the
-- two remaining NUMERIC columns the import writes outside of `products`.
--
-- _013 added V19 for products columns only. A too-large value in recipes.quantity
-- or product_unit_alternatives.factor_to_base still passed the dry-run (no check)
-- and raised a raw 22003 at commit (W5/W7) -> opaque 400. Reproduced on cloud.
--
--   recipes.quantity                       NUMERIC(10,3) -> |round(v,3)|  < 10^7
--   product_unit_alternatives.factor_to_base NUMERIC(20,10) -> |round(v,10)| < 10^10
--
-- CREATE OR REPLACE on the deployed body (= _013 + the two extra V19 checks).
-- Signature unchanged -> no types regen.
...
BEGIN
  ...
  -- V19 — magnitude : valeurs dans les bornes des colonnes NUMERIC cibles, sinon
  -- le commit lève un 22003 brut (400 opaque). On reflète toutes les colonnes
  -- NUMERIC écrites par l'import pour que le dry-run signale la cellule exacte.
  --   products.retail/wholesale_price (12,2) ; cost_price (14,2) ; min_stock (10,3)
  --   variants.retail_price (12,2) ; recipes.quantity (10,3)
  --   product_unit_alternatives.factor_to_base (20,10)
  INSERT INTO t_err
  ...
  SELECT 'Recipes', rr.row_num, rr.product_sku, 'value_out_of_range',
    ...
    FROM t_rec rr
   WHERE rr.quantity IS NOT NULL AND abs(round(rr.quantity, 3)) >= 10000000::NUMERIC;
  INSERT INTO t_err
  SELECT 'Units', u.row_num, u.product_sku, 'value_out_of_range',
    ...
    FROM t_unit u
   WHERE u.factor_to_base IS NOT NULL AND abs(round(u.factor_to_base, 10)) >= 10000000000::NUMERIC;
  ...
  -- W3 — upsert products/ingredients by SKU (soft-deleted SKU restore, DEV-S41-T7-01).
  ...
  -- W4 — upsert variants by SKU (soft-deleted SKU restore, DEV-S41-T7-01).
