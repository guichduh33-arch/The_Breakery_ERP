-- S72 — POS audit P0 fix: close_shift wrote auth.uid() into user_profiles(id) FK columns.
--
-- Bug (present since v1, copied verbatim through v5): the UPDATE set
--   closed_by = v_uid   and the zreport.draft_created audit row used actor_id = v_uid,
-- where v_uid := auth.uid() (the auth_user_id), but both pos_sessions.closed_by and
-- audit_logs.actor_id are FKs to user_profiles(id). For every employee created via the
-- real hiring chain (create_user_v1), user_profiles.id <> auth_user_id, so the write
-- raises foreign_key_violation (23503) and the WHOLE close_shift transaction rolls back
-- — the cashier cannot close their drawer (no variance JE, no z_report draft).
-- The two SEED accounts have id == auth_user_id, which masked the bug entirely (no test
-- ever exercised the create_user_v1 path). Confirmed on V3 dev: all closed sessions were
-- closed by seed accounts.
--
-- Fix = body of close_shift_v5 VERBATIM (pg_get_functiondef, 2026-07-10) with EXACTLY two
-- edits, both v_uid -> v_profile (the correctly-resolved user_profiles.id already used
-- everywhere else in the RPC): the pos_sessions.closed_by write and the second audit_logs
-- actor_id. Everything else — thresholds, 3-way volet logic, PIN gate, denominations,
-- variance JE, snapshot — is unchanged. Signature identical to v5.
--
-- Monotonic versioning: create v6, DROP v5, re-establish the REVOKE/GRANT trio.

CREATE OR REPLACE FUNCTION public.close_shift_v6(
  p_session_id uuid,
...
BEGIN
  ...
  UPDATE pos_sessions
     SET status         = 'closed',
         closed_at      = now(),
         closed_by      = v_profile,   -- S72 FIX: was v_uid (auth id) → FK violation for real users
  ...
  VALUES (
    v_profile,   -- S72 FIX: was v_uid (auth id) → FK violation for real users (audit_logs.actor_id is FK to user_profiles.id)
...
-- Drop the buggy predecessor (monotonic versioning).
DROP FUNCTION IF EXISTS public.close_shift_v5(uuid, numeric, text, uuid, uuid, text, numeric, numeric, jsonb);

-- Anon defense-in-depth trio (S20): REVOKE from PUBLIC + anon, GRANT to authenticated.
REVOKE ALL ON FUNCTION public.close_shift_v6(uuid, numeric, text, uuid, uuid, text, numeric, numeric, jsonb) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.close_shift_v6(uuid, numeric, text, uuid, uuid, text, numeric, numeric, jsonb) FROM anon;
GRANT EXECUTE ON FUNCTION public.close_shift_v6(uuid, numeric, text, uuid, uuid, text, numeric, numeric, jsonb) TO authenticated;
