-- 20260717000174_complete_order_v18_pb1_split.sql
-- Lot 6a (4/8) — complete_order_with_payment_v17 -> _v18 : la formule PB1 en dur
-- est remplacee par un appel a _pb1_split_v1 (migration _171).
--
-- A COMPORTEMENT CONSTANT. `business_config.tax_inclusive` vaut true, donc le
-- helper retourne (base, round_idr(base*r/(1+r)), base) : v_tax_amount identique
-- et v_total inchange. Preuve : s44_money_gates 12/12 + pgTAP dedie.
--
-- Provenance du corps (CLAUDE.md : « tout bump part du corps live ») : le corps
-- de _092 a ete compare au live avant reprise. Resultat verifie le 2026-07-17 :
--   * code executable (hors commentaires/lignes vides) : 775 lignes des deux
--     cotes, md5 IDENTIQUE (e275fe7c1086c56c936925d13b14087a) — aucune derive ;
--   * les 45 RAISE EXCEPTION sont identiques — aucun gate ne manque en base ;
--   * seule difference : 43 commentaires dans le fichier contre 5 en base (les
--     commentaires du corps ne survivent pas a l'application via MCP). C'est
--     cosmetique. Le fichier a donc ete repris pour CONSERVER ces commentaires,
--     apres preuve d'equivalence du code — pas par confiance aveugle.
--
-- Modifications (4, toutes generees par script, zero transcription manuelle) :
--   1. nom v17 -> v18 ;
--   2. la formule -> SELECT ... FROM _pb1_split_v1(v_total) ;
--   3. declaration v_tax_rate retiree (plus aucun lecteur) ;
--   4. SELECT business_config ne lit plus tax_rate (le helper le fait).
--
-- Grants : v17 avait anon=false / authenticated=true / service_role=true.
-- ⚠️ CAVEAT S51/S53/S55 — l'EF process-payment appelle avec le JWT UTILISATEUR :
-- sans GRANT authenticated, toute la money-path casse en `permission denied`.

CREATE OR REPLACE FUNCTION public.complete_order_with_payment_v18(
  p_session_id uuid,
...
BEGIN
  ...
  -- Task 4 (B): reglage global stock negatif. Lot 6a : tax_rate n'est plus lu ici —
  -- _pb1_split_v1 lit business_config lui-meme (seul porteur du mode taxe).
  ...
  -- (migration _171). v_total porte ici le net apres remises/promos.
  --   inclusive : tax extraite, v_total inchange  -> comportement d'origine, a l'identique.
  --   exclusive : tax ajoutee,  v_total devient le TTC a encaisser (et donc la somme
  --               des tenders attendue, cf. le gate 'Sum of tender amounts').
...
DROP FUNCTION IF EXISTS public.complete_order_with_payment_v17(uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, uuid);

REVOKE EXECUTE ON FUNCTION public.complete_order_with_payment_v18(uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, uuid) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.complete_order_with_payment_v18(uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, uuid) FROM anon;
...
GRANT EXECUTE ON FUNCTION public.complete_order_with_payment_v18(uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.complete_order_with_payment_v18(uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, uuid) TO service_role;
