-- 20260710000042_snapshot_dispatch_stations_in_order_rpcs.sql
-- Spec B-1 Ph2 Bloc 2.2 — snapshot dispatch_stations[] dans les RPC de vente.
-- REPLACE EN PLACE (signatures inchangées -> pas de DROP, pas de bump, call-sites/EF
-- intacts). Chaque corps est COPIÉ verbatim de sa derniere migration, avec 4 edits :
--   (1) déclaration v_dispatch_stations TEXT[];
--   (2) calcul via _resolve_dispatch_stations_v1 (remplace le SELECT c.dispatch_station),
--       v_dispatch_station = v_dispatch_stations[1] (legacy single deterministe);
--   (3) colonne dispatch_stations dans l'INSERT order_items;
--   (4) valeur v_dispatch_stations dans l'INSERT.
-- pay_existing_order_v10 : hors périmètre (n'insère pas d'order_items).
-- Helper requis : _resolve_dispatch_stations_v1 (migration ...041).

-- ===== create_tablet_order_v2 (REPLACE in place) =====
...
-- ===== fire_counter_order_v4 (REPLACE in place) =====
...
-- ===== complete_order_with_payment_v14 (REPLACE in place) =====
