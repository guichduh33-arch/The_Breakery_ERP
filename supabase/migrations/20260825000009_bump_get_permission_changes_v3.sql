-- 20260825000009_bump_get_permission_changes_v3.sql
-- ADR-032 — le rapport « Permission changes » voit les naissances et morts de
-- rôles. Corps repris du live pg_get_functiondef de get_permission_changes_v2,
-- + role.created / role.deleted dans le filtre. DROP v2 ici même (versioning
-- monotone).

CREATE OR REPLACE FUNCTION public.get_permission_changes_v3(p_date_start text, p_date_end text)
 RETURNS jsonb
 LANGUAGE plpgsql
...
DROP FUNCTION public.get_permission_changes_v2(text, text);

COMMENT ON FUNCTION public.get_permission_changes_v3(text, text) IS
  'ADR-032 — permission-changes report feed; adds role.created/role.deleted to the action filter.';

REVOKE ALL ON FUNCTION public.get_permission_changes_v3(text, text) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.get_permission_changes_v3(text, text) FROM anon;
GRANT EXECUTE ON FUNCTION public.get_permission_changes_v3(text, text) TO authenticated;
