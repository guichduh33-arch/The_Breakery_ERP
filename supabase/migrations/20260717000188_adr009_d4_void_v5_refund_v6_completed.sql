-- 20260717000188_adr009_d4_void_v5_refund_v6_completed.sql
-- ADR-009 déc. 4 — la transition paid→completed (_189) rendrait les commandes
-- terminées invoidables/irremboursables : les gardes `<> 'paid'` de
-- void_order_rpc_v4 et refund_order_rpc_v5 doivent accepter completed.
--
-- Corps LIVE (pg_get_functiondef, 2026-07-17). SEUL changement par RPC :
-- la garde de statut → NOT IN ('paid', 'completed'). Le trigger JE (_187)
-- accepte désormais le void depuis completed. ACLs : service_role only
-- (EF void-order / refund-order), trio REVOKE — leçon _084.
-- ⚠️ EFs void-order + refund-order mises à jour ET redéployées dans le même
-- chantier (leçon lot 6a : jamais de bump RPC sans redéploiement EF).

-- ─── void_order_rpc_v5 (ex v4) ───────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.void_order_rpc_v5(p_order_id uuid, p_reason text, p_authorized_by uuid, p_acting_auth_user_id uuid, p_idempotency_key uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- ADR-009 déc. 4 : void possible sur paid OU completed.
...
DROP FUNCTION public.void_order_rpc_v4(uuid, text, uuid, uuid, uuid);

REVOKE EXECUTE ON FUNCTION public.void_order_rpc_v5(uuid, text, uuid, uuid, uuid) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.void_order_rpc_v5(uuid, text, uuid, uuid, uuid) FROM anon;
REVOKE EXECUTE ON FUNCTION public.void_order_rpc_v5(uuid, text, uuid, uuid, uuid) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.void_order_rpc_v5(uuid, text, uuid, uuid, uuid) TO service_role;

COMMENT ON FUNCTION public.void_order_rpc_v5(uuid, text, uuid, uuid, uuid) IS
  'Void complet d''une commande paid OU completed (ADR-009 déc. 4) : restock, loyalty reversal, refund full-void, audit. PIN manager vérifié in-EF (void-order). service_role only.';

-- ─── refund_order_rpc_v6 (ex v5) ─────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.refund_order_rpc_v6(p_order_id uuid, p_lines jsonb, p_tenders jsonb, p_reason text, p_authorized_by uuid, p_idempotency_key uuid, p_acting_auth_user_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  SELECT * INTO v_order FROM orders WHERE id = p_order_id FOR UPDATE;
  IF v_order.id IS NULL THEN RAISE EXCEPTION 'Order not found' USING ERRCODE = 'P0002'; END IF;
  -- ADR-009 déc. 4 : refund possible sur paid OU completed.
  ...
  RETURN jsonb_build_object('refund_id', v_refund_id, 'refund_number', v_refund_number, 'order_id', p_order_id, 'order_number', v_order.order_number, 'total_refunded', v_refund_total, 'tax_refunded', v_tax_refunded, 'tenders', p_tenders, 'pts_deducted', v_pts_to_deduct);
END $function$;

DROP FUNCTION public.refund_order_rpc_v5(uuid, jsonb, jsonb, text, uuid, uuid, uuid);

REVOKE EXECUTE ON FUNCTION public.refund_order_rpc_v6(uuid, jsonb, jsonb, text, uuid, uuid, uuid) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.refund_order_rpc_v6(uuid, jsonb, jsonb, text, uuid, uuid, uuid) FROM anon;
REVOKE EXECUTE ON FUNCTION public.refund_order_rpc_v6(uuid, jsonb, jsonb, text, uuid, uuid, uuid) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.refund_order_rpc_v6(uuid, jsonb, jsonb, text, uuid, uuid, uuid) TO service_role;

COMMENT ON FUNCTION public.refund_order_rpc_v6(uuid, jsonb, jsonb, text, uuid, uuid, uuid) IS
  'Refund partiel/ligne d''une commande paid OU completed (ADR-009 déc. 4) : plafonds par ligne et par tender, restock, loyalty prorata, audit. PIN manager vérifié in-EF (refund-order). service_role only.';
