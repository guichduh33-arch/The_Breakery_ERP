-- 20260813000006_bump_void_order_rpc_v10_cross_shift.sql
-- Money-path UX/UI lot 6b — void cross-shift controle depuis le back-office.
-- Corps copie du LIVE pg_get_functiondef de void_order_rpc_v9 (md5 verifie),
...
--      orders.void exigee sur le profil ACTEUR (P0003, la permission existe
--      depuis 20260618000021), session d'origine obligatoire (P0011 'order
--      has no session'), void refuse si la commande a AU MOINS UN paiement
--      cash (P0016 'cash_void_requires_open_session'), sinon rattachement du
--      refund full-void a la session d'ORIGINE, comportement identique ;
--   3. DROP v9 dans la meme migration ; grants identiques (service_role only).

CREATE OR REPLACE FUNCTION public.void_order_rpc_v10(p_order_id uuid, p_reason text, p_authorized_by uuid, p_acting_auth_user_id uuid, p_idempotency_key uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DROP FUNCTION public.void_order_rpc_v9(uuid, text, uuid, uuid, uuid);

REVOKE EXECUTE ON FUNCTION public.void_order_rpc_v10(uuid, text, uuid, uuid, uuid) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.void_order_rpc_v10(uuid, text, uuid, uuid, uuid) FROM anon;
REVOKE EXECUTE ON FUNCTION public.void_order_rpc_v10(uuid, text, uuid, uuid, uuid) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.void_order_rpc_v10(uuid, text, uuid, uuid, uuid) TO service_role;
ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;

COMMENT ON FUNCTION public.void_order_rpc_v10(uuid, text, uuid, uuid, uuid) IS
'v10 (lot 6b) : void cross-shift controle. Meme session ouverte par l''acteur = chemin v9 inchange. Sinon : permission orders.void exigee sur le profil acteur (P0003), void refuse si un paiement cash existe (P0016 cash_void_requires_open_session), session NULL refusee (P0011), refund full-void rattache a la session d''origine. service_role only — appelee par l''EF void-order.';
