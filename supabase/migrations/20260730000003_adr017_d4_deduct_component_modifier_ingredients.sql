-- 20260730000003_adr017_d4_deduct_component_modifier_ingredients.sql
--
-- ADR-017 décision 4 — les ingrédients rattachés à un modificateur de composant
-- sont déduits à la vente, et restitués à l'annulation et au remboursement.
--
-- Jusqu'ici, les 200 ml de lait rattachés à « Oat Milk » sortaient sans trace
-- quand le café était vendu dans un combo : la résolution des ingrédients était
-- explicitement désactivée sur une ligne de combo, aux DEUX endroits qui la
-- font — la vérification de disponibilité (boucle 1) et le calcul du snapshot
-- persisté (boucle 2).
--
-- La cause était structurelle : _resolve_modifier_ingredients_v1 résout sur le
-- produit DE LA LIGNE, or les modificateurs vivent sur le COMPOSANT. Le nouveau
-- helper _resolve_combo_modifier_ingredients_v1 boucle sur la composition et
-- appelle le résolveur avec l'identifiant de chaque composant, en multipliant
-- par la quantité du composant dans le combo.
--
-- Restitution : le résultat est agrégé dans order_items.modifier_ingredients_deducted,
-- que void_order, refund_order et pay_existing_order relisent déjà. L'annulation
-- et le remboursement suivent donc sans code neuf.
--
-- PROVENANCE DU CORPS. Le corps de v21 est repris du fichier de migration qui
-- l'a créé (20260726000237) APRÈS avoir prouvé qu'il n'a pas dérivé du corps en
-- base : md5 du corps normalisé (espaces retirés) identique de part et d'autre,
-- 56d6fb690b805674612e744f1d8161a3 sur 29 043 caractères, vérifié le
-- 2026-07-30. Seuls les deux emplacements ci-dessus changent ; tout le reste est
-- au caractère près celui qui tournait.
--
-- Types : la signature de v22 est identique à celle de v21 (seul le nom change),
-- mais le nom d'une RPC appelée par l'EF figure dans les types générés — regen
-- + repointage de l'EF process-payment dans le même lot.

CREATE OR REPLACE FUNCTION public._resolve_combo_modifier_ingredients_v1(
  p_components jsonb,
  p_line_qty   numeric
)
RETURNS jsonb
...
AS $function$
  -- Un composant sans cle `modifiers` ne produit rien : _resolve_modifier_ingredients_v1
  -- COALESCE deja p_modifiers a '[]'. La quantite est celle de la ligne multipliee
  -- par celle du composant dans le combo (un combo peut porter 2 cafes).
  SELECT COALESCE(jsonb_agg(ing), '[]'::jsonb)
  FROM jsonb_array_elements(COALESCE(p_components, '[]'::jsonb)) AS comp,
       LATERAL jsonb_array_elements(
         _resolve_modifier_ingredients_v1(
           (comp->>'product_id')::uuid,
           comp->'modifiers',
           p_line_qty * COALESCE((comp->>'quantity')::numeric, 1)
         )
       ) AS ing;
$function$;

REVOKE EXECUTE ON FUNCTION public._resolve_combo_modifier_ingredients_v1(jsonb, numeric) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public._resolve_combo_modifier_ingredients_v1(jsonb, numeric) FROM anon;
ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;

COMMENT ON FUNCTION public._resolve_combo_modifier_ingredients_v1(jsonb, numeric) IS
  'ADR-017 D4 — ingredients des modificateurs retenus sur les composants d''un combo, resolus par composant (jamais contre le produit combo) et agreges pour la ligne.';

CREATE OR REPLACE FUNCTION public.complete_order_with_payment_v22(p_session_id uuid, p_order_type order_type, p_items jsonb, p_payment jsonb DEFAULT NULL::jsonb, p_idempotency_key uuid DEFAULT NULL::uuid, p_customer_id uuid DEFAULT NULL::uuid, p_loyalty_points_redeemed integer DEFAULT 0, p_table_number text DEFAULT NULL::text, p_discount_amount numeric DEFAULT 0, p_discount_type text DEFAULT NULL::text, p_discount_value numeric DEFAULT NULL::numeric, p_discount_reason text DEFAULT NULL::text, p_discount_authorized_by uuid DEFAULT NULL::uuid, p_promotions jsonb DEFAULT '[]'::jsonb, p_payments jsonb DEFAULT NULL::jsonb, p_discount_auth_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.complete_order_with_payment_v22(p_session_id uuid, p_order_type order_type, p_items jsonb, p_payment jsonb, p_idempotency_key uuid, p_customer_id uuid, p_loyalty_points_redeemed integer, p_table_number text, p_discount_amount numeric, p_discount_type text, p_discount_value numeric, p_discount_reason text, p_discount_authorized_by uuid, p_promotions jsonb, p_payments jsonb, p_discount_auth_id uuid) TO authenticated, service_role;

COMMENT ON FUNCTION public.complete_order_with_payment_v22(p_session_id uuid, p_order_type order_type, p_items jsonb, p_payment jsonb, p_idempotency_key uuid, p_customer_id uuid, p_loyalty_points_redeemed integer, p_table_number text, p_discount_amount numeric, p_discount_type text, p_discount_value numeric, p_discount_reason text, p_discount_authorized_by uuid, p_promotions jsonb, p_payments jsonb, p_discount_auth_id uuid) IS
  'Money-path POS (via EF process-payment). v21 ADR-013 Lot 4 : gate D8 store_credit (P0015 client requis, P0016 solde insuffisant) + debit ledger avoir.';
