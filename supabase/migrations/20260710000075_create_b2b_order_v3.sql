-- 20260710000075_create_b2b_order_v3.sql
-- S53 P1.4 — create_b2b_order v2 → v3 : route both stock-deduction branches through
-- _record_sale_stock_v1 so B2B orders are display-aware (display_stock + display_movements).
-- Behavior change: display items also decrement display_stock and append to display_movements.
-- Everything else (TOCTOU credit re-check after FOR UPDATE, JE, balance, idempotency) unchanged.
-- DROP v2 same migration.

CREATE OR REPLACE FUNCTION public.create_b2b_order_v3(p_customer_id uuid, p_items jsonb, p_notes text DEFAULT NULL::text, p_delivery_date date DEFAULT NULL::date, p_idempotency_key uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- for is_display_item products. Direct ledger writes replaced by helper calls below.
