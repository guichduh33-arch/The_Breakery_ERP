-- 20260718000195_kot_copies_per_station_set_setting_v4.sql
-- Chantier "toggles workflow cuisine" (ADR-006 décision 9, périmètre réduit en
-- session 2026-07-18) : le seul réglage retenu est la QUANTITÉ de tickets
-- cuisine papier (KOT) imprimés par station à l'envoi. Décisions propriétaire :
--   * lock des items envoyés : HORS chantier — l'ADR-010 le rend obligatoire,
--     pas de toggle ;
--   * auto-send tablette : HORS chantier — la cuisine directe (items lockés au
--     KDS dès create_tablet_order) est le comportement actuel ET définitif ;
--   * auto-fire POS au paiement : inchangé.
--
-- 3 colonnes org-wide sur business_config (catégorie symbolique `printing`) :
--   kot_copies_barista / kot_copies_kitchen / kot_copies_display
--   INTEGER NOT NULL DEFAULT 1, CHECK [0, 5]. 0 = pas de ticket papier pour la
--   station (le KDS écran reçoit toujours — l'envoi DB ne change pas) ;
--   défaut 1 = comportement actuel exact. Le ticket waiter consolidé n'est PAS
--   couvert (1 copie best effort, inchangé).
--
-- Cérémonie RPC (versioning monotone, corps repris du LIVE pg_get_functiondef
-- du 2026-07-18 — vérifié identique aux fichiers _179/_168, pas de drift) :
--   * set_setting_v3 → v4 : + 3 branches WHEN (entier [0,5], pattern des seuils
--     KDS), v3 droppée ici même ;
--   * get_settings_by_category_v2 → v3 : branche 'printing' enrichie des 3
--     clés, v2 droppée ici même.
-- Aucune edge function n'appelle ces RPCs (vérifié) — pas de redeploy EF.
...
ALTER TABLE public.business_config
  ADD COLUMN kot_copies_barista INTEGER NOT NULL DEFAULT 1
    CONSTRAINT business_config_kot_copies_barista_range CHECK (kot_copies_barista BETWEEN 0 AND 5),
  ADD COLUMN kot_copies_kitchen INTEGER NOT NULL DEFAULT 1
    CONSTRAINT business_config_kot_copies_kitchen_range CHECK (kot_copies_kitchen BETWEEN 0 AND 5),
  ADD COLUMN kot_copies_display INTEGER NOT NULL DEFAULT 1
    CONSTRAINT business_config_kot_copies_display_range CHECK (kot_copies_display BETWEEN 0 AND 5);

COMMENT ON COLUMN public.business_config.kot_copies_barista IS
  'Copies du ticket cuisine (KOT) imprimées pour la station barista à chaque envoi. 0 = pas de papier (KDS écran seulement).';
COMMENT ON COLUMN public.business_config.kot_copies_kitchen IS
  'Copies du ticket cuisine (KOT) imprimées pour la station kitchen à chaque envoi. 0 = pas de papier (KDS écran seulement).';
COMMENT ON COLUMN public.business_config.kot_copies_display IS
  'Copies du ticket cuisine (KOT) imprimées pour la station display (vitrine) à chaque envoi. 0 = pas de papier (KDS écran seulement).';

-- ---------------------------------------------------------------------------
-- 2. set_setting_v4 — corps live de v3 + 3 branches kot_copies_*
-- ---------------------------------------------------------------------------

CREATE FUNCTION public.set_setting_v4(p_key text, p_value jsonb, p_category text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
DROP FUNCTION public.set_setting_v3(text, jsonb, text);

-- ---------------------------------------------------------------------------
-- 3. get_settings_by_category_v3 — corps live de v2, branche 'printing' + 3 clés
-- ---------------------------------------------------------------------------

CREATE FUNCTION public.get_settings_by_category_v3(p_category text)
RETURNS jsonb
LANGUAGE plpgsql
...
DROP FUNCTION public.get_settings_by_category_v2(text);

-- ---------------------------------------------------------------------------
-- 4. Grants — REVOKE trio (anon defense-in-depth) + miroir des ACLs v3/v2
-- ---------------------------------------------------------------------------

REVOKE EXECUTE ON FUNCTION public.set_setting_v4(text, jsonb, text) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.set_setting_v4(text, jsonb, text) FROM anon;
GRANT  EXECUTE ON FUNCTION public.set_setting_v4(text, jsonb, text) TO authenticated, service_role;

REVOKE EXECUTE ON FUNCTION public.get_settings_by_category_v3(text) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.get_settings_by_category_v3(text) FROM anon;
GRANT  EXECUTE ON FUNCTION public.get_settings_by_category_v3(text) TO authenticated, service_role;

ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;

COMMENT ON FUNCTION public.set_setting_v4(text, jsonb, text) IS
  'Écriture des réglages business_config (validation par clé, audit-log '
  'old/new, gate tax_inclusive inchangé). v4 : + kot_copies_barista/kitchen/'
  'display — copies du ticket cuisine papier par station, entier [0, 5], '
  '0 = pas d''impression (KDS écran seulement).';

COMMENT ON FUNCTION public.get_settings_by_category_v3(text) IS
  'Lecture des réglages business_config par catégorie symbolique. v3 : la '
  'catégorie printing expose kot_copies_barista/kitchen/display.';
