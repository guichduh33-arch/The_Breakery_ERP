-- ADR-013 Lot 3 (D15/M3) -- void_order_rpc v6 -> v7.
-- Corps de depart = pg_get_functiondef LIVE (dump 2026-07-26).
-- SEUL changement : SET search_path = public, pg_temp (durcissement).
-- Consommateur unique : EF void-order (repointee + redeployee dans le meme
-- chantier -- lecon lot 6a). DROP v6 dans la meme migration.
CREATE OR REPLACE FUNCTION public.void_order_rpc_v7(p_order_id uuid, p_reason text, p_authorized_by uuid, p_acting_auth_user_id uuid, p_idempotency_key uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DROP FUNCTION IF EXISTS public.void_order_rpc_v6(p_order_id uuid, p_reason text, p_authorized_by uuid, p_acting_auth_user_id uuid, p_idempotency_key uuid);

-- ACL : EF-only (authenticated explicitement revoque, pattern v6).
REVOKE ALL ON FUNCTION public.void_order_rpc_v7(p_order_id uuid, p_reason text, p_authorized_by uuid, p_acting_auth_user_id uuid, p_idempotency_key uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.void_order_rpc_v7(p_order_id uuid, p_reason text, p_authorized_by uuid, p_acting_auth_user_id uuid, p_idempotency_key uuid) FROM anon;
REVOKE ALL ON FUNCTION public.void_order_rpc_v7(p_order_id uuid, p_reason text, p_authorized_by uuid, p_acting_auth_user_id uuid, p_idempotency_key uuid) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.void_order_rpc_v7(p_order_id uuid, p_reason text, p_authorized_by uuid, p_acting_auth_user_id uuid, p_idempotency_key uuid) TO service_role;

COMMENT ON FUNCTION public.void_order_rpc_v7(p_order_id uuid, p_reason text, p_authorized_by uuid, p_acting_auth_user_id uuid, p_idempotency_key uuid) IS
  'Void money-path (EF void-order uniquement, service_role). v7 ADR-013 Lot 3 : search_path durci public, pg_temp.';
