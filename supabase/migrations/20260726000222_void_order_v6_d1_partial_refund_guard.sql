-- ADR-013 Lot 2 D1 — void_order_rpc v5 -> v6 : garde « pas de void par-dessus un
-- refund partiel ». Si un refund non-`is_full_void` existe déjà pour l'ordre, le
-- void est refusé (le reliquat se rembourse via refund partiel). Invariant
-- SUM(refunds.total) <= orders.total préservé, une seule sortie « pleine ».
--
-- Corps de départ = pg_get_functiondef live de v5. Seul ajout : la garde D1 juste
-- avant la mutation. L'idempotence unique_violation + replay (déjà présente) est
-- conservée telle quelle. Signature inchangée.
-- ACL répliquée à l'identique de v5 (service_role only ; PUBLIC/anon/authenticated
-- révoqués — sinon CREATE re-grant EXECUTE à PUBLIC par défaut).

CREATE OR REPLACE FUNCTION public.void_order_rpc_v6(
  p_order_id uuid, p_reason text, p_authorized_by uuid,
  p_acting_auth_user_id uuid, p_idempotency_key uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- ADR-013 D1 : void INTERDIT si un refund partiel (non-full-void) existe déjà
  -- pour cet ordre. Le reliquat se rembourse via refund partiel ; on ne void pas
  -- par-dessus (sinon double contre-passation / SUM(refunds) > total).
...
-- Défense en profondeur : réplique l'ACL de v5 (service_role uniquement).
REVOKE ALL ON FUNCTION public.void_order_rpc_v6(uuid, text, uuid, uuid, uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.void_order_rpc_v6(uuid, text, uuid, uuid, uuid) FROM anon;
REVOKE ALL ON FUNCTION public.void_order_rpc_v6(uuid, text, uuid, uuid, uuid) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.void_order_rpc_v6(uuid, text, uuid, uuid, uuid) TO service_role;

-- Versioning monotone : DROP de la version précédente dans la même migration.
DROP FUNCTION IF EXISTS public.void_order_rpc_v5(uuid, text, uuid, uuid, uuid);
