-- 20260721000198_pay_existing_v13_offline_replay.sql
-- Spec 006x (hub LAN) lot 4 — arbitrage A4 : au retour d'internet, le replay
-- d'une vente CASH encaissée hors-ligne est ACCEPTÉ par le serveur (jamais de
-- rejet silencieux d'une vente encaissée) et TRACÉ dans audit_logs.
--
-- pay_existing_order_v12 → v13 (versioning monotone, corps repris du LIVE
-- pg_get_functiondef du 2026-07-21 — le fichier _175 a un drift de
-- commentaires seulement, le corps fonctionnel live fait foi) :
--   * + p_offline_replay boolean DEFAULT false (posé par le moteur de replay
--     POS uniquement — les chemins online ne le passent jamais) ;
--   * p_offline_replay = true force v_allow_negative (A4 : le stock peut
--     passer négatif, la vente a eu lieu) ;
--   * audit_logs 'order.pay_existing' : metadata + offline_replay,
--     rpc_version 'v13-lot4' (les v11/v12 étampaient encore 'v11-s53').
-- Idempotence inchangée : p_idempotency_key (flavor 2) — le replay porte la
-- clé d'ORIGINE de la tentative offline, un double replay renvoie le résultat
-- de la 1ʳᵉ exécution.
-- Aucune edge function n'appelle cette RPC (vérifié) ; call-site unique :
-- apps/pos useCheckout + moteur de replay lot 4.

CREATE FUNCTION public.pay_existing_order_v13(p_order_id uuid, p_payment jsonb DEFAULT NULL::jsonb, p_customer_id uuid DEFAULT NULL::uuid, p_loyalty_points_redeemed integer DEFAULT 0, p_idempotency_key uuid DEFAULT NULL::uuid, p_discount_amount numeric DEFAULT 0, p_discount_type text DEFAULT NULL::text, p_discount_value numeric DEFAULT NULL::numeric, p_discount_reason text DEFAULT NULL::text, p_discount_authorized_by uuid DEFAULT NULL::uuid, p_promotions jsonb DEFAULT '[]'::jsonb, p_payments jsonb DEFAULT NULL::jsonb, p_offline_replay boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- Versioning monotone : v12 droppée dans la même migration.
DROP FUNCTION public.pay_existing_order_v12(uuid, jsonb, uuid, integer, uuid, numeric, text, numeric, text, uuid, jsonb, jsonb);

-- ---------------------------------------------------------------------------
-- Grants — REVOKE trio (anon defense-in-depth), miroir des ACLs v12
-- ---------------------------------------------------------------------------

REVOKE EXECUTE ON FUNCTION public.pay_existing_order_v13(uuid, jsonb, uuid, integer, uuid, numeric, text, numeric, text, uuid, jsonb, jsonb, boolean) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.pay_existing_order_v13(uuid, jsonb, uuid, integer, uuid, numeric, text, numeric, text, uuid, jsonb, jsonb, boolean) FROM anon;
GRANT  EXECUTE ON FUNCTION public.pay_existing_order_v13(uuid, jsonb, uuid, integer, uuid, numeric, text, numeric, text, uuid, jsonb, jsonb, boolean) TO authenticated, service_role;

ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;

COMMENT ON FUNCTION public.pay_existing_order_v13(uuid, jsonb, uuid, integer, uuid, numeric, text, numeric, text, uuid, jsonb, jsonb, boolean) IS
  'Paiement d''une commande existante (multi-tender, promos gatées advisory-lock, '
  'PB1 split serveur). v13 : + p_offline_replay (spec 006x lot 4, A4) — replay '
  'accepté même à stock insuffisant (forcé négatif) et tracé offline_replay:true '
  'dans audit_logs.';
