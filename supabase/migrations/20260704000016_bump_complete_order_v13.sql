-- 20260704000016_bump_complete_order_v13.sql
-- Session 47 / Wave A / A5 — combo-aware sale RPC.
-- v12 → v13:
--   * order_items gains combo_components JSONB (snapshot of the deducted components).
--   * A combo line (product_type='combo') is virtual: its price = combo_base_price,
--     surcharges still ride in modifiers[]. Stock is checked + deducted on each chosen
--     COMPONENT (from p_items[].combo_components), NOT on the combo product, and the
--     combo's unit_price is NOT reconciled via get_customer_product_price.
-- Versioning monotone: DROP v12 in the same migration.

ALTER TABLE order_items ADD COLUMN IF NOT EXISTS combo_components JSONB;
COMMENT ON COLUMN order_items.combo_components IS
  'Session 47: for combo lines, snapshot of deducted components [{product_id, quantity}] (per combo unit).';

DO $drop$
DECLARE _r RECORD;
BEGIN
  FOR _r IN SELECT oid::regprocedure AS sig FROM pg_proc
    WHERE proname = 'complete_order_with_payment_v12' AND pronamespace = 'public'::regnamespace
  LOOP EXECUTE 'DROP FUNCTION ' || _r.sig::text || ' CASCADE'; END LOOP;
END $drop$;

CREATE OR REPLACE FUNCTION public.complete_order_with_payment_v13(
  p_session_id uuid,
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    IF v_product.product_type = 'combo' THEN
      -- S47: virtual combo. Price = base; stock checked on chosen components.
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    IF v_item_product_type = 'combo' THEN
      -- S47: deduct each chosen component instead of the virtual combo product.
      FOR v_comp IN SELECT * FROM jsonb_array_elements(COALESCE(v_item->'combo_components', '[]'::jsonb)) LOOP
        v_comp_qty := (v_comp->>'quantity')::DECIMAL * v_quantity;
