-- ADR-013 Lot 2 D12 — refund_order_rpc v6 -> v7 : durcissement idempotence.
-- L'INSERT `refunds` (early claim, porte l'idempotency_key UNIQUE) n'était pas
-- enveloppé : deux appels concurrents avec la même clé passaient tous deux le
-- pré-check replay puis le 2e levait 23505 (unique_violation non gérée) au lieu
-- de rejouer l'enveloppe du 1er (M4). Le fix reprend le pattern de void_order :
-- BEGIN INSERT ... EXCEPTION WHEN unique_violation THEN re-read + RETURN replay.
-- Le travail lourd (refund_lines / stock / tenders / loyalty) étant APRÈS l'INSERT,
-- le RETURN dans le catch le court-circuite → pas de double travail.
--
-- Corps repris verbatim de v6 ; seul changement = le wrap. Signature inchangée.
-- ACL répliquée (service_role only ; PUBLIC/anon/authenticated révoqués).

CREATE OR REPLACE FUNCTION public.refund_order_rpc_v7(
  p_order_id uuid, p_lines jsonb, p_tenders jsonb, p_reason text,
  p_authorized_by uuid, p_idempotency_key uuid, p_acting_auth_user_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- D12 : INSERT enveloppé — un refund concurrent avec la même clé a gagné la
  -- course -> rejouer l'enveloppe du 1er au lieu de lever 23505. Le travail lourd
  -- ci-dessous n'est PAS exécuté (RETURN dans le catch).
  BEGIN
...
-- Défense en profondeur : réplique l'ACL de v6 (service_role uniquement).
REVOKE ALL ON FUNCTION public.refund_order_rpc_v7(uuid, jsonb, jsonb, text, uuid, uuid, uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.refund_order_rpc_v7(uuid, jsonb, jsonb, text, uuid, uuid, uuid) FROM anon;
REVOKE ALL ON FUNCTION public.refund_order_rpc_v7(uuid, jsonb, jsonb, text, uuid, uuid, uuid) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.refund_order_rpc_v7(uuid, jsonb, jsonb, text, uuid, uuid, uuid) TO service_role;

-- Versioning monotone : DROP de la version précédente dans la même migration.
DROP FUNCTION IF EXISTS public.refund_order_rpc_v6(uuid, jsonb, jsonb, text, uuid, uuid, uuid);
