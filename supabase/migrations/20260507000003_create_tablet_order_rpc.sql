...
-- Session 5 / migration 3 : create_tablet_order RPC
-- Atomically creates an order (status=pending_payment, created_via=tablet) + locked items.
-- No payment, no JE, no stock movement — those happen at pay_existing_order.
-- The JE trigger won't fire because status != 'paid' on INSERT.
...
COMMENT ON FUNCTION create_tablet_order IS
  'Session 5: creates pending_payment tablet order with locked items sent to kitchen immediately. No payment/JE/stock.';
