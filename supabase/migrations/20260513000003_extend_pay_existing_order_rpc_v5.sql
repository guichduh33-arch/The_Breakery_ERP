-- 20260513000003_extend_pay_existing_order_rpc_v5.sql
-- Session 11 — extend pay_existing_order to v5: multi-tender split-pay support.
-- v5 = v4 (20260511000005) + p_payments JSONB array (mirror of complete_order_with_payment v8).
--
-- Same behaviour as v8 for tender validation:
--   - Mutually exclusive p_payment / p_payments
--   - Length 1..5, sum(amounts) = v_total
--   - Only the LAST tender may have cash_received > amount
--   - Insert N rows in order_payments
--
-- ISO-COMPORTEMENT WITH v4: when only p_payment is supplied, behaviour is byte-for-byte
-- identical (single iteration of the tender loop).
...
BEGIN
  ...
  -- Promotion validation (carry-over v4)
  ...
  -- v5 — validate tender sum + cash overpay rule
  v_pay_idx := 0;
  v_pay_sum := 0;
  ...
  -- Lock + stock-check + INSERT stock_movements + UPDATE products
  FOR v_item IN
  ...
  -- v5 — INSERT N rows into order_payments (one per tender, with `reference` from session 10)
  FOR v_payment_entry IN SELECT * FROM jsonb_array_elements(v_payments_arr) LOOP
...
COMMENT ON FUNCTION pay_existing_order IS
  'Session 11 v5: adds p_payments JSONB array (multi-tender) mirroring complete_order_with_payment v8. Iso-comportement v4 when only p_payment is supplied.';
