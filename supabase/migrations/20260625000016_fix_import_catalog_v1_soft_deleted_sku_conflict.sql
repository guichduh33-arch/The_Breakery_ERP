-- 20260625000016_fix_import_catalog_v1_soft_deleted_sku_conflict.sql
-- S41 corrective — DEV-S41-T7-01
--
-- Bug: import_catalog_v1 W3/W4 use `INSERT INTO products ... VALUES (...)`
-- when `SELECT id ... FROM products WHERE sku = r.sku AND deleted_at IS NULL`
-- returns no row.  But products.sku has a global UNIQUE constraint (no partial
-- predicate), so soft-deleted rows (deleted_at IS NOT NULL) still block the
-- INSERT with error 23505 unique_violation.  This causes the commit to fail
-- silently on the second import run (after the test cleanup soft-deletes the
-- data), and "Import complete" never appears.
--
-- Fix: in W3, before attempting INSERT, check whether a soft-deleted product
-- with the same SKU exists.  If it does, UPDATE (un-delete) it and capture its
-- id.  Only fall back to INSERT when no such row exists at all.
-- Same fix applied to W4 (variants use the same products table + products_sku_key).
...
COMMENT ON FUNCTION public.import_catalog_v1(JSONB, BOOLEAN, UUID) IS
  'S41 — bulk catalog import (6 sheets), dry-run validation report + atomic upsert-by-SKU commit. Gate catalog.import. Idempotency S25 flavor 2. Corrective _016: restore soft-deleted products/variants by SKU to avoid 23505 unique_violation on products_sku_key (global UNIQUE, no partial predicate). DEV-S41-T7-01.';
