-- 20260625000015_fix_import_catalog_v1_update_where_clause.sql
-- S41 corrective — DEV-S41-T6-01
-- Bug: Supabase/PostgREST enforces a "dangerous writes" safety check that
-- rejects UPDATE statements without a WHERE clause (even inside SECURITY DEFINER
-- functions). The `UPDATE t_item SET eff_unit = ...` staging step has no WHERE
-- clause and triggers this check with error 21000 "UPDATE requires a WHERE clause".
-- Fix: add `WHERE 1 = 1` (satisfies the check while still updating all rows).
-- This same corrective ensures the function definition stored in the cloud matches
-- the fix applied in _014 (COALESCE min_stock_threshold).
...
BEGIN
  ...
  -- W3 — FIX DEV-S41-T5-01: COALESCE(r.min_stock_threshold, 0)
...
COMMENT ON FUNCTION public.import_catalog_v1(JSONB, BOOLEAN, UUID) IS
  'S41 — bulk catalog import (6 sheets), dry-run validation report + atomic upsert-by-SKU commit. Gate catalog.import. Idempotency S25 flavor 2. Corrective _015: WHERE 1=1 on UPDATE t_item to satisfy Supabase dangerous-writes check.';
