-- 20260710000096_pay_existing_v11_promo_cap_hard_gate.sql
-- S57 fix pattern-guardian (MEDIUM) : pay_existing_order_v11 n'avait herite que
-- du filtrage ADVISORY des plafonds promo (evaluate_promotions_v2, _091) — sans
-- le verrou atomique. Deux paiements concurrents d'additions pouvaient depasser
-- max_uses / max_uses_per_customer (meme TOCTOU que celui ferme par _092 pour
-- complete_order_with_payment_v17). Fix in-place (precedent P10, cf. _091) :
-- pg_advisory_xact_lock + re-count + RAISE promo_cap_exceeded avant l'INSERT
-- promotion_applications — miroir exact du bloc de _092.
