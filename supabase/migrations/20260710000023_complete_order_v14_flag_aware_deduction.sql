-- 20260710000023_complete_order_v14_flag_aware_deduction.sql
-- Task 4 — câblage track_inventory/deduct_stock + allow_negative_stock dans la
-- VENTE. Remplacement EN PLACE de complete_order_with_payment_v14 (signature
-- inchangée → CREATE OR REPLACE ; pas de bump → l'EF process-payment et les
-- call-sites ne bougent pas ; précédent : 20260622000015 sur v11).
--
-- Changements vs le corps live (20260705000012), tout le reste verbatim :
--   A. DECLARE : v_allow_negative, v_line_track, v_line_deduct, v_cons, v_cons_is_display
--   B. lecture de business_config.allow_negative_stock
--   C. validation non-combo : décrément du fini si track_inventory ; sinon
--      (deduct_stock) validation des nœuds suivis de la cascade ; gate allow_negative
--   D. checks combo + modificateurs gardés par allow_negative
--   E. déduction non-combo : 1× le fini si track_inventory ; sinon (deduct_stock)
--      cascade recette via _resolve_recipe_consumption_v1 (arrêt aux nœuds suivis) ;
--      sinon rien

CREATE OR REPLACE FUNCTION public.complete_order_with_payment_v14(
  p_session_id uuid,
...
BEGIN
  ...
  -- Task 4 (B): tax_rate + réglage global stock négatif.
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    IF v_product.product_type = 'combo' THEN
      v_unit_price := COALESCE(v_product.combo_base_price, v_product.retail_price);
    ...
    ELSE
      -- Task 4 (C): validation flag-aware (fini suivi vs cascade recette), gardée par allow_negative.
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    IF v_item_product_type = 'combo' THEN
      v_unit_price := COALESCE(v_combo_base, v_unit_price);
    ELSIF NOT v_item_is_gift THEN
    ...
    -- (NULL for combo lines and for lines with no ingredient-bearing modifiers).
    ...
    -- display-aware, mirroring the combo component loop above.
