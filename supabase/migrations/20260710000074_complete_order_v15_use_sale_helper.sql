-- 20260710000074_complete_order_v15_use_sale_helper.sql
-- S53 P1.4 Task 2 : refactor complete_order_with_payment_v15 to route all four
-- stock-deduction blocks through _record_sale_stock_v1 (created in _073).
-- Behavior-identical refactor — signature unchanged, no new params or columns.
-- Changes vs _064 source:
--   A. Combo components loop body -> PERFORM _record_sale_stock_v1(...)
--   B. Simple tracked line IF v_line_track block -> PERFORM _record_sale_stock_v1(...)
--   C. Recipe consumption FOR v_cons loop body -> PERFORM _record_sale_stock_v1(...)
--      (includes removing the SELECT is_display_item INTO dead-var line)
--   D. Modifier ingredients FOR v_ing loop body -> PERFORM _record_sale_stock_v1(...)
--   DECLARE: removed dead boolean var for recipe display-check (no longer referenced).
