-- ADR-013 Lot 4 (D8) — pay_existing_order v15 -> v16.
-- Meme gate D8 que complete_order_v21 (client rattache P0015, solde sous verrou
-- P0016, agregation multi-tender) + debit ledger apres order_payments.
-- p_offline_replay ne bypasse PAS le gate (contrairement au stock / nonce
-- remise) : un solde d'avoir ne peut pas etre vrai hors-ligne.
-- Corps de depart = pg_get_functiondef live v15 (2026-07-26). DROP v15 ici.
CREATE OR REPLACE FUNCTION public.pay_existing_order_v16(p_order_id uuid, p_payment jsonb DEFAULT NULL::jsonb, p_customer_id uuid DEFAULT NULL::uuid, p_loyalty_points_redeemed integer DEFAULT 0, p_idempotency_key uuid DEFAULT NULL::uuid, p_discount_amount numeric DEFAULT 0, p_discount_type text DEFAULT NULL::text, p_discount_value numeric DEFAULT NULL::numeric, p_discount_reason text DEFAULT NULL::text, p_discount_authorized_by uuid DEFAULT NULL::uuid, p_discount_auth_id uuid DEFAULT NULL::uuid, p_promotions jsonb DEFAULT '[]'::jsonb, p_payments jsonb DEFAULT NULL::jsonb, p_offline_replay boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION public.pay_existing_order_v16(p_order_id uuid, p_payment jsonb, p_customer_id uuid, p_loyalty_points_redeemed integer, p_idempotency_key uuid, p_discount_amount numeric, p_discount_type text, p_discount_value numeric, p_discount_reason text, p_discount_authorized_by uuid, p_discount_auth_id uuid, p_promotions jsonb, p_payments jsonb, p_offline_replay boolean) TO authenticated;
GRANT EXECUTE ON FUNCTION public.pay_existing_order_v16(p_order_id uuid, p_payment jsonb, p_customer_id uuid, p_loyalty_points_redeemed integer, p_idempotency_key uuid, p_discount_amount numeric, p_discount_type text, p_discount_value numeric, p_discount_reason text, p_discount_authorized_by uuid, p_discount_auth_id uuid, p_promotions jsonb, p_payments jsonb, p_offline_replay boolean) TO service_role;

COMMENT ON FUNCTION public.pay_existing_order_v16(p_order_id uuid, p_payment jsonb, p_customer_id uuid, p_loyalty_points_redeemed integer, p_idempotency_key uuid, p_discount_amount numeric, p_discount_type text, p_discount_value numeric, p_discount_reason text, p_discount_authorized_by uuid, p_discount_auth_id uuid, p_promotions jsonb, p_payments jsonb, p_offline_replay boolean) IS
  'Money-path POS pickup/ardoise (RPC direct navigateur). v16 ADR-013 Lot 4 : gate D8 store_credit (P0015/P0016) + debit ledger avoir, non bypassable offline.';
