-- 20260512000006_extend_complete_order_rpc_v8.sql
-- Session 10 — extend complete_order_with_payment to v8.
-- v8 = v7 (20260511000004) + multi-tender split payment.
-- New behaviour:
--   - Param p_payments JSONB (default NULL) — array of {method, amount, cash_received?, change_given?, reference?}
--     Length 1..5, sum(amount) == v_total. Only the LAST entry may have cash_received > amount.
--   - If both p_payment and p_payments are provided → check_violation.
--   - If neither → check_violation.
--   - If only p_payment → wrap into single-element array (iso-comportement v7).
--   - INSERT one order_payments row per entry (carries new `reference` column from migration 1).
--   - return.change_given = sum(change_given of all tenders) — single-tender call returns same value as v7.
--
-- ISO-COMPORTEMENT WITH v7: a call with only p_payment={method,amount,...} matches v7 byte-for-byte
-- (single iteration of the tender loop, same INSERT into order_payments, same audit metadata for
-- the single payment_method). Tablet pay_existing_order v4 (which calls v7 signature) is unaffected.
...
BEGIN
  ...
  -- v8 — normalize payments input. Mutually exclusive params; at least one required.
  ...
  -- v8 — validate tenders sum to total + cash overpay rule (only last tender may overpay)
  v_pay_idx := 0;
  v_pay_sum := 0;
  v_total_change := 0;
  ...
  -- Loop 3 : INSERT order_items + stock_movements + UPDATE products.
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
  ...
  -- Session 9 — INSERT promotion_applications (no-op if empty/null)
  IF p_promotions IS NOT NULL AND jsonb_array_length(p_promotions) > 0 THEN
    FOR v_promo IN SELECT * FROM jsonb_array_elements(p_promotions) LOOP
  ...
  -- v8 — INSERT N rows into order_payments (one per tender), now carrying `reference`.
  FOR v_payment_entry IN SELECT * FROM jsonb_array_elements(v_payments_arr) LOOP
  ...
  -- Audit log : enriched with tender_count + payment_methods aggregate
...
COMMENT ON FUNCTION complete_order_with_payment IS
  'Session 10 v8: adds p_payments JSONB array (multi-tender). Mutually exclusive with p_payment (legacy single). Wraps single into array when only p_payment supplied → iso v7. Sum(amounts) = total ; only last tender may have cash_received > amount. Returns aggregated change_given.';
