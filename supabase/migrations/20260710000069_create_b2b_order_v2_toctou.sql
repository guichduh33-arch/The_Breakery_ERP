-- 20260710000069_create_b2b_order_v2_toctou.sql
-- S52 P1.2 (T5) — create_b2b_order v1 -> v2 : fix credit-limit TOCTOU.
-- v1 (_059) validated the credit limit BEFORE taking the customer FOR UPDATE lock, so two
-- concurrent orders could each pass just under the limit. v2 moves validate_b2b_credit_limit_v1
-- to AFTER the lock and checks against the locked balance. All flag-aware stock logic from
-- _059 is preserved verbatim. Gate stays pos.sale.create. DROP v1 same migration.

CREATE OR REPLACE FUNCTION public.create_b2b_order_v2(p_customer_id uuid, p_items jsonb, p_notes text DEFAULT NULL::text, p_delivery_date date DEFAULT NULL::date, p_idempotency_key uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
COMMENT ON FUNCTION public.create_b2b_order_v2(uuid, jsonb, text, date, uuid) IS
  'S52 P1.2 — B2B order creation with credit-limit re-check AFTER the customer FOR UPDATE lock '
  '(TOCTOU fix). Flag-aware stock + JE (DR AR/CR revenue) preserved from v1. Gate pos.sale.create.';
