...
-- v6 = v5 (20260508000003) avec merge des deux premiers loops sur p_items.
-- Loop 1 (lock+stock-check) et Loop 2 (compute items_total) → UN seul FOR loop.
-- Loop 3 (INSERT order_items + stock_movements + UPDATE products) reste séparé car requiert v_order_id (FK).
...
BEGIN
  ...
  -- MERGED LOOP (v6) : lock + stock-check + compute items_total en UN passage sur p_items.
  -- v5 faisait deux passes séparées (lignes 130-144 + 150-162). Iso-comportement maintenu :
  -- l'ordre des operations par item est lock → stock check → compute, et l'agrégation
  -- v_items_total reste une simple somme commutative.
  v_items_total := 0;
  ...
  -- INSERT order (v5: includes discount_* columns)
  INSERT INTO orders (
  ...
  -- Dépend de v_order_id (FK order_items.order_id) → ne peut pas fusionner avec le merged loop.
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
...
COMMENT ON FUNCTION complete_order_with_payment IS
  'RPC central transactionnel v6 (session 8 perf-debt D1): merge lock+stock+items_total en UN loop. Sémantique iso v5; seul gain perf (1 pass au lieu de 2 sur p_items, 1 SELECT FOR UPDATE par item au lieu de 2).';
