-- 20260810000003_adr022_d5_complete_order_with_payment_v25.sql
--
-- ADR-022 décision 5 — l'envoi en cuisine d'une vente payée directement est
-- assuré par le serveur, plus par le poste de caisse.
--
-- CONSTAT (mesuré sur la base de développement le 2026-08-09). Les familles
-- fire_counter_order et create_tablet_order marquent leurs lignes comme
-- envoyées en cuisine — verrou, statut cuisine, horodatage. v24 résolvait bien
-- une station de dispatch mais ne posait AUCUNE de ces trois marques. Or
-- l'écran de cuisine ne retient que les lignes verrouillées et les ordonne par
-- horodatage d'envoi. Une vente payée au comptoir n'atteignait donc jamais la
-- cuisine : elle n'existait pour la production que par son ticket papier, et si
-- l'impression échouait il ne restait rien. 137 lignes payées portant une
-- station réelle n'avaient aucune chance de s'afficher.
--
-- CE QUE FAIT CETTE MIGRATION — v24 → v25, trois changements et seulement trois :
--   1. is_locked = true, kitchen_status = 'pending', sent_to_kitchen_at = now()
--      sur toutes les lignes insérées ;
--   2. la station est résolue par _resolve_dispatch_stations_v1 — le même
--      résolveur que les deux autres portes, qui honore l'override produit
--      au-dessus de la catégorie et renseigne l'array multi-station. Le
--      JOIN categories de v24 ignorait l'override et laissait
--      dispatch_stations NULL : sans ce point, une ligne à station surchargée
--      se serait affichée à la mauvaise station ;
--   3. la garde de vendabilité, jusqu'ici écrite inline, passe par
--      _assert_product_sellable_v1 (ADR-022 déc. 1) — ligne et composants de
--      combo. Comportement et messages identiques, définition unique.
--
-- L'auto-impression post-paiement du poste CONSERVE son rôle d'impression et
-- cesse d'être le chemin par lequel la cuisine apprend l'existence de la
-- commande. Le marquage ne vaut que pour l'avenir : les commandes déjà payées
-- sans marque ne sont pas reprises (ADR-022 conséquence 4).
--
-- PROVENANCE DU CORPS. Repris de pg_get_functiondef sur ikcyvlovptebroadgtvd le
-- 2026-08-10, après application des sept remplacements gardés décrits ci-dessus.
-- La parité fichier ↔ base est prouvée par md5 du corps normalisé.

CREATE OR REPLACE FUNCTION public.complete_order_with_payment_v25(
  p_session_id uuid,
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    END IF;
    -- ADR-022 dec. 1 : la garde ADR-011 dec. 2, jusqu'ici ecrite inline ici,
    -- passe par le depositaire unique partage par les quatre portes de vente.
    -- Comportement et messages identiques ; la definition, elle, est unique.
    PERFORM _assert_product_sellable_v1(v_product.id, false);
    ...
    IF v_product.product_type = 'combo' THEN
      ...
      FOR v_comp IN SELECT * FROM jsonb_array_elements(COALESCE(v_item->'combo_components', '[]'::jsonb)) LOOP
        ...
        END IF;
        -- ADR-012 dec. 1, via le depositaire unique (ADR-022 dec. 1). La garde
        -- tourne meme pour une ligne cadeau : c'est le resolveur de prix qui est
        -- saute, pas la boucle des composants.
        PERFORM _assert_product_sellable_v1(v_comp_product.id, true);
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- ADR-022 dec. 5 : meme resolveur que fire_counter_order et
    -- create_tablet_order (override produit > categorie), et l'array
    -- multi-station est renseigne. Le JOIN categories precedent ignorait
    -- l'override produit et laissait dispatch_stations NULL.
    ...
    SELECT
      ...
      v_item_promo_id,
      v_mod_ingredients,
      -- ADR-022 dec. 5 : le marquage d'envoi en cuisine appartient a la RPC qui
      -- cree la commande, pas au poste. Sans ces trois marques, l'ecran de
      -- cuisine -- qui ne retient que les lignes verrouillees -- ne voyait
      -- jamais une vente payee directement : elle n'existait pour la production
      -- que par son ticket papier, et rien cote serveur ne constatait la perte.
      true, 'pending', now()
...
DROP FUNCTION IF EXISTS public.complete_order_with_payment_v24(uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, uuid);

REVOKE ALL ON FUNCTION public.complete_order_with_payment_v25(uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.complete_order_with_payment_v25(uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, uuid) FROM anon;
GRANT EXECUTE ON FUNCTION public.complete_order_with_payment_v25(uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, uuid) TO authenticated, service_role;

COMMENT ON FUNCTION public.complete_order_with_payment_v25(uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, uuid) IS
  'ADR-022 dec. 5 — v25 = v24 + marquage d''envoi en cuisine (is_locked, kitchen_status, sent_to_kitchen_at) sur toutes les lignes, + resolution de station par _resolve_dispatch_stations_v1 (override produit, array multi-station). Garde de vendabilite deleguee au depositaire unique (dec. 1). Une vente payee directement atteint desormais l''ecran de cuisine.';
