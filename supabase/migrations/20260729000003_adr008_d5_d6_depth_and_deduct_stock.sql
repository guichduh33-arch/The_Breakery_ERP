-- ADR-008 D5 + D6 — production: refus des articles non suivis en stock,
-- echec explicite quand un intermediaire non depliable subsiste a la profondeur max.
--
-- D6 : produire un article marque deduct_stock = false est refuse (record_production_v4,
--      + refus au bord dans record_batch_production_v6 pour nommer l'item fautif).
-- D5 : si un intermediaire NON suivi en stock subsiste a depth = 5, la production
--      echoue avec 'recipe_depth_exceeded' au lieu de sous-consommer en silence.
--      Depuis ADR-016 (20260729000001) seuls les intermediaires non stockes sont deplies :
--      un intermediaire suivi en stock a la profondeur max est legitimement consomme tel quel.
--
-- Corps repris a l'identique de pg_get_functiondef (record_production_v3 /
-- record_batch_production_v5), seules les modifications ci-dessus sont appliquees.

CREATE OR REPLACE FUNCTION public.record_production_v4(p_product_id uuid, p_quantity_produced numeric, p_section_id uuid, p_batch_number text DEFAULT NULL::text, p_quantity_waste numeric DEFAULT 0, p_notes text DEFAULT NULL::text, p_idempotency_key uuid DEFAULT NULL::uuid, p_recurse_subrecipes boolean DEFAULT true, p_expected_yield_qty numeric DEFAULT NULL::numeric, p_actual_yield_qty numeric DEFAULT NULL::numeric, p_yield_variance_reason text DEFAULT NULL::text, p_force_negative boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.record_batch_production_v6(p_batch jsonb, p_items jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  FOR v_agg IN SELECT * FROM _batch_items ORDER BY item_idx LOOP
    ...
    v_rec_result := record_production_v4(
...
-- Retrait des versions precedentes (la batch depend de l'unitaire : on drop la batch d'abord)
DROP FUNCTION IF EXISTS public.record_batch_production_v5(jsonb, jsonb);
DROP FUNCTION IF EXISTS public.record_production_v3(uuid, numeric, uuid, text, numeric, text, uuid, boolean, numeric, numeric, text, boolean);

-- Droits : anon herite EXECUTE via PUBLIC, le REVOKE sur anon seul est insuffisant.
REVOKE EXECUTE ON FUNCTION public.record_production_v4(uuid, numeric, uuid, text, numeric, text, uuid, boolean, numeric, numeric, text, boolean) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.record_production_v4(uuid, numeric, uuid, text, numeric, text, uuid, boolean, numeric, numeric, text, boolean) FROM anon;
GRANT EXECUTE ON FUNCTION public.record_production_v4(uuid, numeric, uuid, text, numeric, text, uuid, boolean, numeric, numeric, text, boolean) TO authenticated;

REVOKE EXECUTE ON FUNCTION public.record_batch_production_v6(jsonb, jsonb) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.record_batch_production_v6(jsonb, jsonb) FROM anon;
GRANT EXECUTE ON FUNCTION public.record_batch_production_v6(jsonb, jsonb) TO authenticated;

ALTER DEFAULT PRIVILEGES FOR ROLE postgres REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;

COMMENT ON FUNCTION public.record_production_v4(uuid, numeric, uuid, text, numeric, text, uuid, boolean, numeric, numeric, text, boolean) IS
  'ADR-008 D6: refuse un produit deduct_stock = false (production_requires_deduct_stock). ADR-008 D5: echoue avec recipe_depth_exceeded si un intermediaire non stocke subsiste a la profondeur max du depliage BOM. Remplace record_production_v3.';

COMMENT ON FUNCTION public.record_batch_production_v6(jsonb, jsonb) IS
  'ADR-008 D6: refuse au bord tout item dont le produit est deduct_stock = false. ADR-008 D5: pre-vol recipe_depth_exceeded si un intermediaire non stocke subsiste a la profondeur max. Appelle record_production_v4. Remplace record_batch_production_v5.';
