-- 20260809000002_adr012_d1_garde_parent_composants_combo.sql
--
-- ADR-012 décision 1, volet money-path. Corps repris du corps LIVE de
-- complete_order_with_payment_v23 (pg_get_functiondef), jamais du fichier
-- d'origine, et transformé par substitutions comptées — pas de recopie.
--
-- Un seul changement de comportement : dans la boucle des composants de combo,
-- un composant qui est un produit-PARENT (groupe de variantes) est refusé avec
-- la même erreur que la ligne, 'product_is_parent'. Jusqu'ici les composants
-- n'étaient contrôlés qu'en is_active / deleted_at : un combo dont un composant
-- était un parent se vendait.
--
-- La garde est dans la boucle des composants et non dans _resolve_combo_price_v1
-- parce que ce dernier est sauté pour une ligne cadeau (is_promo_gift) : la
-- boucle, elle, tourne toujours. Un seul bump plutôt que deux.
--
-- Versioning ADR-011 déc. 4 : v23 droppée dans la même migration.
-- EF process-payment redéployée dans la foulée (elle appelle la RPC par son nom).

CREATE OR REPLACE FUNCTION public.complete_order_with_payment_v24(p_session_id uuid, p_order_type order_type, p_items jsonb, p_payment jsonb DEFAULT NULL::jsonb, p_idempotency_key uuid DEFAULT NULL::uuid, p_customer_id uuid DEFAULT NULL::uuid, p_loyalty_points_redeemed integer DEFAULT 0, p_table_number text DEFAULT NULL::text, p_discount_amount numeric DEFAULT 0, p_discount_type text DEFAULT NULL::text, p_discount_value numeric DEFAULT NULL::numeric, p_discount_reason text DEFAULT NULL::text, p_discount_authorized_by uuid DEFAULT NULL::uuid, p_promotions jsonb DEFAULT '[]'::jsonb, p_payments jsonb DEFAULT NULL::jsonb, p_discount_auth_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
      ...
      FOR v_comp IN SELECT * FROM jsonb_array_elements(COALESCE(v_item->'combo_components', '[]'::jsonb)) LOOP
        ...
        IF v_comp_product.deleted_at IS NOT NULL OR NOT v_comp_product.is_active THEN
          RAISE EXCEPTION 'product_inactive: composant de combo % desactive', v_comp_product.name
            USING ERRCODE = 'check_violation';
        END IF;
        -- ADR-012 dec. 1 : la garde product_is_parent de la ligne s'applique aussi
        -- a chaque composant de combo. Placee dans la boucle des composants, qui
        -- tourne meme pour une ligne cadeau (le resolveur de prix, lui, est saute).
        IF EXISTS (
          SELECT 1 FROM products c
           WHERE c.parent_product_id = v_comp_product.id
             AND c.is_active
             AND c.deleted_at IS NULL
        ) THEN
          RAISE EXCEPTION 'product_is_parent: composant de combo % est un groupe de variantes - selectionnez une variante', v_comp_product.name
            USING ERRCODE = 'check_violation';
        END IF;
...
DROP FUNCTION IF EXISTS public.complete_order_with_payment_v23(uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, uuid);

-- Grants repris à l'identique de v23 (proacl relevé : postgres, authenticated,
-- service_role). L'EF process-payment appelle la RPC sous le JWT utilisateur :
-- sans GRANT authenticated, tout l'encaissement tombe en permission denied.
-- Défense en profondeur : anon hérite EXECUTE via PUBLIC.
REVOKE EXECUTE ON FUNCTION public.complete_order_with_payment_v24(uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, uuid) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.complete_order_with_payment_v24(uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, uuid) FROM anon;
GRANT EXECUTE ON FUNCTION public.complete_order_with_payment_v24(uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, uuid) TO authenticated;

COMMENT ON FUNCTION public.complete_order_with_payment_v24(uuid, order_type, jsonb, jsonb, uuid, uuid, integer, text, numeric, text, numeric, text, uuid, jsonb, jsonb, uuid) IS
  'Money-path POS (appelée par l''EF process-payment). v24 ADR-012 déc. 1 : le refus des produits-parents s''étend aux composants de combo (product_is_parent). Remplace complete_order_with_payment_v23.';
