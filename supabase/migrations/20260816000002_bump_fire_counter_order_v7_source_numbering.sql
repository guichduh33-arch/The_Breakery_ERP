-- 20260816000002_bump_fire_counter_order_v7_source_numbering.sql
-- Numérotation par origine (voir 20260816000001) — porte « fire » comptoir.
-- v7 = v6 (corps live pg_get_functiondef, verbatim) + p_source_code text DEFAULT 'P'
-- + format <code><DDMMYYYY><NNN> ; labels rpc_version fire_v6 → fire_v7.

CREATE OR REPLACE FUNCTION public.fire_counter_order_v7(p_client_uuid uuid, p_session_id uuid, p_items jsonb, p_order_id uuid DEFAULT NULL::uuid, p_table_number text DEFAULT NULL::text, p_order_type order_type DEFAULT 'take_out'::order_type, p_discount_authorized_by uuid DEFAULT NULL::uuid, p_tolerate_unsellable boolean DEFAULT false, p_source_code text DEFAULT 'P')
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
  ...
  IF p_tolerate_unsellable THEN
    ...
    INSERT INTO audit_logs (actor_id, action, entity_type, entity_id, metadata)
      VALUES (v_actor_profile, 'order.sellability_tolerated', 'orders', v_order_id,
              jsonb_build_object('rpc_version', 'fire_v7',
    ...
    INSERT INTO audit_logs (actor_id, action, entity_type, entity_id, metadata)
      VALUES (v_actor_profile, 'order.discount_applied', 'orders', v_order_id,
              jsonb_build_object('authorized_by', p_discount_authorized_by,
                                 'source', 'fire_counter_append', 'rpc_version', 'fire_v7'));
  END IF;
...
DROP FUNCTION IF EXISTS public.fire_counter_order_v6(uuid, uuid, jsonb, uuid, text, order_type, uuid, boolean);

REVOKE ALL ON FUNCTION public.fire_counter_order_v7(uuid, uuid, jsonb, uuid, text, order_type, uuid, boolean, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.fire_counter_order_v7(uuid, uuid, jsonb, uuid, text, order_type, uuid, boolean, text) FROM anon;
GRANT EXECUTE ON FUNCTION public.fire_counter_order_v7(uuid, uuid, jsonb, uuid, text, order_type, uuid, boolean, text) TO authenticated, service_role;
ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;

COMMENT ON FUNCTION public.fire_counter_order_v7(uuid, uuid, jsonb, uuid, text, order_type, uuid, boolean, text) IS
  'v7 = v6 + numérotation par origine : p_source_code (P | Tn | BO, défaut P) + format <code><DDMMYYYY><NNN> sur la séquence quotidienne partagée order_sequences. Aucun autre changement de comportement.';
