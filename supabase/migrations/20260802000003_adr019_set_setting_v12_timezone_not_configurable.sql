-- ADR-019 (D3), lot 1 « le verrou » — le fuseau horaire métier sort de la
-- surface de réglage.
--
-- L'autorité du fuseau est le paramètre de session PostgreSQL, posé pour tous
-- les rôles par 20260503000000_init_extensions_enums.sql. La colonne
-- business_config.timezone est conservée comme MIROIR (D2) : elle reste lue par
-- les fonctions qui la lisent aujourd'hui, cet ADR n'impose aucune conversion de
-- masse. Ce qui disparaît, c'est la possibilité de l'écrire à chaud : le
-- paramètre de session ne peut pas suivre (le modifier exige l'ownership de la
-- base et ne prend effet qu'aux nouvelles connexions, que le mutualiseur
-- recycle), si bien qu'un changement ferait diverger en silence les fonctions
-- qui lisent la colonne de celles qui lisent le paramètre de session.
--
-- Corps repris du corps LIVE de set_setting_v11 (pg_get_functiondef), seule la
-- branche 'timezone' change. v11 est droppée dans la même migration.

CREATE OR REPLACE FUNCTION public.set_setting_v12(p_key text, p_value jsonb, p_category text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DROP FUNCTION IF EXISTS public.set_setting_v11(text, jsonb, text);

-- Grants : miroir de l'ACL live de v11 (postgres/authenticated/service_role).
-- anon hérite EXECUTE via PUBLIC — le REVOKE sur anon seul serait insuffisant.
REVOKE EXECUTE ON FUNCTION public.set_setting_v12(text, jsonb, text) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.set_setting_v12(text, jsonb, text) FROM anon;
ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.set_setting_v12(text, jsonb, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.set_setting_v12(text, jsonb, text) TO service_role;

COMMENT ON FUNCTION public.set_setting_v12(text, jsonb, text) IS
  'Écriture des réglages business_config (validation par clé, audit-log old/new). v12 (ADR-019 D3) : la clé timezone est refusée — le fuseau métier est une constante de déploiement, son autorité est le paramètre de session PostgreSQL.';
