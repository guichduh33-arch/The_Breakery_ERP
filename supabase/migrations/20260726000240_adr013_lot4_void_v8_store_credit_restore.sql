-- ADR-013 Lot 4 (T5, decouverte de plan) — void_order_rpc v7 -> v8.
-- Restitution de l'avoir au void : une commande payee en avoir voit son JE
-- sale_void crediter 2220 (repoint _232) ; sans re-credit du ledger, la
-- reconciliation D5 casserait au premier void. Rien d'autre ne change.
-- Corps de depart = pg_get_functiondef live v7 (2026-07-26). DROP v7 ici.
CREATE OR REPLACE FUNCTION public.void_order_rpc_v8(p_order_id uuid, p_reason text, p_authorized_by uuid, p_acting_auth_user_id uuid, p_idempotency_key uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
END $function$;
DROP FUNCTION IF EXISTS public.void_order_rpc_v7(p_order_id uuid, p_reason text, p_authorized_by uuid, p_acting_auth_user_id uuid, p_idempotency_key uuid);

-- ACL : EF-only (authenticated explicitement revoque, pattern v7).
REVOKE ALL ON FUNCTION public.void_order_rpc_v8(p_order_id uuid, p_reason text, p_authorized_by uuid, p_acting_auth_user_id uuid, p_idempotency_key uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.void_order_rpc_v8(p_order_id uuid, p_reason text, p_authorized_by uuid, p_acting_auth_user_id uuid, p_idempotency_key uuid) FROM anon;
REVOKE ALL ON FUNCTION public.void_order_rpc_v8(p_order_id uuid, p_reason text, p_authorized_by uuid, p_acting_auth_user_id uuid, p_idempotency_key uuid) FROM authenticated;
ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.void_order_rpc_v8(p_order_id uuid, p_reason text, p_authorized_by uuid, p_acting_auth_user_id uuid, p_idempotency_key uuid) TO service_role;

COMMENT ON FUNCTION public.void_order_rpc_v8(p_order_id uuid, p_reason text, p_authorized_by uuid, p_acting_auth_user_id uuid, p_idempotency_key uuid) IS
  'Void money-path (EF void-order uniquement, service_role). v8 ADR-013 Lot 4 : restitution du ledger d''avoir pour les commandes payees en store_credit.';
