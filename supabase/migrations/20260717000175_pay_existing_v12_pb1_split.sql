-- 20260717000175_pay_existing_v12_pb1_split.sql
-- Lot 6a (5/8) — pay_existing_order_v11 -> _v12 : formule PB1 -> _pb1_split_v1.
--
-- A COMPORTEMENT CONSTANT (tax_inclusive = true). Meme substitution que le bump
-- complete_order v18 (_174) : le helper reecrit v_tax_amount ET v_total, car
-- v_total est aussi la somme des tenders attendue et le total persiste.
--
-- Provenance : corps de _096 PROUVE equivalent au live avant reprise
-- (2026-07-17 : 497 lignes de code hors commentaires des deux cotes,
-- md5 identique a051ef4109f34ec6d5d5b280f0ee11e9). Modifications generees par
-- script, zero transcription manuelle.
--
-- ⚠️ Call-site : le POS appelle cette RPC EN DIRECT (useCheckout.ts, JWT
-- utilisateur) — pas via une EF. Le GRANT authenticated est donc vital ici
-- aussi. (Ecart constate vs CLAUDE.md « le POS n'appelle jamais la RPC
-- directement » — signale en session, hors perimetre du lot.)

CREATE OR REPLACE FUNCTION public.pay_existing_order_v12(
  p_order_id uuid,
...
BEGIN
  ...
  -- Lot 6a : tax_rate n'est plus lu ici — _pb1_split_v1 lit business_config
  -- lui-meme (seul porteur du mode taxe).
  ...
  -- (migration _171). v_total porte le net apres remises/promos ; en exclusive il
  -- devient le TTC a encaisser (somme des tenders attendue).
  ...
  IF p_promotions IS NOT NULL AND jsonb_array_length(p_promotions) > 0 THEN
    FOR v_promo IN SELECT * FROM jsonb_array_elements(p_promotions) LOOP
      ...
      -- complete_order_with_payment_v17 (_092). Sans lui, deux paiements
      -- concurrents d'additions pouvaient depasser max_uses / per_customer
      -- (le filtrage evaluate_promotions_v2 en amont est advisory-only).
...
DROP FUNCTION IF EXISTS public.pay_existing_order_v11(uuid, jsonb, uuid, integer, uuid, numeric, text, numeric, text, uuid, jsonb, jsonb);

REVOKE EXECUTE ON FUNCTION public.pay_existing_order_v12(uuid, jsonb, uuid, integer, uuid, numeric, text, numeric, text, uuid, jsonb, jsonb) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.pay_existing_order_v12(uuid, jsonb, uuid, integer, uuid, numeric, text, numeric, text, uuid, jsonb, jsonb) FROM anon;
ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;
-- Le POS appelle en direct avec le JWT utilisateur : sans ce grant, le paiement
-- d'une commande existante (ardoise, debt, fired order) casse en permission denied.
GRANT EXECUTE ON FUNCTION public.pay_existing_order_v12(uuid, jsonb, uuid, integer, uuid, numeric, text, numeric, text, uuid, jsonb, jsonb) TO authenticated;
GRANT EXECUTE ON FUNCTION public.pay_existing_order_v12(uuid, jsonb, uuid, integer, uuid, numeric, text, numeric, text, uuid, jsonb, jsonb) TO service_role;
