...
| D8 | `expandRecipeCascade` (domain helper) stays exported and unchanged. | Still used by `RecipeEditor` live preview where the graph is already loaded client-side. `IngredientAggregatePreview` switches away from it for the multi-level cascade case where a server-side BFS is cheaper. |
...
| `DEV-S17-2.A-01` | `IngredientAggregatePreview` switches to `recipe_bom_full_v1` ; `RecipeEditor` live preview still uses `useRecipeCostBreakdown` + `expandRecipeCascade`. The split is intentional (live editor needs to project unsaved changes, which a server RPC can't do). Documented in component header. |
