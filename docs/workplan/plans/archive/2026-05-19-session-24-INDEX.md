...
- **DEV-S24-1.A-01** (informational) — The Wave 1 migrations (`20260601000005..022`) were committed and pushed but **never `apply_migration`-ed on the cloud V3 dev** until Wave 2 began ; the lead caught the drift via `list_migrations` MCP + smoke `SELECT EXISTS` query when types regen returned no B2B objects. Root cause : the prior subagent that authored the migration files emitted SQL files and `git commit`-ed them but skipped the `mcp__plugin_supabase_supabase__apply_migration` call. **Mitigation already in place** : the closeout step re-applied all 11 migrations and re-ran types regen ; verified via 10/10 smoke `EXISTS` query. **Future-proofing** : the `subagent-driven-development` prompts for DB phases should add a post-step assertion that `list_migrations` includes the freshly-added version numbers before the subagent reports success.
- **DEV-S24-1.A-02** (informational) — Migration `20260601000014_seed_b2b_payment_bank_mapping.sql` was repurposed from the spec'd "AR_B2B mapping" seed (turned out the V3 schema already has `B2B_AR` mapping_key seeded by S13, not `AR_B2B` as the spec assumed). Pre-flight `accounting_mappings` query confirmed `B2B_AR` (1132) + `SALE_B2B_REVENUE` (4131) already existed ; only `B2B_PAYMENT_BANK` (1112) was added by `_014`. RPCs `record_b2b_payment_v1` + `create_b2b_order_v1` resolve `B2B_AR` (not `AR_B2B`). No functional impact.
- **DEV-S24-1.A-03** (informational) — `record_b2b_payment_v1` builds the `allocation` JSONB by iterating B2B invoices `WHERE status='b2b_pending' AND paid_at IS NULL` — but at S24 there are no B2B orders yet on the cloud DB, so on first run the allocation array is always `[]`. This is expected and the metadata column will populate organically as B2B orders accumulate. Allocation per-invoice settlement (transitioning `b2b_pending → paid` automatically) is **explicit out-of-scope** for S24 and tracked for S26 Comptable Cockpit.

### Wave 2 (UI BO)

- **DEV-S24-2.A-01** (informational) — Spec called for `i18n fr.json (~20 strings)` but the BO codebase has **no `useTranslation` setup** (verified via `grep -r 'useTranslation\|i18next' apps/backoffice/src` → 0 matches, no `apps/backoffice/src/i18n/` directory exists). All sibling pages (B2BDashboardPage, ReceiveModal, etc.) ship inline English strings. S24 follows the same convention ; CreateB2bOrderModal and RecordB2bPaymentModal use inline English. If the project later adopts i18n it's a sweep across all BO pages, not a single-session task.
- **DEV-S24-2.A-02** (informational) — Spec called for a "CenterModal pattern S22" but the established BO modal pattern is the `Dialog` + `DialogContent` primitive (ReceiveModal, WasteModal, AdjustModal, RecipeDuplicateModal, etc., all use `Dialog`). `CenterModal` is a sibling primitive used mostly by POS. S24 uses `Dialog` for consistency with adjacent BO modals.
- **DEV-S24-2.A-03** (informational) — Spec called for a 3-step wizard (Customer / Items / Review) for CreateB2bOrderModal, but the established BO pattern (ReceiveModal, WasteModal, AdjustModal) is a single-screen Dialog form. S24 ships a single-screen form to match the visual & accessibility patterns of the rest of the BO. Multi-step wizards exist in POS only.
- **DEV-S24-2.A-04** (informational) — `useCreateB2bOrder` casts the RPC args to `any` for the `supabase.rpc('create_b2b_order_v1', rpcArgs as any)` call. The generated types declare `p_items: Json` but `B2bOrderItemInput[]` is structurally a `Json[]` — TypeScript's `Json` type uses an index-signature pattern that strict typed objects don't satisfy. Pattern matches other `supabase.rpc` calls in the codebase that pass JSONB ; refactor on hold pending a project-wide Json-cast helper.
- **DEV-S24-2.A-05** (informational) — `useB2bPaymentsReceived` uses PostgREST embedded resource (`customers!inner(name, b2b_company_name)`) for the customer join. View-side coverage of joins through `view_b2b_invoices` would be cleaner — refactor for S26 when the page also surfaces invoice rows.

### Wave 3 (closeout)

- **DEV-S24-3.A-01** (informational) — Full BO test suite (`pnpm --filter @breakery/app-backoffice test`) reports 9 failures across 7 files in the parallel run, BUT all S24-touched tests + the previously-impacted files (btob-dashboard, b2b-foundation, inventory.smoke) **pass in isolation or in smaller batches**. The failed runs all show 60-80s durations on tests that take <1s individually — i.e. the worker pool is CPU-starved on this Windows machine during the full sweep. No real regression introduced by S24 ; the test suite has pre-existing flakiness under load that is independent of this session. Recommended follow-up : a Vitest worker-count tune in `apps/backoffice/vitest.config.ts` (or a CI-only `--pool=forks --poolOptions.forks.singleFork`), tracked for a future polish session.
- **DEV-S24-3.A-02** (informational) — The CLAUDE.md `## Active Workplan` pointer was stale on **Session 19** at the start of S24 (skipped S20/S21/S22/S23). S24 closeout bumps it to S24 + moves S19 to "Session 19 reference" but the historical jumps S20/S21/S22/S23 are not re-summarized in the pointer block. Acceptable trade-off — those sessions are documented in their respective INDEX/specs and the roadmap globale §Sessions table.
