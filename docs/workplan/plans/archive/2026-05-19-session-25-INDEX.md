...
**Spec :** [`../specs/2026-05-19-session-25-spec.md`](../../specs/archive/2026-05-19-session-25-spec.md)
...
- **DEV-S25-1.A-01** (informational) — `supabase/functions/_shared/idempotency.ts` UUID_REGEX accepts UUID v1-5 (`/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i`) but the `InvalidIdempotencyKeyError` message says "must be UUID v4". Harmless in practice because POS uses `crypto.randomUUID()` which always returns v4 ; the regex matches any RFC4122 UUID. If strict v4 validation is needed, tighten to `/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i`.
- **DEV-S25-1.A-02** (informational) — Migration `20260602000013_alter_default_privileges_revoke_from_public` was added as a corrective after code review noted the originally-applied `_012` only revoked EXECUTE FROM anon, not FROM PUBLIC. The `ALTER DEFAULT PRIVILEGES ... FROM PUBLIC` line in `_013` is currently a no-op (S20 global sweep migration `20260524000031` already set this for all future functions in schema public), BUT the `_012` + `_013` pair is now the canonical defense-in-depth template for any future per-RPC REVOKE-EXECUTE migration. Future migrations should emit both lines explicitly even when the default is already in place — readability and self-documenting intent over saved characters.
- **DEV-S25-1.A-03** (**medium**) — Silent-skip on unknown `product_id` in the `create_tablet_order_v2` FOR loop is inherited from v1. Pre-S25, a typo'd product id would create an empty order, the user would just retry with a corrected payload, no harm done. **With idempotency the failure mode is now permanently unrecoverable for the session** : the first call creates a truncated order, the idempotency key is bound to that truncated `order_id`, and every retry (even with corrected payload) replays the truncated `order_id` because the PK match wins before any new INSERT. Track for v3 : add `IF NOT FOUND THEN RAISE EXCEPTION 'Product % not found', v_product_id` in the FOR loop. Marker for backlog post-S30.
- **DEV-S25-1.A-04** (informational) — Idempotency replay in `create_tablet_order_v2` bypasses the `has_permission(v_user_id, 'sales.create')` check. Deliberate trade-off (matches S24 `record_b2b_payment_v1` pattern) — replay is read-only-like since the work already happened ; gating it would create a confusing UX where the same client_uuid succeeds at T0 but fails at T1 after a role change. Documented in the RPC body comment.
- **DEV-S25-1.A-05** (informational) — `apps/pos/src/features/tablet/TabletCheckoutButton.tsx` was extended with the same `clientUuidRef = useRef(crypto.randomUUID())` lifecycle as `TabletOrderPage.tsx`. Each component owns its own ref (correct — separate mounted components, separate submission cycles, distinct user intents). Not in the original spec because the spec only mentioned the page-level caller, but necessary for the 2nd production call site. Discovered via grep during Sub-phase 1.A.5 implementation.
- **DEV-S25-1.A-06** (**medium, fixed this session**) — Corrective migration `20260602000014_relax_orders_session_id_for_tablet`. S24 migration `20260601000006_relax_orders_session_id_nullable` introduced a CHECK gate that allowed `session_id IS NULL` ONLY when `order_type='b2b'`. The existing tablet ordering flow (which inserts `created_via='tablet'` with `session_id=NULL`) has been silently broken since S24 shipped — no end-to-end test exercised the tablet→order→insert path between S24 merge and S25 Wave 2. Caught by Phase 2.A.1 T1 (`create_tablet_order_v2` first call inserts row) which failed with `new row violates check constraint orders_session_id_required`. Migration `_014` now allows `created_via='tablet'` to also bypass `session_id NOT NULL`. Lesson : adding a CHECK constraint touching a NOT NULL column needs an explicit happy-path smoke test for every existing `created_via` value, not just the new feature's value.
- **DEV-S25-1.A-07** (**medium, fixed this session**) — Corrective migration `20260602000015_fix_refund_order_rpc_v2_replay_record`. S13 migration `20260517000014_bump_refund_order_rpc_v2.sql` had a dormant bug at line 90 : the replay-envelope branch wrote to `v_order.order_number` on an unassigned `RECORD` variable, raising a `record_has_no_field` error. The bug was dormant S13→S25 because no caller passed `p_idempotency_key` (S13 added the arg without wiring the EF). S25 wired the header in Sub-phase 1.A.3, and the bug surfaced immediately in Phase 2.A.1 T4 (`refund_order_rpc_v2` first call with idempotency key). Fix via `CREATE OR REPLACE` with a local `v_existing_order_number TEXT` variable assigned from a separate SELECT before being used in the replay envelope. RPC signature unchanged ; replay semantics now correct.

### Wave 2 (tests)

- **DEV-S25-2.A-01** (informational) — Vitest live tests in `supabase/tests/functions/idempotency-hardening.test.ts` (TS1-TS5, 5 scenarios) were authored + structurally validated (Vitest loads file, 5 tests collected, fails on `EMP001 not found` with placeholder service-role key — expected behavior). Actual live run requires `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` exported user-side before `npx vitest run functions/idempotency-hardening`. Same project-wide gap as S19 DEV-S19-2.A-01. The `refund-order` EF v7 was deployed to V3 dev cloud via MCP so live tests will exercise the right code on first user-driven run.
- **DEV-S25-2.A-02** (informational) — 13 backoffice test files fail with `Invalid environment variables: VITE_SUPABASE_URL Required` / `VITE_SUPABASE_ANON_KEY Required` when `pnpm --filter @breakery/app-backoffice test` runs without the `.env` populated. Pre-existing env-gating pattern (the `apps/backoffice/src/env.ts` module validates at import time). S25 touched zero backoffice files, so these are out-of-scope. Cleaner fix would be to lazy-load the env validation or split env vars into test-overridable groups, tracked for a future polish session.
- **DEV-S25-2.A-03** (informational) — pgTAP test file `supabase/tests/idempotency_hardening.test.sql` uses Postgres GUCs (`current_setting('breakery.t1_pass'::text)`) to chain pass/fail flags between DO blocks because pgTAP's `ok()` must be called from a SELECT — not from a DO block. Pattern is portable and works against both MCP `execute_sql` and any future pgTAP harness. The alternative (turning each test into a CTE) would have required restructuring all 8 tests as a single giant query. GUC approach scales linearly with test count.

### Wave 3 (closeout)

- **DEV-S25-3.A-01** (informational) — The corrective `_014` (relax `orders.session_id` for tablet) is the second consecutive session to catch a latent constraint-relaxation bug from the previous session at pgTAP gate (S24 relaxed B2B path, S25 caught + fixed the tablet path). Pattern emerging : "session N adds a new `created_via`/`order_type` path with a check gate" → "session N+1 the next time it ships a related feature is the one that catches the missing gate path". Suggests adding to the subagent-driven-development DB phase prompt template : "If your migration touches a CHECK constraint on a column with multiple existing values, list all values + assert each path still works via a SELECT pgTAP test in the same migration."
- **DEV-S25-3.A-02** (informational) — `tests/idempotency-hardening.test.ts` lives under `supabase/tests/functions/` per the CLAUDE.md "Inventory phase test layout" convention, but the convention page only explicitly lists `inventory-*.test.ts` as the pattern. The S24 `record-b2b-payment.test.ts` and now S25 `idempotency-hardening.test.ts` extend the convention informally to non-inventory EF tests. Worth a brief update to CLAUDE.md's "Inventory phase test layout" section to generalize ("Vitest live RPC: `supabase/tests/functions/<scope>-*.test.ts` (per-scope, one file per RPC family or hardening sweep)"). Tracked but not blocking.
