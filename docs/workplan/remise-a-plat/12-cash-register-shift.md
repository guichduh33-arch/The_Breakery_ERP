...
> ⚠️ **Corps figé au `5b0fa92` (2026-07-04) — partiellement périmé.** Le tableau §C ligne **B1.4** (encore noté 🔴 MANQUANT « 2 sous-claims sur 4 ») est dépassé : le **PIN manager sur gros écart** a été livré en **S66** (`close_shift_v4`, seuils `business_config.shift_variance_pin_threshold_abs/pct`) et le **comptage 3 volets cash/QRIS/carte + comptage par coupure** en **S67** (`close_shift_v5`, `packages/domain/src/cash/denominations.ts`). Le corps/tableau figé ci-dessous n'est PAS réécrit (append-only) ; se fier au présent bandeau, aux mises à jour S66/S67 ci-dessous et à CLAUDE.md pour l'état courant.

> ⚠️ **Mise à jour S67 (2026-07-07, `swarm/session-67`)** : **D2.2 + D2.3 livrés** — **`close_shift_v4 → v5`** (migrations `_125..128`, DROP v4) : **comptage 3 volets** cash / QRIS / carte (carte = `card`+`edc` fusionnés ; `p_counted_qris`/`p_counted_card`, NULL = volet non compté — méthode désactivée S64) avec expected par volet calculé serveur, variances persistées (`pos_sessions.counted_qris/card`) et figées au **snapshot Z section `reconciliation`** (+ rendu PDF/BO rétro-compatible) ; **gardes note (S60) et PIN (S66) étendues en OR sur les volets** ; **aucune JE non-cash** (décision propriétaire — trace + gardes seulement, la JE cash 1110↔4910/5910 est inchangée). **Comptage par coupure opt-in** (`business_config.shift_denomination_count_enabled`, défaut OFF, toggle BO Settings) : grille IDR canonique (`packages/domain/src/cash/denominations.ts`) à l'open (client-only, `opening_denominations`) et au close (**enforced serveur** : `denominations_required`/`invalid_denomination`/`denomination_total_mismatch`, somme = `p_counted_cash`, persistée `closing_denominations`). B1.4 « trois volets » et B1.1/B2.5 « par coupure » deviennent réels. Suites `close_shift_three_way` 15/15 · `close_shift_pin_gate` 11/11 (repointée v5) · `close_shift_note_enforced` 7/7 · `cash_register` 12/12. Dettes D-1..D-8 + déviations DEV-S67-01..07 : `docs/workplan/plans/archive/2026-07-07-session-67-INDEX.md`.

> ⚠️ **Mise à jour S66 (2026-07-07, `swarm/session-66`)** : **D2.1 livré** — **`close_shift_v3 → v4`** (migrations `_118..120`, DROP v3) : au-delà des nouveaux seuils `business_config.shift_variance_pin_threshold_abs/pct` (défauts 200 000 IDR / 2 %, éditables BO Settings), la clôture exige un **approbateur désigné** (permission `shift.variance.approve`, seedée MANAGER/ADMIN/SUPER_ADMIN) + son **PIN 6 chiffres** validé `_verify_pin_with_lockout` ; trace `pos_sessions.variance_approved_by` + audit ; section « Manager approval » dans `CloseShiftModal` (blind count intact). Le sous-claim « PIN manager gros écarts » de B1.4 est donc réel ; restent D2.2 (3 volets) et D2.3 (coupures). Suites `close_shift_pin_gate` 11/11 · `close_shift_note_enforced` 7/7 (repointée v4) · `cash_register` 12/12. Finding F-1 (lockout PIN-in-arg non durable, pré-existant S38) : INDEX S66. Voir `docs/workplan/plans/archive/2026-07-07-session-66-INDEX.md`.

> ⚠️ **Mise à jour S60 (2026-07-05, `swarm/session-60`)** : **D1.1 livré** — `reason_code` exposé dans `CashInOutModal` (+ clé d'idempotence) et **le modal, découvert orphelin, est monté** (SideMenuDrawer → Cash In / Cash Out, session requise) : un apport owner produit sa JE 1110/3100. **D1.4 livré** — **`close_shift_v2 → v3`** (migration `_105`, DROP v2) : la note d'écart au-delà du seuil est enforced serveur (`variance_note_required`, prédicat miroir de `shouldShowWarning`, replay idempotent exempté) ; suite pgTAP `close_shift_note_enforced` + `cash_register.test.sql` réparée (T_SHIFT_03 assertait les v1 droppées). Voir `docs/workplan/plans/archive/2026-07-05-session-60-INDEX.md`.
