...
| 04 | [Écran cuisine (KDS)](04-kds-kitchen.md) | Opérationnel | 1 | 5 | 2 | Surclamé ; beaucoup de prêt-mais-débranché (§2.3) — le 🟠 de B1.7 renvoie à la fiche 21 qui fait foi sur le transport local ; **seuils réglables org + chips câblés (S75)** | Réglage **par poste** (police/disposition/mute) + alarme critique >12 min réelle + « Tout prêt » |
| 05 | [Catalogue produits](05-products-categories.md) | Op. (partiel) | 5 | 3 | 0 | Fidèle à la vente ; prix négociés éditables (S69) | Reste : couleur de catégorie configurable, vente unité alternative |
...
| 08 | [Clients & fidélité](08-customers-loyalty.md) | Opérationnel | 5 | 2 | 0 | Points OK ; **CRUD catégories + prix négocié client livrés (S69, ferme D-W6-CUSTCAT-01)** ; QR membre inexistant | QR membre ; expiration des points (D3) |
| 09 | [Clients pros (B2B)](09-b2b-wholesale.md) | Opérationnel | 6 | 0 | 1 | ~~ni facture PDF~~ facture PDF (S68) ; **prix négocié client livré (S69)** ; ~~reste surclamé : cycle livraison~~ → **hors périmètre** | ~~Chantier cycle de livraison~~ **ANNULÉ (2026-07-10 : pas de livraison motorisée, retrait sur place)** |
...
> Exécutée sur `swarm/session-58` — détail, commits et findings dans [`../plans/2026-07-04-session-58-INDEX.md`](../plans/archive/2026-07-04-session-58-INDEX.md).
...
- ✅ **CRUD Customer Categories + UI prix négociés — SOLDÉ (S69, 2026-07-08)** (08/05) : CRUD `customer_categories` (`create/update/delete_customer_category_v1`, ferme D-W6-CUSTCAT-01) + overrides catégorie éditables + **prix négocié par client** (table `customer_product_prices` + perm `customer_prices.manage`, résolution serveur `_resolve_b2b_line_price_v1` négocié > catégorie > retail dans `create_b2b_order_v5`). Débloque et ferme le B2B « prix négocié » (09-B1.1). Cf. [`../plans/2026-07-08-session-69-INDEX.md`](../plans/2026-07-08-session-69-INDEX.md).
...
**Décision propriétaire — 2026-07-10 : pas de livraison motorisée B2B (retrait sur place).** Les clients pros **viennent récupérer leur commande eux-mêmes** ; il n'y a donc pas de cycle de livraison B2B (préparation → livraison partielle → livrée), et aucune tournée/expédition à modéliser. Conséquences : le chantier **cycle de livraison B2B (fiche 09, D3.1)** est **ANNULÉ** (le stock reste déduit à la création, modèle actuel conservé) ; l'item dépendant **avoirs officiels B2B (09-B2.4)** tombe aussi ; côté doc v1.3, retirer la revendication « cycle confirmée→préparation→prête→livrée + livraisons partielles » (module 09-B1.3) plutôt que la reformuler « À venir ». Le fait générateur de la JE AR B2B (module 10) reste donc « à la création » — non impacté.
