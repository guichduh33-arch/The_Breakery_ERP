...
> **MAJ S72 (2026-07-11)** : côté **POS Reports** (adjacent au périmètre BO de cette fiche), l'onglet Activity est scindé **Sales | Journal** — nouveau **journal d'audit opérationnel** (`pos_devices` + `pos_events` partitionnée append-only + `record_pos_events_v1` ingest idempotent offline + `get_pos_events_v1` keyset, flux unifié gestes client ∪ outcomes serveur, gate `reports.audit.read`) : chaque manipulation opérateur par terminal, signaux de fraude surlignés, timeline par ticket, export CSV. Les onglets financiers restent dérivés des tables money-path (réconciliation assertée en pgTAP). Cf. `../plans/2026-07-11-session-72-INDEX.md`.
> **MAJ S74 (2026-07-12)** : la **refonte Reports POS est soldée** (PR #208, dernier lot) — onglet **Margin** côté POS (`apps/pos/src/features/reports`) : RPC lecture pure `get_pos_margin_v1` (migration `_160`), COGS = **WAC courant** (`products.cost_price`, caveat permanent affiché — le snapshot COGS reste en Vague 3 / B2.1), périmètre ≡ Overview (réconciliation `revenue_ttc` assertée en pgTAP), lignes promo-gift comptées en COGS avec revenue 0, compteur `products_without_cost`, gate **`reports.financial.read`** (onglet masqué + 42501 serveur), export CSV. **B1.3 est désormais couvert aussi côté POS** (le BO l'était depuis `_093`). Divergence documentée vs `get_gross_margin_by_product_v1` BO (qui inclut le B2B settled et n'exclut pas les produits test). Cf. `../plans/2026-07-12-session-74-margin-plan.md`.
