...
> ⚠️ **Mise à jour S62 (2026-07-06, `swarm/session-62`)** : **D2.3 soldé par PURGE** (décision propriétaire 3 du 2026-07-06) — le champ `discount` des `TIERS` est retiré du domaine : les paliers ne servent plus qu'au multiplicateur de POINTS (`points_multiplier`, intact serveur et client). Besoin futur de remise → promotion par catégorie client (mécanisme existant). Voir `docs/workplan/plans/archive/2026-07-06-session-62-INDEX.md`.

> ✅ **Mise à jour S69 (2026-07-08, `worktree-session-69`)** : **D2.1 SOLDÉ — CRUD catégories clients réel + prix négocié par client.** La page `CustomerCategoriesPage` est désormais un **vrai CRUD** (RPCs `create/update/delete_customer_category_v1`, `CategoryFormModal`) — **ferme la déviation D-W6-CUSTCAT-01** — et les overrides de prix catégorie sont éditables (`upsert/delete_product_category_price_v1`, `PricingTab`). En plus, **prix négocié par CLIENT** : table **`customer_product_prices`** + perm **`customer_prices.manage`** + RPCs `upsert/delete_customer_product_price_v1`, résolution serveur `_resolve_b2b_line_price_v1` (négocié > catégorie > retail) dans `create_b2b_order_v5` (côté B2B), UI `NegotiatedPricesSection` sur la fiche client. Le verdict C-B1.2 passe de 🟠 PARTIEL à ✅ CONFORME (S69). Voir [`../plans/2026-07-08-session-69-INDEX.md`](../plans/2026-07-08-session-69-INDEX.md). — corps figé ci-dessous NON réécrit (`5b0fa92`).

> ✅ **Mise à jour S76 (2026-07-13)** : **D1 nettoyage soldé** — `RedeemButton.tsx` (⚫#14, jamais importé, le redeem passe par `BottomActionBar`) **purgé** (commit `6f189bb4`). **Découverte connexe (fiche 09)** : en câblant `adjust_b2b_balance_v2` (module 09, ⚫#13) sur la fiche client, le composant `B2BFieldsSection.tsx` — jusqu'ici documenté comme la « carte B2B » de la fiche client — s'est révélé **sans importeur en production** (`InfoTab` porte sa propre carte B2B inline) et a été **purgé** (décision propriétaire 2026-07-13, commit `b55b276b`) ; détail complet en fiche 09. Voir [`../plans/2026-07-13-session-76-plan.md`](../plans/2026-07-13-session-76-plan.md).
...
| B1.2 | Catégories tarifaires appliquées automatiquement | Application automatique ✓ (POS + serveur `_resolve_line_price_v1`) ; ~~CRUD catégories désactivé (D-W6-CUSTCAT-01) et aucune UI d'écriture des prix négociés~~ **LIVRÉ S69** : CRUD catégories réel (`create/update/delete_customer_category_v1`, ferme D-W6-CUSTCAT-01), overrides catégorie éditables (`upsert/delete_product_category_price_v1`) **et** prix négocié par client (`customer_product_prices` + `customer_prices.manage`, `NegotiatedPricesSection`). *(Constat figé `5b0fa92` : page read-only, config SQL-seule — corrigé S69.)* | ✅ CONFORME (S69) |
...
2. ✅ **Nettoyage** : le composant orphelin `RedeemButton.tsx` (jamais importé — le redeem passe par `BottomActionBar`) — **purgé S76** (commit `6f189bb4`).

### D2. Chantiers moyens (1 session, plan requis)
1. ✅ **CRUD catégories tarifaires + prix négociés — SOLDÉ (S69, 2026-07-08)** (ferme D-W6-CUSTCAT-01, ouvert depuis S14) : RPCs `create/update/delete_customer_category_v1` + `upsert/delete_product_category_price_v1` livrés, boutons de `CustomerCategoriesPage` activés (`CategoryFormModal`), `PricingTab` éditable ; **en plus** prix négocié par client (`customer_product_prices` + perm `customer_prices.manage` + `create_b2b_order_v5` résolution serveur). Le scénario « le responsable B2B enregistre le tarif produit par produit » passe en vrai. Cf. [`../plans/2026-07-08-session-69-INDEX.md`](../plans/2026-07-08-session-69-INDEX.md).
