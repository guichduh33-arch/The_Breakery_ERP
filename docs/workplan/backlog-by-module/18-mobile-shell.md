...
### TASK-18-001 — Indicateur réseau réel dans MobileLayout [P2] [BLOCKED]
**Status note (2026-05-14)** : Deferred to Wave 7 (Session 16 — Capacitor mobile shell). Per INDEX `## WAVE 7 — Deferred` line 1086 : "18 mobile shell Capacitor (Session 16, ou PWA-first hors-périmètre toujours)". V3 has no Capacitor wrappers, no `MobileLayout`, no `apps/pos/src/pages/mobile/*` — entire mobile-native surface deferred.
...
### TASK-18-002 — Activer Capacitor Haptics + Network plugins [P2] [BLOCKED]
**Status note (2026-05-14)** : Deferred to Wave 7 (Session 16) per INDEX line 1086. No `@capacitor/*` deps in V3 `package.json` ; no native plugin pipeline exists.
...
### TASK-18-003 — CI/CD build APK Android automatisé [P2] [BLOCKED]
**Status note (2026-05-14)** : Deferred to Wave 7 (Session 16) per INDEX line 1086. V3 ships PWA-only via Vercel ; no `android/` folder, no `mobile-android.yml` workflow. Picked up once the Capacitor port lands.
...
### TASK-18-004 — iOS build pipeline (validation manuelle) [P3] [BLOCKED]
**Status note (2026-05-14)** : Deferred to Wave 7 (Session 16+) per INDEX line 1086. Depends on TASK-18-002 (Capacitor plugins) which is itself BLOCKED — iOS is downstream of Android port.
...
### TASK-18-005 — Performance cold start Android (mesure + budget) [P2] [BLOCKED]
**Status note (2026-05-14)** : Deferred to Wave 7 (Session 16) per INDEX line 1086. Depends on TASK-18-003 (APK build) which is BLOCKED.
...
### TASK-18-006 — Push notifications (orders ready, low-stock) [P3] [BLOCKED]
**Status note (2026-05-14)** : Deferred to Wave 7 (Session 16+) per INDEX line 1086. Session 13 Phase 5.B shipped the email-only notification pipeline (`packages/domain/src/notifications/`, EF `notification-dispatch`) — FCM push is downstream of the Capacitor port (TASK-18-002 BLOCKED).
...
### TASK-18-007 — Deep linking custom scheme [P3] [BLOCKED]
**Status note (2026-05-14)** : Deferred to Wave 7 (Session 16+) per INDEX line 1086. Depends on TASK-18-003 (Android pipeline) — BLOCKED.
...
### TASK-18-008 — Mobile-first redesign pages /mobile/* [P2] [OBSOLETE]
**Status note (2026-05-14)** : V2-only — no `/mobile/*` route exists in V3. POS is desktop/tablet-first (`apps/pos/src/pages/`) with no `MobileLayout` component. Reframe as a fresh Session 16 spec once Capacitor lands ; the V2 page list does not survive translation.
...
### TASK-18-009 — Capacitor 7 → 8 upgrade [P3] [OBSOLETE]
**Status note (2026-05-14)** : V2-only task — V3 has no Capacitor install (no `@capacitor/core` in `package.json`, no `capacitor.config.ts`). The Wave 7 port (Session 16) will start fresh on Capacitor 8+, making the V2-era upgrade ticket moot.
...
### TASK-18-010 — Camera barcode scanner (mobile inventory) [P3] [BLOCKED]
**Status note (2026-05-14)** : Deferred to Wave 7 (Session 16+) per INDEX line 1086. Depends on TASK-18-002 (Capacitor plugins) — BLOCKED. A web-only `getUserMedia` MVP could land separately but is out-of-scope until the mobile shell is committed.
