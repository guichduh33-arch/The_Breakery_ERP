...
**Status note (2026-05-18)** : S22 update — empirical audit confirmed 30+ modals in `apps/` all route through Radix-backed primitives (`Dialog`, `FullScreenModal`, `CenterModal`, `Sheet` from `packages/ui`). Lock-in via 16 RTL focus-trap regression tests colocated as `packages/ui/src/{primitives,components}/__tests__/*.focus-trap.test.tsx` (4 files × 4 cases : Tab fwd / Shift+Tab back / ESC closes / focus restored on close — adapted "focus not stranded" assertion for FullScreenModal/CenterModal which lack `DialogTrigger`, see DEV-S22-1.A.1-01). ESLint custom rule `no-raw-modal-overlay` shipped via inline `tools/eslint-rules/no-raw-modal-overlay.mjs` + wired in root `eslint.config.mjs` as plugin `breakery-local` at level `error` (DEV-S22-1.A.2-01 — `packages/eslint-config` does not exist in this monorepo, inline path was the only viable shape). Rule detects `<div className="fixed inset-0">` literal + `<div style={{position:'fixed', inset:0}}>` object expression ; exempts `packages/ui/**`, `**/__tests__/**`, `*.test.*`, `*.stories.*`. RuleTester 9 cases green (4 valid + 5 invalid). Pre-existing raw overlay discovered in `apps/backoffice/src/pages/inventory/MarginWatchPage.tsx:195` migrated to `<Dialog>` from `@breakery/ui` in the same commit (`8290d9e`).
