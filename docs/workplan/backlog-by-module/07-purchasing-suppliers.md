...
**Status note (2026-05-18)** : S22 update — DEV-S17-1.B-01 (manual `products.cost_price` UPDATE bypasses WAC + no audit row) **closed** by Stream B. Column-level `REVOKE UPDATE (cost_price) ON public.products FROM authenticated/anon/PUBLIC` (migration `20260526000010` + corrective `_013` to also revoke the silent table-level UPDATE on authenticated — see DEV-S22-1.B-02 medium) + new SECURITY DEFINER RPC `update_cost_price_v1` (migration `_012` + replay envelope fix `_014`) that emits `stock_movements` row `movement_type='cost_price_correction'` with old/new/reason in metadata jsonb. `receive_stock_v1` WAC path intact (verified by pgTAP T3 : cost mutated to 13.75 via WAC after a manual correction to 12.50). New permission `inventory.cost_correction` (granted to SUPER_ADMIN, ADMIN, MANAGER). Remaining scope on this TASK : landed cost shipping/douane pro-rata + `purchase_orders.allocation_method` column + `landed_unit_cost` recalc — still TODO, S23+ candidate.
