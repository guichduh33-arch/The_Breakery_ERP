...
> Refresh : intègre l'avancement S13-S18 (Sessions 13 à 18 toutes mergées)
...
### Transition V3 et avancement S13-S18

Entre Session 13 (2026-05-13) et Session 18 (2026-05-17), The Breakery a effectué une transition complète vers le monorepo V3 (pnpm + turbo, apps/pos + apps/backoffice + packages/{domain,supabase,ui,utils}) et résolu la majorité des gaps fonctionnels bakery historiquement identifiés.
...
2. **Hardening résiduel** — rate limiting `auth-verify-pin`, audit RLS `anon USING(true)` sur tables PII, message dedup LAN (TTL 5s), Playwright E2E en CI (D-W6-6C-05), `pg_net` birthday cron (D-W6-6B-02), Cash Flow Investing/Financing sections (D-W6-6A-2), staging-deploy.yml secrets (D-W6-CICD-01).
...
- **Session 19+** : triage du backlog résiduel post-S18, probables thèmes : compliance fiscale (si PKP confirmé) ; ou polish hardening (rate limiting + RLS anon) ; ou mobile shell ; ou WAC landed cost.
...
| 2 | Rate limiting IP-level sur `auth-verify-pin` Edge Function | 01-auth | P1 | M | `01-architecture-security-audit.md§P1-02` + `08-operations-lan-audit.md§P2-4` |
| 3 | Auditer & remplacer 16 RLS `anon USING(true)` par auth-only | 01-auth | P1 | L | `01-architecture-security-audit.md§P1-01` |
| 4 | Message dedup LAN (TTL 5s) hub + client | 21-lan | P1 | M | `08-operations-lan-audit.md§P1-1` |
| 5 | Vérifier phantom tables résiduelles : `system_alerts`, `customer_invoices` | 06-inventory + 08-customers | P2 | M | reliquat de l'item top-10 #9 historique (`stock_reservations` DONE S13 TASK-06-003) |
| 6 | Fix modal focus traps : migrer modales custom vers shadcn `Dialog` (Radix) | 02-pos + cross-modules | P1 | L | `05-uiux-design-audit.md§A1-3` |
| 7 | Playwright E2E en CI (D-W6-6C-05) | 23-tests | P2 | M | Workflow CI pre-S13, déféré Session 14+ |
| 8 | WAC landed cost shipping pro-rata (TASK-07-012 finir partial S17) | 07-purchasing | P3 | M | DEV-S17-1.B-01 + DEV-S17-1.C-01..02 |
| 9 | Mobile shell Capacitor + push native (TASK-15-009 + TASK-18-***) | 18-mobile | P3 | XL | Wave 7 deferred, Session 16+ scope |
| 10 | Compliance fiscale I1/I2/I3 (si PKP confirmé) | n/a (cross) | P0* | XL | `07-product-backlog-audit.md§Compliance` |
...
graph TD
  ...
  RLSAudit --> RateLimit[Rate limiting Edge Functions]
...
**Session 19+** : triage du backlog résiduel post-S18. Candidats prioritaires : compliance fiscale (si PKP confirmé) | polish hardening sécurité (rate limiting + RLS anon) | WAC landed cost.
...
| Test coverage modules critiques | 70 % lines / 60 % branches | À mesurer (S15 a ajouté ~50+ tests Vitest + pgTAP) |
| RLS `anon USING(true)` sur tables PII | 0 | 16 historiques (audit P1-01) — non traité S13-S18 |
| Findings P0 audits 2026-04 | 0 | DONE (Global Audit & Fixes 2026-04-09 + S13 P0 cleanup) |
| Findings P1 audits 2026-04 | < 10 | 37+ historiques, plusieurs résolus en S13-S18 (à recompter) |
| Migrations cloud V3 dev appliquées | monotonic, no drift | OK (`list_migrations` MCP, dernier bloc S18 `20260522000010`) |
...
|--------|---------|-------|
| Auth & Permissions | [`01-auth-permissions.md`](./01-auth-permissions.md) | 10 |
...
| Settings & Configuration | [`19-settings-configuration.md`](./19-settings-configuration.md) | 14 |
...
| Tests | [`23-tests.md`](./23-tests.md) | 12 (S16 pgTAP nightly) |
| Deployment & Ops | [`24-deployment-ops.md`](./24-deployment-ops.md) | 11 |
| Security | [`25-security.md`](./25-security.md) | 17 |

**Total : 25 modules, ~344 items, ~85 DONE (S13-S18).**
...
- **Append-only Status notes** : ne jamais réécrire une `**Status note (YYYY-MM-DD)**` existante. Ajouter une nouvelle ligne datée avec préfixe `S15 update:` / `S16 update:` / `S17 update:` / `S18 update:` (etc.) sous la note précédente.
