...
**S22 update:** RFC 9110 `Retry-After` header now set on every 429 response across the same 5 EFs (DEV-S19-2.A-02 closed). Helper `rateLimitedResponse(retryAfterSec)` in `supabase/functions/_shared/responses.ts` clamps with `Math.max(1, Math.ceil(...))` and adds `Access-Control-Expose-Headers: Retry-After` so browser fetch callers can read the header. Value dynamically sourced from `record_rate_limit_v1` return (`window_end - now()`). Live curl smoke confirmed `Retry-After: 57` on `auth-verify-pin` 4th attempt from same IP. Vitest live test `supabase/tests/functions/rate-limit-retry-after.test.ts` (5/5 green) exercises all 5 EFs with distinct RFC 5737 IPs to avoid cross-test bucket bleed.
