...
> (éditeur drag & drop grille 12×8 BO #273 — `set_table_position` ; rendu
> positionné POS + tablette #274) ; **business hours** + rapport Off-Hours Sales
> (PR #275 — `get_off_hours_sales`, page `/settings/business-hours`) ;
> **politique PIN configurable** (PR #276 — catégorie `security`, EF
> `auth-verify-pin`, page Security). Socle courant : `set_setting` /
> `get_settings_by_category`. Restent hors-code :
...
- Lecture `get_settings_by_category`, écriture `set_setting` (validation par
  ...
  `business_hours` (#275), v9 clés `security` PIN (#276), puis la bascule vers la clé
  unique `offline_payments_enabled` (ADR-015, PR #303) ; côté lecture, la
...
| `/backoffice/lan-devices` (groupe Network de la sidebar, hors hub `/settings`) | Registre `lan_devices` + heartbeat batch via le hub (EF `lan-heartbeat-batch`, PR #245) ; panneau **Hub** (état du bus LAN) ; carte **« Mode hors-ligne »** → `offline_payments_enabled`, clé unique (catégorie `network`) → POS : gate `useOfflinePaymentGate`, encaissement hors-ligne **toutes méthodes sauf l'avoir**, split débloqué, bannières offline (PR #248, ADR-015 PR #303) |
...
| **`tax_inclusive` global** | — | ✅ Livré — Lot 6a (#224, socle `_pb1_split`) + Lot 6b (#225 : bascule gatée côté `set_setting` + dialog BO, surfaces HT/TTC POS via `splitPb1`/`useTaxConfig`, flag produit déprécié UI+RPC, colonne conservée) |
