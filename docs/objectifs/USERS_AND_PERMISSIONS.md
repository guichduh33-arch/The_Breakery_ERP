...
> **Statut V2/V3** : décrit la vision business cible. **V2 jamais déployée**. Implémentation V3 = DONE (RPCs de gestion des utilisateurs, `apps/backoffice/src/features/users`, `SettingsPermissionsPage`, RBAC matrix). Améliorations V3 : `record_rate_limit` durable Postgres, GRANT hardening, `update_role_session_timeout`. Voir [`../V2_V3_GLOSSARY.md`](../V2_V3_GLOSSARY.md).
>
