...
> ⚠️ **Mise à jour S75 (2026-07-12, `swarm/session-75-floorplan-kds` + `swarm/session-75-kds-config`, PRs #209/#210)** : les **2 dernières tuiles « Planned (dedicated session) »** laissées par S73 sont livrées — **Floor Plan** (`/backoffice/settings/floor-plan`, CRUD `table_sections` + rattachement `restaurant_tables.section_id`, 6 RPCs gatés `tables.*`) et **KDS Configuration** (`/backoffice/settings/kds`, catégorie RPC `kds`, seuils `business_config.kds_*_threshold_minutes`/`kds_auto_archive_minutes`). **Le hub Settings BO est désormais 100 % câblé — zéro tuile Soon/Planned restante.** Détail KDS : fiche 04. Voir `docs/workplan/plans/2026-07-12-session-75-INDEX.md`.

> ⚠️ **Mise à jour S73 (2026-07-12, `swarm/session-73-settings`)** : **refonte Settings POS + BO livrée** (audit validé propriétaire 2026-07-11, `docs/workplan/audits/settings-pos-bo-audit.md`). **Hub sans cul-de-sac** : plus aucune tuile « (Soon) » — chaque tuile est liée, implémentée ou retirée ; seules **Floor Plan** et **KDS Configuration** restent visibles marquées « Planned (dedicated session) » ; tuiles Security/Accounting gatées par permission ; tuile **Settings History** → `/backoffice/reports/audit?action=setting.update`. **Nouvelles pages BO** : POS Configuration (`pos_presets` — montants rapides, fonds de caisse, presets de remise ; la tuile « (Soon) » de A. est fermée), Notifications (`notification_templates`, update-only, gate d'édition `notifications.send`), Customer Display + Printing org (colonnes `business_config` migration `_159`, catégories `customer_display`/`printing`). **General durci** : currency/timezone en selects ISO/IANA, `tax_rate` + seuils `_pct` affichés en % (stockage décimal [0,1] inchangé — garde testée), sections Identity/Cash. **POS** : verrou `settings.update` sur tous les onglets, badges portée org/terminal, onglet Automation supprimé (Printing = surface unique), presets de remise câblés dans les modals cart/ligne, copy display org (localStorage droppé). **Transverse** : dictionnaire typé `packages/supabase/src/settings-keys.ts` + doc `docs/reference/settings-authority-model.md`. B1.1 (hors logo/fidélité/imprimantes), B1.6 renforcé et le solde D1.1 (tuiles Soon) sont fermés ; NPWP (B5) reporté.

> ⚠️ **Mise à jour S64 (2026-07-06, `swarm/session-64`)** : **D2.1 livré** — les moyens de paiement sont **activables/désactivables** : colonne `business_config.enabled_payment_methods` (migration `_115`, défaut = les 6, CHECK non vide, validation whitelist dans `set_setting_v1` — chaque changement audité old/new), page BO **Settings → Payment Methods** (tuile « (Soon) » remplacée, gates `settings.read`/`settings.update`), effet POS ≤ 60 s sans redémarrage (hook fail-open `useEnabledPaymentMethods`). Le scénario phare « désactiver la carte en un clic » est désormais possible — B1.1c/B1.7 fermés. Enforcement UI v1 (l'EF accepte toujours les 6, dette D-1 INDEX S64). Voir `docs/workplan/plans/archive/2026-07-06-session-64-INDEX.md`.
...
> 🟡 **Mise à jour S76 (2026-07-13)** : **⚫#16/#17 re-statués « À venir »** (décision propriétaire 2026-07-13, pas un câblage réel) — les pages `settings/templates/email` (l.23) et `settings/templates/receipt` (l.24) affichent désormais un bandeau honnête « pas encore câblé » (`data-testid="templates-not-wired-banner"`) au lieu de laisser croire que l'éditeur/aperçu a un effet ; le vrai câblage (envoi e-mail réel, impression POS lisant `receipt_templates`) rejoint la Vague 3 (notifications / print-bridge versionné). Le verdict C-B1.4 (🟠 PARTIEL, l.62) est inchangé — seule la transparence UI change. Commit `0a744264`. Détail : [`../plans/2026-07-13-session-76-plan.md`](../plans/2026-07-13-session-76-plan.md).
