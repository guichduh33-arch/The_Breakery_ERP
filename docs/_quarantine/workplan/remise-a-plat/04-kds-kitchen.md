...
> ⚠️ **Mise à jour S75 (2026-07-12, `swarm/session-75-floorplan-kds` + `swarm/session-75-kds-config`, PRs #209/#210)** : **D2.1 partiellement livré (la part org)** — seuils warning/urgent/auto-archive réglables au niveau **organisation** via `business_config` (`kds_warning_threshold_minutes`/`kds_urgent_threshold_minutes`/`kds_auto_archive_minutes`, défauts 5/10/5, gardes 1..120 + warning<urgent, migration `_163`) + catégorie RPC `kds` (`get_settings_by_category_v1`/`set_setting_v1`) + page BO `/backoffice/settings/kds`, consommés par `useKdsConfig` (`KdsOrderCard` ageStyle, `useKdsAlarm`, auto-archive `KdsBoard`), appliqués en ~1 min ; le réglage **par poste** (police, disposition, mute par station) reste hors périmètre. **D1.4 livré** : chips `StationFilter` **câblés** — `useKdsOrders` résout `kds_station` via `products → categories.kds_station` (item sans station = passe tous les chips) ; fix latent connexe `CategoryFormDialog` BO (valeurs hors CHECK `kds_station`, remappées hot/cold/bar/prep/expo). C-B1.3/C-B1.8 réconciliés en section C. Voir `docs/workplan/plans/2026-07-12-session-75-INDEX.md`.
...
- **Chips de filtre granulaire** hot/cold/bar/prep/expo — `apps/pos/src/features/kds/components/StationFilter.tsx`. **Câblés depuis S75** : `useKdsOrders` résout `kds_station` via `products → categories.kds_station` (un item sans station passe tous les chips) — l'ancien no-op (le prédicat ne s'appliquait que si le champ existait sur la row, jamais sélectionné avant) est résolu. [UI + RPC câblés]
...
| B1.3 | Vert / orange / rouge clignotant + **alarme sonore** > 12 min | Couleurs + pulse ✓ ; seuils **réglables org depuis S75** (`business_config.kds_warning_threshold_minutes`/`kds_urgent_threshold_minutes`, défauts 5/10 min, BO `/backoffice/settings/kds`) — toujours pas 12 min, toujours pas par poste. Un re-bip urgent existe déjà (`useKdsAlarm.ts`) et consomme désormais ce même seuil org ; la mention « zéro audio » de cette ligne était déjà obsolète avant S75 — `KdsOrderCard.tsx` | 🟠 PARTIEL (S75 : seuils déhardcodés) |
...
| B1.5 | Vue serveur agrégée tous postes | Inexistante — un seul poste affiché à la fois (le chip « All » agrège mais pas de vue expo dédiée ; les chips par poste sont **câblés depuis S75** — l'ancien argument « chips no-op » ne vaut plus comme preuve) | 🔴 MANQUANT |
| B1.6 | Compteur du jour + disparition auto des terminées | Compteur du jour : **inexistant**. Disparition auto : ✓ archive auto (défaut 5 min, **réglable org depuis S75** — `kds_auto_archive_minutes`, `KdsBoard.tsx`) | 🟠 PARTIEL |
...
| B1.8 | Réglable par poste : seuils, sons, police, disposition | **Seuils réglables depuis S75, mais au niveau organisation, pas par poste** (`business_config.kds_*_threshold_minutes` + `kds_auto_archive_minutes`, BO `/backoffice/settings/kds`) ; toujours pas de sons/police/disposition réglables, toujours pas de granularité par poste (`posSettingsStore.ts` reste sans champ KDS) | 🟠 PARTIEL (S75) |
...
4. ✅ **Livré S75** — chips StationFilter câblés : `useKdsOrders` sélectionne `categories.kds_station` et filtre réellement (un item sans station passe tous les chips). Fichiers : `useKdsOrders.ts`, `KdsBoard.tsx`, `StationFilter.tsx`.
...
1. **Réglages par poste** — 🟠 **partiellement livré S75** : la part **organisation** est faite (seuils warning/urgent/auto-archive dans `business_config`, BO `/backoffice/settings/kds`, remplace les anciennes constantes codées en dur, consommés via `useKdsConfig`) ; reste ouvert : granularité **par poste** (mute son, taille de police) — champs dans `kdsStore` (persisté par appareil) + panneau réglages sur l'écran KDS.
