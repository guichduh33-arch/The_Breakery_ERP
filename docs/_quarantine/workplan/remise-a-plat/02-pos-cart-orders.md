...
> ✅ **Exigence propriétaire (2026-07-07) LIVRÉE le jour même (consignée via #162, implémentation PR #165, branche `worktree-fiche02-dinein-table-req`)** : **table obligatoire en dine-in + traçabilité des transferts.** (1) En `dine_in`, la table est **bloquante** au fire ET au checkout (garde partagée `useDineInTableGuard` — toast + ouverture auto du plan de salle + reprise de l'action ; filet serveur `table_required_for_dine_in` P0011 dans `fire_counter_order_v4`, migration **`20260710000122`** in-place depuis le corps live). (2) KOT : table + heure déjà portés ; le marqueur « adding order » est désormais **matérialisé serveur** — tout fire d'append écrit `audit_logs` `order.fire_appended` {order_number, items_count, table_number}. (3) **Transfert de table réel** : RPC **`transfer_order_table_v1`** (migration **`20260710000121`**, gate `pos.sale.create`, trio S20, destination validée `restaurant_tables` active, no-op même table) + audit **`order.table_transfer`** {from, to} lisible au BO via le journal d'audit ; UI = mode transfert dans `FloorPlanModal` (table occupée → « Transfer order » → table libre) ; le drawer BO de détail commande affiche `table_number`. pgTAP **`table_transfer_dinein_guard` 13/13 live** ; smoke garde 3/3 ; money-path v17/v11 non touchée. **Dettes D2.5** : (a) colonne table dans `OrdersListPage` = bump additif `get_orders_list_v2→v3` requis ; (b) pas de filet serveur au paiement direct dine-in sans fire (v17 = money-path, intouchée par décision) ; (c) transfert multi-commandes par table = la plus récente gagne ; (d) flux tablette non gaté (table optionnelle côté tablette).

> ⚠️ **Mise à jour S62 (2026-07-06, `swarm/session-62`)** : **D1.2 livré** — bouton « Ardoise » sur les lignes `pending_payment` de `HeldOrdersModal` → picker client → RPC `attach_tab_customer_v1` (attache le client + pose le total provisoire, gate plafond `retail_credit_limit` P0011). L'ardoise nommée apparaît dans `/pos/debts` et s'encaisse via le flux S60. Voir `docs/workplan/plans/archive/2026-07-06-session-62-INDEX.md`.

> ⚠️ **Mise à jour S60 (2026-07-05, `swarm/session-60`)** : **D1.1 livré** — le CTA « Pay » de `/pos/debts` charge la créance dans le panier (`useLoadDebtOrder`, mirror pickup) et l'encaissement route vers `pay_existing_order_v11` ; lignes B2B exclues (hint « settle in Backoffice »). Le B1.7 passe 🟠→✅ pour l'ardoise retail (D1.2 « bouton Ardoise nommé » reste ouvert). Voir `docs/workplan/plans/archive/2026-07-05-session-60-INDEX.md`.
