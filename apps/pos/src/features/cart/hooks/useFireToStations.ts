    ...
    mutationFn: async (ctx) => {
      ...
      if (!printOnly) {
        ...
        if (toPersist.length > 0) {
          ...
          // replay rejouera fire_counter_order avec le MÊME client_uuid au
          ...
          if (isOfflineMode() && existingOrderId === null) {
            ...
            // S44 P0-C(3) — fire_counter_order gates any line discount on an
            ...
            const { data, error } = await supabase.rpc('fire_counter_order_v6', {
              ...
              p_items: toPersist.map((i) => ({
                ...
                unit_price: i.unit_price,
                modifiers: i.modifiers,
                // S47 — combo lines persist their components so pay_existing_order
