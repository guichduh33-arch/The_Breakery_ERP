...
// (fire_counter_order_v5), et marquer les items locked+printed même si
...
describe('useFireToStations persists before printing (P0-3)', () => {
  ...
  it('calls fire_counter_order_v5 with ALL unprinted items, sets pickedUpOrderId, marks items even when print fails', async () => {
    ...
    expect(fnName).toBe('fire_counter_order_v5');
  ...
  it('printOnly (post-payment auto-fire): RPC is NOT called, items still seal and print', async () => {
    ...
    // pay_existing_order_v16) — persisting here would mint an orphan order
