...
//     forwards `clientUuid` as `p_client_uuid` to the `create_tablet_order_v6`
...
describe('S25 useCreateTabletOrder — idempotency wiring', () => {
  ...
  it('C1: passes the provided clientUuid as p_client_uuid to create_tablet_order_v6', async () => {
  ...
  it('C2: retry mutate with SAME clientUuid forwards the same p_client_uuid on both RPC calls', async () => {
    ...
    expect(firstCallArgs[0]).toBe('create_tablet_order_v6');
    expect(secondCallArgs[0]).toBe('create_tablet_order_v6');
