...
// (fire_counter_order_v6 append mode) AVANT le paiement, sinon le client paie
// un total partiel.
//
// Couvre :
...
describe('useCheckout — fired counter order syncs unfired items before paying (P0-3)', () => {
  beforeEach(() => {
    ...
    rpcMock.mockImplementation((fn: unknown) =>
      Promise.resolve(fn === 'fire_counter_order_v6' ? FIRE_OK : PAY_OK),
    );
  ...
  it('retry after an append FAILURE replays the SAME p_client_uuid (nothing was locked)', async () => {
    rpcMock.mockImplementation((fn: unknown) =>
      Promise.resolve(
        fn === 'fire_counter_order_v6' ? { data: null, error: { message: 'append_boom' } } : PAY_OK,
      ),
    );
    ...
    const fireCalls = rpcMock.mock.calls.filter((c) => c[0] === 'fire_counter_order_v6');
  ...
  it('retry after append SUCCESS + pay failure makes ZERO additional append calls (lines locked)', async () => {
    rpcMock.mockImplementation((fn: unknown) =>
      Promise.resolve(
        fn === 'fire_counter_order_v6' ? FIRE_OK : { data: null, error: { message: 'network' } },
      ),
    );
