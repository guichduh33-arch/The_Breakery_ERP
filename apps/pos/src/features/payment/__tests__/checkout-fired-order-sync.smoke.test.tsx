...
// pay_existing_order_v7 paie les order_items PERSISTÉS, pas le panier local —
// les items ajoutés APRÈS le dernier fire doivent être appendés à l'ordre DB
// (fire_counter_order_v1 append mode) AVANT le paiement, sinon le client paie
// un total partiel.
//
// Couvre :
...
describe('useCheckout — fired counter order syncs unfired items before paying (P0-3)', () => {
  ...
  it('appends the unsynced items (fire append mode) BEFORE pay_existing_order_v7', async () => {
    await runCheckout();
