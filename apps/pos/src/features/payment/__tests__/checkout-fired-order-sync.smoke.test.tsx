...
// pay_existing_order_v17 paie les order_items PERSISTÉS, pas le panier local —
// les items ajoutés APRÈS le dernier fire doivent être appendés à l'ordre DB
// (fire_counter_order_v7 append mode) AVANT le paiement, sinon le client paie
// un total partiel.
//
// Couvre :
...
describe('useCheckout — fired counter order syncs unfired items before paying (P0-3)', () => {
  beforeEach(() => {
    ...
    rpcMock.mockImplementation((fn: unknown) =>
      Promise.resolve(fn === 'fire_counter_order_v7' ? FIRE_OK : PAY_OK),
    );
  ...
  it('appends the unsynced items (fire append mode) BEFORE pay_existing_order_v17', async () => {
    await runCheckout();
  ...
  it('pays directly (no append) when every item was already fired', async () => {
    ...
    expect(rpcMock.mock.calls.map((c: unknown[]) => c[0])).toEqual(['pay_existing_order_v17']);
  });
  ...
  it('retry after an append FAILURE replays the SAME p_client_uuid (nothing was locked)', async () => {
    rpcMock.mockImplementation((fn: unknown) =>
      Promise.resolve(
        fn === 'fire_counter_order_v7' ? { data: null, error: { message: 'append_boom' } } : PAY_OK,
      ),
    );
    ...
    const fireCalls = rpcMock.mock.calls.filter((c) => c[0] === 'fire_counter_order_v7');
  ...
  it('retry after append SUCCESS + pay failure makes ZERO additional append calls (lines locked)', async () => {
    rpcMock.mockImplementation((fn: unknown) =>
      Promise.resolve(
        fn === 'fire_counter_order_v7' ? FIRE_OK : { data: null, error: { message: 'network' } },
      ),
    );
