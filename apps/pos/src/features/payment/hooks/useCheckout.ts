...
type PayExistingOrderArgs = Database['public']['Functions']['pay_existing_order_v15']['Args'];
...
interface CheckoutResponse {
  ...
  // S51 — server-authoritative subtotal + per-line breakdown (money-path v15).
  subtotal?: number;
...
export function useCheckout() {
  ...
  return useMutation({
    mutationFn: async (input: CheckoutInput): Promise<PaymentResult> => {
      ...
      // (complete_order_with_payment_v13 / pay_existing_order_v15). The client no
      ...
      if (pickedUpOrderId) {
        ...
        if (isCounterFired && unsynced.length > 0) {
          ...
          const { error: appendErr } = await supabase.rpc('fire_counter_order_v4', {
            ...
            p_items: unsynced.map((i) => ({
              ...
              // S47 — combo lines persist their components so pay_existing_order_v15
        ...
        // ADR-013 D9 — remise sur commande reprise : pay_existing_order_v15 exige
        ...
        const { error, data } = await supabase.rpc('pay_existing_order_v15', args as PayExistingOrderArgs);
        ...
        const envelope = data as unknown as {
          ...
          // omitted otherwise (le bump S51 concernait complete_order, pas pay_existing).
          lines?: PaymentResultLine[];
          loyalty_points_earned?: number;
          idempotent_replay: boolean;
        };
        ...
        return {
          ...
          // S51 — forward the server breakdown so the receipt consumes it.
          ...(envelope.subtotal != null ? { subtotal: envelope.subtotal } : {}),
          ...(envelope.lines ? { lines: envelope.lines } : {}),
      ...
      return {
        ...
        // S51 — the money-path v15 envelope carries the canonical subtotal + the
        // per-line breakdown; thread them so the receipt/display consume server
        // values instead of recomputing client-side.
