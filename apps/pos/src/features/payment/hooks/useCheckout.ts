...
type PayExistingOrderArgs = Database['public']['Functions']['pay_existing_order_v18']['Args'];
...
export function useCheckout() {
  ...
  return useMutation({
    mutationFn: async (input: CheckoutInput): Promise<PaymentResult> => {
      ...
      // (complete_order_with_payment_v13 / pay_existing_order_v18). The client no
      ...
      if (pickedUpOrderId) {
        ...
        if (isCounterFired && unsynced.length > 0) {
          ...
          const { error: appendErr } = await supabase.rpc('fire_counter_order_v7', {
            ...
            p_items: unsynced.map((i) => ({
              ...
              // S47 — combo lines persist their components so pay_existing_order_v18
        ...
        // ADR-013 D9 — remise sur commande reprise : pay_existing_order_v18 exige
        ...
        const { error, data } = await supabase.rpc('pay_existing_order_v18', args as PayExistingOrderArgs);
