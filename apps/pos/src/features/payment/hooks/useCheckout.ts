...
type PayExistingOrderArgs = Database['public']['Functions']['pay_existing_order_v11']['Args'];
...
export function useCheckout() {
  ...
  return useMutation({
    mutationFn: async (input: CheckoutInput): Promise<PaymentResult> => {
      ...
      // (complete_order_with_payment_v13 / pay_existing_order_v11). The client no
      ...
      if (pickedUpOrderId) {
        ...
        const { error, data } = await supabase.rpc('pay_existing_order_v11', args as PayExistingOrderArgs);
        ...
        const envelope = data as unknown as {
          ...
          // omitted otherwise (pay_existing_order_v11 is not part of the v15 bump).
          lines?: PaymentResultLine[];
          loyalty_points_earned?: number;
          idempotent_replay: boolean;
        };
