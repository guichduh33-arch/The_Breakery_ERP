...
// get_settings_by_category_v8 RPC, whose settings.read gate rejects
...
// WRITE: still set_setting_v10 (POSSettingsPage is manager-gated).
//
// Read shape :
...
export function usePOSPresets() {
  ...
  const mutateQuickPayments = useMutation({
    mutationFn: async (next: number[]) => {
      const { error } = await supabase.rpc('set_setting_v10', {
