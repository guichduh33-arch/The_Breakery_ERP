...
// get_settings_by_category_v6 / set_setting_v8 RPC pair (extended in
...
export function usePOSPresets() {
  ...
  const query = useQuery<POSPresets>({
    ...
    staleTime: 60_000,
    queryFn: async () => {
      const { data, error } = await supabase.rpc('get_settings_by_category_v6', {
  ...
  const mutateQuickPayments = useMutation({
    mutationFn: async (next: number[]) => {
      const { error } = await supabase.rpc('set_setting_v8', {
