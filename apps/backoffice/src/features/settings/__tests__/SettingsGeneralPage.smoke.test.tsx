...
// Lot 6b — set to true to make the next set_setting_v13('tax_inclusive') fail
...
describe('SettingsGeneralPage', () => {
  ...
  it('calls set_setting_v13 once per dirty key on submit', async () => {
    renderPage();
    await waitFor(() => screen.getByLabelText(/Business name/i));
    rpcCalls.length = 0;
    ...
    await waitFor(() => {
      const setCalls = rpcCalls.filter((c) => c.fn === 'set_setting_v13');
  ...
  it('changing tax_inclusive opens the confirmation dialog before any write', async () => {
    ...
    expect(rpcCalls.filter((c) => c.fn === 'set_setting_v13')).toHaveLength(0);
