...
describe('SettingsPrintingPage', () => {
  ...
  it('saves a KOT copies change as a number via set_setting_v11', async () => {
    canUpdate = true;
    ...
    await waitFor(() => expect(rpcCalls.some((c) => c.fn === 'set_setting_v11')).toBe(true));
  ...
  it('calls set_setting_v11 with the printing category on save', async () => {
    canUpdate = true;
    mockSettings = { pos_auto_print_receipt: true, pos_auto_open_drawer: false };
    rpcCalls.length = 0;
    render(wrap(<SettingsPrintingPage />));
