...
// Server-side (set_setting_v6, migration 20260712000163) enforces
...
describe('SettingsKdsConfigPage', () => {
  ...
  it('saves warning before urgent when urgent is not increasing (anti-22023 order)', async () => {
    ...
    await waitFor(() => expect(rpcCalls.filter((c) => c.fn === 'set_setting_v6')).toHaveLength(2));

    const sets = rpcCalls.filter((c) => c.fn === 'set_setting_v6');
  ...
  it('saves the archive delay last when all 3 fields change', async () => {
    ...
    await waitFor(() => expect(rpcCalls.filter((c) => c.fn === 'set_setting_v6')).toHaveLength(3));
