...
// Server-side (set_setting_v10, migration 20260712000163) enforces
...
vi.mock('@/lib/supabase.js', () => ({
  supabase: {
    rpc: (fn: string, args: unknown) => {
      ...
      if (fn === 'get_settings_by_category_v8') {
      ...
      return Promise.resolve({ data: null, error: null }); // set_setting_v10
    },
  },
}));
...
describe('SettingsKdsConfigPage', () => {
  ...
  it('saves warning before urgent when urgent is not increasing (anti-22023 order)', async () => {
    ...
    await waitFor(() => expect(rpcCalls.filter((c) => c.fn === 'set_setting_v10')).toHaveLength(2));

    const sets = rpcCalls.filter((c) => c.fn === 'set_setting_v10');
  ...
  it('saves the archive delay last when all 3 fields change', async () => {
    ...
    await waitFor(() => expect(rpcCalls.filter((c) => c.fn === 'set_setting_v10')).toHaveLength(3));
