...
// Session 27 — Wave 2 — Wraps update_product_v4 RPC (JSONB patch, 18-col allowlist).
//
...
/** Subset of ProductRow that update_product_v4 can write. */
export type ProductUpdatePatch = Partial<Pick<ProductRow,
  ...
  // Persisted via update_product_v4 allowlist (migration 20260710000043); an explicit
...
export function useUpdateProduct() {
  ...
  return useMutation<UpdateProductResult, Error, UpdateProductArgs>({
    mutationFn: async ({ productId, patch }) => {
      ...
      const { data, error } = await supabase.rpc('update_product_v4', {
