      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-4 py-4">
          <div className="col-span-1">
            <label htmlFor="sup-code" className="font-data font-semibold text-xs uppercase tracking-widest text-text-secondary">Code *</label>
            <input id="sup-code" value={draft.code} onChange={(e) => setField('code', e.target.value)}
              maxLength={32} disabled={mode === 'edit'}
              className={`h-9 w-full rounded-md border border-border-strong bg-bg-input px-3 text-sm font-mono uppercase text-text-primary disabled:opacity-50 ${FOCUS_RING}`} />
            {errors.code && <p className="text-red text-xs mt-1">{errors.code}</p>}
          </div>
          <div className="col-span-1">
            <label htmlFor="sup-name" className="font-data font-semibold text-xs uppercase tracking-widest text-text-secondary">Name *</label>
          ...
          <div>
            <label htmlFor="sup-phone" className="font-data font-semibold text-xs uppercase tracking-widest text-text-secondary">Phone</label>
          ...
          <div>
            <label htmlFor="sup-email" className="font-data font-semibold text-xs uppercase tracking-widest text-text-secondary">Email</label>
          ...
          <div className="col-span-2">
            <label htmlFor="sup-addr" className="font-data font-semibold text-xs uppercase tracking-widest text-text-secondary">Address</label>
            <input id="sup-addr" value={draft.address} onChange={(e) => setField('address', e.target.value)} maxLength={255}
              className={`h-9 w-full rounded-md border border-border-strong bg-bg-input px-3 text-sm text-text-primary ${FOCUS_RING}`} />
          </div>
          <div>
            <label htmlFor="sup-terms" className="font-data font-semibold text-xs uppercase tracking-widest text-text-secondary">Payment terms (days)</label>
          ...
          <div className="col-span-2">
            <label htmlFor="sup-notes" className="font-data font-semibold text-xs uppercase tracking-widest text-text-secondary">Notes</label>
            <textarea id="sup-notes" rows={3} value={draft.notes} onChange={(e) => setField('notes', e.target.value)} maxLength={500}
              className={`w-full rounded-md border border-border-strong bg-bg-input px-3 py-2 text-sm text-text-primary ${FOCUS_RING}`} />
          </div>
        </div>
