...
// T4: Apply after double-pick → rpcMock called with add_order_item_v2,
...
describe('ProductPicker smoke [S39-W-C1]', () => {
  ...
  it('T4: Apply after double-pick calls add_order_item_v2 with p_qty:2 and correct p_product_id', async () => {
    render(
      <Providers qc={qc}>
        <EditOrderItemsModal
          open
    ...
    // Orchestrator calls add_order_item_v2 once with the coalesced qty
    await waitFor(() => {
    ...
    const addCalls = rpcMock.mock.calls.filter(([fn]) => fn === 'add_order_item_v2');
