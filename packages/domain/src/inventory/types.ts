...
export type MovementType =
  ...
  | 'waste'
  | 'adjustment'
  // Phase 1 extensions. ADR-027 — `transfer_in` / `transfer_out` restent dans
  // l'enum DB (ledger append-only, lignes d'époque) : ils ne sont plus jamais
  // émis, mais restent lisibles et filtrables.
  | 'transfer_in'
  | 'transfer_out'
  | 'production_in'
  | 'production_out'
