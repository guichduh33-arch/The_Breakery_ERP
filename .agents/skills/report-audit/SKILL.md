...
Z-report flow, drill-down entities), read `reports-exports` instead of re-deriving it. `AGENTS.md` is the
...
- **AGENTS.md pattern compliance of a branch/diff** (REVOKE pairs, append-only ledgers, versioning) → `pattern-guardian`
...
- **No i18n.** No `useTranslation` / i18next — don't flag a missing `t()`. Mais l'absence de lib i18n n'est PAS une absence de règle :
  **l'interface parle ANGLAIS, les commentaires et la doc parlent français** (`AGENTS.md`). Les chaînes sont
