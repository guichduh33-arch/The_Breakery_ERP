...
description: >-
  Stock et production The Breakery : inventaire / inventory, WAC, recettes / recipes,
  réception / purchase orders, pertes / wastage, opname et mouvements append-only.
  Utiliser pour auditer ou modifier ces flux, avant toute RPC écrivant le stock.
  Péremption/FIFO et stock par section renvoient aux décisions existantes ADR-004 et
  ADR-027, sans rouvrir ces chantiers. Le catalogue relève de products-catalog ; les
...
[checklists détaillées](references/checklists.md). Avant de modifier un mouvement,
...
Hiérarchie de vérité (AGENTS.md) : **le code et le schéma DB** d'abord, puis `docs/adr/`,
