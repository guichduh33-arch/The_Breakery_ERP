...
**`AGENTS.md` est la source de vérité** for project-wide patterns (REVOKE pairs, RPC
versioning, idempotency flavors, PIN header). This skill adds B2B-specific mental model,
schema reality, and audit checklists that AGENTS.md doesn't carry.
...
4. **Idempotency flavor 2 (RPC arg)** — `record_b2b_payment`, `create_b2b_order`,
   ...
   Pattern AGENTS.md §"Idempotence, 2 saveurs". `record_b2b_payment` déduplique sur
...
12. **REVOKE pair canonique sur TOUTE RPC B2B** (PUBLIC + anon + `ALTER DEFAULT
    PRIVILEGES`), y compris les bumps et les RPC de lecture. Voir AGENTS.md
    §Critical patterns.
