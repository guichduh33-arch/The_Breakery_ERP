...
- **In flight:** **S56 — UI déférées + reliquat B2B/compta** (branche `swarm/session-56`) : **DEV-S54-01** bouton cockpit « Clôture annuelle » (`close_fiscal_year_v1`) + affichage erreur `period_undefined` ; **DEV-S52-03** liste-factures B2B BO (allocation ciblée `invoiceIds` dans `RecordB2bPaymentModal` + Cancel par facture, `useCancelB2bOrder` déjà câblé) ; **P2.2 reliquat** : consolidation audit `audit_log`/`audit_logs` sur 1 table. **P1 (P1.1→P1.5) : intégralement soldée** (S51→S55). **Reliquat Spec B (Phase 2 import) :** Sales + Expenses bulk import (specs ultérieures) ; Spec-B-1 design `docs/superpowers/specs/2026-06-26-pos-spec-b1-dispatch-routing-design.md`.
