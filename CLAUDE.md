...
- **In flight:** **Prochaine vague — B2B per-invoice settlement (P1.2)** (pas encore démarrée, pas de branche) : `b2b_payment_allocations` (allocation FIFO d'un paiement aux factures B2B individuelles) + `cancel_b2b_order_v1` (annulation propre avec réconciliation AR/JE). Suite directe de la tranche 2a-i (#132). Scope : §10 de `docs/workplan/plans/2026-06-28-session-51-INDEX.md`. **Reliquat Spec B (Phase 2 import) :** Sales + Expenses bulk import (specs ultérieures) ; Spec-B-1 design `docs/superpowers/specs/2026-06-26-pos-spec-b1-dispatch-routing-design.md`.
