...
- **Current session:** Session 16 — **CI revival + S15 follow-ups** (operational hygiene). Spec: [`docs/workplan/specs/2026-05-16-session-16-spec.md`](docs/workplan/specs/2026-05-16-session-16-spec.md). INDEX: [`docs/workplan/plans/2026-05-16-session-16-INDEX.md`](docs/workplan/plans/2026-05-16-session-16-INDEX.md). Branch `swarm/session-16` off `7ed9781` master. 4 waves, 6 executable phases, ~15h parallel. Scope: (1) drop broken Docker `supabase-tests` CI job, add nightly cloud pgTAP cron — resolves `DEV-S15-CI-01` (medium) ; (2) `is_semi_finished` product flag + pg_trgm indexes + `search_ingredients_v1` trigram ranking — resolves `DEV-S15-3.A-01/02` ; (3) per-version recipe cost embedded in `recipe_versions.snapshot` + history UI cost column — resolves `DEV-S15-2.B-01` ; (4) multi-level cascade in `IngredientAggregatePreview` via new `expandRecipeCascade` domain helper — resolves `DEV-S15-4.A-02`. Allergens on receipt deferred to S17. Migration block `20260520000001..099` (8 planned).
...
- **Session 15 follow-ups (deferred Session 17+):** Receipt template + customer display allergen integration (DEV-S15-5.C-01). The other S15 items (DEV-S15-3.A-01/02, DEV-S15-2.B-01, DEV-S15-4.A-02) are now Session 16 in-scope.
