...
- **In flight:** `feat/stock-tracking-deduct-flags` — **`track_inventory`/`deduct_stock` flag wiring + global negative-stock setting** (branch not yet merged): the two product-form toggles were inert (`deduct_stock` orphan, `track_inventory` only gating a pre-sale check while deduction ran unconditionally). Now **`track_inventory`** = the product carries its own counted stock (decremented at sale, raised at production); **`deduct_stock`** = consumes recipe materials (at production if tracked = croissant; at sale if not = café). New helper `_resolve_recipe_consumption_v1` (recursive BOM, **stop-at-tracked-nodes**, internal — REVOKE PUBLIC+anon+authenticated). Sale `complete_order_with_payment_v14` **replaced in place** (no version bump → EF/call-sites untouched): finished good decremented only if `track_inventory`, else recipe cascade if `deduct_stock`, else nothing. Production `record_production_v1` + batch `record_batch_production_v1` (v2 is a wrapper) gate consumption on `deduct_stock`. New `business_config.allow_negative_stock BOOLEAN DEFAULT true` (vente+production, default **permissive**) via `inventory` settings category + `record_stock_movement_v1` gains `p_allow_negative` (13-arg; primitive negative-guard bypass). BO **Réglages Inventory** page (toggle) + product-toggle subtitles clarified. Migrations `20260710000020..025` applied on cloud, types regen'd. Verified end-to-end via MCP jwt-sim (4 sale archetypes + negative gate; production block/allow/deduct_stock-false) + pgTAP suites + whole-repo typecheck 6/6 + build 2/2. Plan: `docs/superpowers/plans/2026-06-26-stock-tracking-deduct-flags.md`; spec: `docs/superpowers/specs/2026-06-26-stock-tracking-deduct-flags-negative-stock-design.md`. ⚠️ A subagent's `supabase migration repair` damaged the cloud `schema_migrations` bookkeeping (~400 clock-stamped rows dropped; max `20260629000012`) — actual schema intact, MCP `apply_migration` workflow unaffected; not reconstructed.
- **In flight:** `feat/pos-held-order-lifecycle` — Spec A **POS held-order lifecycle ("addition ouverte")** **shipped** (branch not yet merged): (1) data-only migration routing 6 sale categories to prep stations (`barista`←Coffee/Speciale Latte/Special Drinks, `kitchen`←Simple Plate/Panini/Savoury Croissant) to unblock Send-to-Kitchen (was 100% `dispatch_station='none'`); (2) two additive `_v1` RPCs — `hold_fired_order_v1` (flags a fired `pending_payment` PO order `is_held=true`) and `reopen_held_order_v1` (claims `is_held=false`, returns items + `is_locked`/`kitchen_status` for cart rehydration, no delete) — both with S25 REVOKE pair, applied on cloud (`20260710000010/11/12`), types regen'd. Front: `cartStore.reopenOrder` (rehydrates locks from `is_locked`→locked+printed), Send-to-Kitchen now parks the order in Held Orders + clears the terminal (`useHoldFiredOrder`), held list branches draft→`restore`/sent→`reopen` with Sent/Draft badge, `ADDITIONAL ORDER` KOT flag on appended fires. Verified: whole-repo typecheck CLEAN, build PASS, domain PASS, targeted POS smokes green incl. fixed fire/held smokes (`828fb95`); 3 pre-existing full-suite parallel-timeout flakes (variant-modal, pos-grid, product-tap) pass in isolation. Plan: `docs/superpowers/plans/2026-06-25-pos-held-order-lifecycle.md`; spec: `docs/superpowers/specs/2026-06-25-pos-held-order-lifecycle-design.md`. **Spec B (follow-up):** `display` station, transversal `waiter` destination, full category mapping.
- **Merged:** Phase 2a **historical purchases** bulk import (`feat/bulk-import-purchases`, now on `master`) — RPC `import_purchases_v1` (groups Excel rows by `po_reference` → `received` POs flagged `is_historical_import`, reports-only) + `BEFORE INSERT` payment-block trigger (`20260708000010/11`), `purchasesImportDef` + `useHistoricalPurchasesExport` + Template/Import/Export buttons & "Imported" badge on the PO list. Plan: `docs/superpowers/plans/2026-06-24-bulk-import-purchases.md`. **Remaining Phase 2:** Sales + Expenses bulk import (later specs).
- **Merged:** Phase 1 master-data bulk import (suppliers upsert + customers create-only) — PRs #114/#115. Generic `apps/backoffice/src/features/data-import/` framework (typed column-def, pure `.xlsx` parser, template/export builders, `useImportEntity` hook, 3-step `ImportEntityModal`) + RPCs `import_suppliers_v1`/`import_customers_v1` (idempotency table, anon defense-in-depth) wired to the Suppliers + Customers pages. Plan: `docs/superpowers/plans/2026-06-23-bulk-import-master-data.md`.
