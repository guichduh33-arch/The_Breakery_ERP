...
- **In flight:** `feat/bulk-import-master-data` — Phase 1 master-data bulk import (suppliers upsert + customers create-only) **shipped**: generic `apps/backoffice/src/features/data-import/` framework (typed column-def, pure `.xlsx` parser, template/export builders, `useImportEntity` hook, 3-step `ImportEntityModal`) + RPCs `import_suppliers_v1`/`import_customers_v1` (idempotency table, anon defense-in-depth) wired to the Suppliers + Customers page Template/Import/Export buttons. Migrations applied on cloud (`20260624135959`, `20260624140243`), types regen'd. Framework is ready to extend to **Phase 2** (sales/expenses/purchases + the PO-page button). Plan: `docs/superpowers/plans/2026-06-23-bulk-import-master-data.md`. Branch not yet merged.
