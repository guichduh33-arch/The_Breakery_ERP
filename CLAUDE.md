...
- **In flight (PR, `swarm/session-51`, stacked on `swarm/session-50`):** **Session 51 — Vague 2a money-path « prix-ligne canonique serveur »** (audit C8/C9 / P1.1). Money-path bumped **`complete_order_with_payment_v14 → v15`** (identical 16-arg signature, **DROP v14** same migration): the server is now the sole authority on line price — modifier `price_adjustment` + combo surcharges are re-read from `product_modifiers` (client price **ignored**), gift lines validated against `evaluate_promotions_v1` `free_items` (product+qty), and the return is enriched with a server-authoritative **`lines[]`** breakdown. New internal helper **`_resolve_line_price_v1`** (SECURITY DEFINER STABLE, REVOKE PUBLIC+anon+authenticated). EF `process-payment` repointed to v15 + redeployed (v13); the POS receipt/customer-display now **consume the server `lines[]`/`tax_amount`** and `useTaxRate()` reads `business_config.tax_rate` (no more hardcoded `0.10` on checkout). ⚠️ **Grant caveat caught:** v15 MUST `GRANT EXECUTE TO authenticated` (EF calls via the user JWT) — a missing grant in the first draft would have broken every checkout; mirror v14's grant block on any future bump. Migrations `20260710000063..064`. pgTAP `canonical_line_price` 13/13 + 7 baseline suites realigned green. Deferred → **B2B per-invoice settlement (P1.2)** is the next wave. Spec `docs/superpowers/specs/2026-06-28-pos-canonical-server-line-price-design.md`; INDEX `docs/workplan/plans/2026-06-28-session-51-INDEX.md`.
- **In flight (PR, `swarm/session-50`):** **Session 50 — Vague 1 « cutover sain »** (security + drift + CI hardening; **does NOT touch the money-path** — that is Vague 2). Closed deploy-drift #125 (reapplied `_resolve_dispatch_stations_v1` + `categories.dispatch_station` idempotently); gated 5 financial/report RPCs (`get_general_ledger`/`get_trial_balance`/`get_profit_loss`/`get_balance_sheet`/`get_sales_by_hour` → `_v2` with `has_permission`); closed `audit_log` (security_invoker) + MV (`mv_pl_monthly`/`mv_sales_daily`/`mv_stock_variance` REVOKE) leaks; gated BO routes/sidebar (`products.read`, new `b2b.read`, new `settings.security.manage`) + customer RPCs (`search_customers_v3`/`get_customer_v3`); activated the CI net (`supabase/tests` workspace, nightly live-RPC + drift jobs, blocking `pgtap-pr`, types-regen gate). Migrations `20260710000051..056`. New perms `b2b.read`, `settings.security.manage`. **Deferred (S51+):** POS-view security_invoker (would break CASHIER/waiter cascade), `product-images` bucket privatization, INVOKER-fn search_path, Leaked Password Protection (manual), `SUPABASE_SERVICE_ROLE_KEY` repo secret. Spec/plan/INDEX: `docs/workplan/{specs,plans}/2026-06-27-session-50-*.md`.
