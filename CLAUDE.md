...
- **In flight:** `feat/bulk-import-purchases` — Phase 2a **historical purchases** bulk import **shipped** (branch not yet merged): RPC `import_purchases_v1` (groups Excel rows by `po_reference` → inserts `received` POs flagged `is_historical_import`, **reports-only** — zero GRN/stock/JE) + `BEFORE INSERT` trigger blocking payments on historical-import POs, both applied on cloud (`20260708000010`, `20260708000011`), types regen'd. Front: `purchasesImportDef` + `useHistoricalPurchasesExport` hook + Template/Import/Export buttons & "Imported" badge wired on the PO list page, reusing the Phase 1 `data-import/` framework. Verified: targeted Vitest 2/2 + full BO suite green (modulo the known 4-test env-gated baseline) + clean build. Plan: `docs/superpowers/plans/2026-06-24-bulk-import-purchases.md`. **Remaining Phase 2:** Sales + Expenses bulk import (later specs).
