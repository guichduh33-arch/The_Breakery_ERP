...
- **In flight:** **Prochaine vague — P1.3 (T6) correctness compta** (pas encore démarrée, pas de branche). **P1.3 (T6)** : TB cumulative as-of (déjà `get_trial_balance_v3` en 2a-i — vérifier), dédup PB1 void+refund, garde fiscale fail-closed, clôture annuelle (carry-forward → 3200). Source : §4 P1 de `docs/workplan/audits/2026-06-27-audit-integral-par-module.md`. **P1.4 (T2) unification stock : FAIT en S53** (voir « Merged (latest) »). **Suite UI S52 (DEV-S52-03)** : surface liste-factures B2B BO → adopter l'allocation ciblée (`invoiceIds`) dans `RecordB2bPaymentModal` + bouton Cancel par facture (`useCancelB2bOrder` déjà câblé). **Reliquat Spec B (Phase 2 import) :** Sales + Expenses bulk import (specs ultérieures) ; Spec-B-1 design `docs/superpowers/specs/2026-06-26-pos-spec-b1-dispatch-routing-design.md`.
...
- **Latest on `master`:** PR **#135** (S52 B2B settlement, doublon no-op de #134 ; voir « Merged (latest) »). Avant : #132. Historique antérieur (held-order #120/#121, Phase 2a #116, Phase 1 #114/#115, #117 Cost Analytics, #118 POS P0 hardening, #106/#103/#95/#98/#99/#100) archivé dans [`docs/workplan/2026-06-26-claude-md-workplan-archive.md`](docs/workplan/2026-06-26-claude-md-workplan-archive.md). Détail par session (S13→S49) dans `docs/workplan/` — ouvrir l'INDEX daté.
