...
- **Session 13 follow-ups (deferred Session 17+):** Playwright CI job (D-W6-6C-05), `pg_net`-based birthday cron (D-W6-6B-02), Cash Flow Investing/Financing sections (D-W6-6A-2), `mv_pl_monthly` branched reuse (D-W6-6A-1), staging-deploy.yml secrets (D-W6-CICD-01).
- **Session 15 follow-ups still deferred:** Receipt template + customer display allergen integration (DEV-S15-5.C-01).
- **Session 16 follow-ups (deferred Session 17+):** Trigram indexes not yet engaged by RPC predicate (DEV-S16-2.A-01, would need `%` operator + `set_limit`) ; full-cascade per-version cost in snapshot (DEV-S16-2.B-01, currently depth-1) ; legacy `recipe_versions` rows have no cost reconstructible (DEV-S16-2.B-02) ; `WHEN OTHERS` cleanup in `tr_snapshot_recipe_version` (DEV-S16-2.B-03, carryover from S15) ; NULL `material_cost_price` silently dropped via SUM in trigger + refresh (DEV-S16-2.B-04, COALESCE fix) ; refresh `change_note` could be more descriptive (DEV-S16-2.B-05) ; UI BFS only goes depth-2 vs walker's MAX_BFS_DEPTH=5 (DEV-S16-2.C-01/02, future `recipe_bom_full_v1` RPC) ; nightly pgTAP is the only automated check, no PR-time gate (DEV-S16-1.A-01).
...
- **Backlogs:** `docs/workplan/backlog-by-module/01-…25-….md` — 25 modules, ~280 tasks total. Session 17 source : TBD (open with backlog triage post-S16 merge).
...
- **Migration sequence active:** Session 16 reserved block `20260520000001..099` (8 planned). Session 15 used `20260519000001..162` (32 migrations). Session 14 used `20260518000001..003`. Session 13 used `20260517xxxxxx`. Keep numbering monotonic — check `supabase/migrations/` before picking the next number.
