...
- **In flight:** rien — **P1 (audit 2026-06-27) intégralement fermé** (P1.1→P1.5 = S50/S51→S55). Prochaine vague à trier : **P2** (§4 de l'audit : plafonds promo + validation combo serveur, consolidation audit 1 table, états d'erreur POS/KDS, empty-states BO, lint-ratchet) ou backlog par module. **UI déférées** : **DEV-S54-01** bouton cockpit « Clôture annuelle » + erreur `period_undefined` ; **DEV-S52-03** liste-factures B2B BO (allocation ciblée `invoiceIds` dans `RecordB2bPaymentModal` + Cancel par facture, `useCancelB2bOrder` déjà câblé). **Follow-ups S55 (backlog, non bloquants)** : purge périodique `discount_authorizations` ; retirer le fallback vestigial `body.discount_authorized_by` de process-payment ; aligner les 3 calculs hasDiscount (POS/EF/RPC) ; discount `pay_existing_order_v11`/`fire_counter_order_v4` sans PIN (état S44). **Reliquat Spec B (Phase 2 import) :** Sales + Expenses bulk import (specs ultérieures) ; Spec-B-1 design `docs/superpowers/specs/2026-06-26-pos-spec-b1-dispatch-routing-design.md`.
...
- **Merged (latest):** **S55 — Vague P1.5 « Durcissement EF restant » (branche `swarm/session-55`).** Ferme audit **T7**. **(A) Idempotency void/cancel** : `void_order_rpc_v3→v4` (replay via `refunds.idempotency_key`) et `cancel_order_item_rpc_v2→v3` (+ colonne `order_items.cancel_idempotency_key` UNIQUE partiel) — replay = même enveloppe + `idempotent_replay:true`, DROP anciennes signatures, **service_role-only avec REVOKE authenticated** (`_084` + in-place — le bloc 3-lignes hérité de `_018` était fautif, même classe que l'incident `20260709000010` ; suite `reversal_rpc_revoke` 11/11 refresh v4/v3) ; EFs void-order/cancel-item lisent `x-idempotency-key` (helper S25) ; POS clés per-modal (pattern `RefundOrderModal`). **(B) Discount-PIN hors arg SQL money-path** : table nonce **`discount_authorizations`** (service-role only, TTL 60 s, single-use) ; **`complete_order_with_payment_v15→v16`** — le check `p_manager_pin`+`_verify_pin_with_lockout` remplacé par la consommation atomique du nonce (message P0003 inchangé), DROP v15 ; l'EF `process-payment` vérifie le PIN in-EF (`_shared/manager-pin`, bucket SEC-07 partagé) et **dérive `p_discount_authorized_by` du PIN vérifié** (body plus cru), mint le nonce, appelle v16 — contrat HTTP POS inchangé. ⚠️ **v16 DOIT `GRANT EXECUTE TO authenticated`** (caveat S51 reconduit). **T0** : les 2 items d'audit « déjà faits » (rate-limit `auth-change-pin`, secret dispatch en header) n'étaient PAS déployés (drift repo↔cloud, DEV-S55-01) → redéployés. Migrations **`20260710000082..086`** (`_086` via runner API-from-file, DEV-S55-06). pgTAP live : reversal_idempotency 5/5, discount_auth_nonce 6/6, revoke 11/11 + 9 ancres repointées v16/v4 vertes ; typecheck 6/6, build 2/2, POS 133/133. Types regénérés. Review finale whole-branch : READY TO MERGE, 0 Critical/Important. INDEX `docs/workplan/plans/2026-07-02-session-55-INDEX.md` ; spec `docs/superpowers/specs/2026-07-02-ef-hardening-p15-design.md`.
- **Previously merged (S54):** **Vague P1.3 « Correctness compta » (branche `swarm/session-54`, PR #137).** Ferme audit **T6/C10**. **`check_fiscal_period_open` fail-closed** (`_077`, in-place) : aucune période couvrante → `period_undefined` P0004 (les 34 call-sites traitent déjà P0004). **Fix leak cumul `get_trial_balance_v3`** (`_078`, in-place) : le double LEFT JOIN de `_061` laissait les JE draft/futures/void-dédupliquées polluer le cumul des comptes permanents — join interne parenthésé. **`close_fiscal_year_v1(p_fiscal_year, p_manager_pin)`** (`_079` permission `accounting.year.close` + `_080` contrainte+RPC) : préconditions 12 périodes closed/locked (`FOR UPDATE`) + pas de `year_close` existante ; JE `year_close` 31/12 zérotant classes 4/5/6 (dédup canonique) → contrepartie **3200** (CR profit / DR perte) ; **seed 12 périodes N+1** (neutralise la bombe à retardement du fail-closed) ; `_verify_pin_with_lockout` ; `reference_type='year_close'` ajouté à la CHECK. **Exclusion `year_close` des rapports** (`_081`, in-place ×2) : P&L v2 (WHERE) ; TB v3 colonnes de période seulement (ouverture/cumul l'incluent) ; **BS v2 inchangé** (CYE YTD retombe à 0 au 31/12). Migrations **`20260710000077..081`**. pgTAP : `fiscal_guard_fail_closed` 4/4, `close_fiscal_year_v1` 19/19, `trial_balance_v3_cumulative` +T7 ; ancres PB1 3/3 + gates + sanity P&L ; typecheck 6/6, build 2/2. Types regénérés (+`close_fiscal_year_v1`). INDEX `docs/workplan/plans/2026-07-02-session-54-INDEX.md` ; spec `docs/superpowers/specs/2026-07-02-fiscal-correctness-design.md`.
...
- **Latest on `master`:** PR **#136** (S53 sale-stock unification). Avant : #135/#134 (S52), #132. Historique antérieur (held-order #120/#121, Phase 2a #116, Phase 1 #114/#115, #117 Cost Analytics, #118 POS P0 hardening, #106/#103/#95/#98/#99/#100) archivé dans [`docs/workplan/2026-06-26-claude-md-workplan-archive.md`](docs/workplan/2026-06-26-claude-md-workplan-archive.md). Détail par session (S13→S49) dans `docs/workplan/` — ouvrir l'INDEX daté.
...
- **Order writes go through RPCs** — never raw inserts. The POS POSTs the `process-payment` EF (`apps/pos/src/features/payment/hooks/useCheckout.ts`), which server-side calls the current money-path RPC `complete_order_with_payment_v16` (`supabase/functions/process-payment/index.ts`) — the POS never calls it directly. **Discount-PIN (S55 T7)** : le PIN ne descend plus en arg SQL — l'EF le vérifie (`_shared/manager-pin.ts`) et mint un nonce single-use `discount_authorizations` (service-role only, TTL 60 s) que v16 consomme atomiquement ; `p_discount_authorized_by` est dérivé serveur du PIN vérifié. Reversals (EF-only, service_role + REVOKE authenticated) : `void_order_rpc_v4`, `cancel_order_item_rpc_v3`, `refund_order_rpc_v4` — tous idempotents via `x-idempotency-key` (S55 : replay = enveloppe + `idempotent_replay:true`). Other order RPCs: `pay_existing_order_v11` (S53, flag-aware), `fire_counter_order_v4`, `create_tablet_order_v2`, `pickup_tablet_order` (unversioned), `evaluate_promotions_v1`, `mark_item_served` (unversioned). B2B order/AR RPCs: `create_b2b_order_v3` (S53, display-aware), `record_b2b_payment_v2`, `cancel_b2b_order_v1`, `adjust_b2b_balance_v2`, `get_pos_b2b_debts_v3`, `reconcile_b2b_balance_v1` (+ append-only `b2b_payment_allocations`). They handle JE triggers, loyalty, promotions, table state atomically. **Sale-time stock deduction (S53 P1.4)** : les 3 RPCs de vente (`complete_order_with_payment_v16`, `create_b2b_order_v3`, `pay_existing_order_v11`) déduisent via l'unique helper interne **`_record_sale_stock_v1`** (flag-aware + display isolation) — plus aucun `INSERT INTO stock_movements` brut en flux de vente. **RPC versions bump nearly every session — always verify the live version in `supabase/migrations/` + the call-site before relying on a number.**
